...
class AbstractOpenaiJSONQueryClient(AbstractLLMJSONQueryClient):
    ...
    def run_query(self, temperature: float = 1.0, **request_params) -> str | dict:
        for _ in range(self.retry_count):
            ...
            response = self.receive_response(messages=messages, temperature=temperature)
    ...
    def receive_response(self, messages: list[OpenaiRequestMessage], temperature: float):
