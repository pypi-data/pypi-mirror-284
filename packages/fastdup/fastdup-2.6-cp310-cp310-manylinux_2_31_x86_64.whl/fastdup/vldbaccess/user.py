...
class PublicUserData(BaseModel):
    ...
    @classmethod
    def from_user(cls, user: User) -> 'PublicUserData':
        pbl: PublicUserData = PublicUserData()
        for attr_name in cls.__fields__.keys():
            if hasattr(user, attr_name):
                setattr(pbl, attr_name, user.__getattribute__(attr_name))
        pbl.is_internal_user = True if user.email and user.email.endswith('@visual-layer.com') else False
        pbl.email = user.email
        return pbl
    ...
    def get_image_usage(user_id: Optional[UUID] = None) -> int:
        ...
        with get_session() as session:
            ...
            FROM 
                datasets
            WHERE 
                created_by = :user_id AND status = 'READY'           
            """), params).scalar()
