...
如果有看到 pdf-page-screenshot 的 tag，在結尾附上點擊的連結 markdown，連結不會用 http或 https開頭，直接完整放 tag 裡有的資訊就好，不要將一開始的 / 移除
範例：
原本要回答的內容
    ...
    for idx, hit in enumerate(search_result, start=1):
        ...
        if google_file_id and page_number and page_number.lower() != 'n/a':
            ...
            str_knowledge_base += (f"<pdf-page-screenshot>\n"
                                   f"/api/users/botrun_ask_folder/img/{google_file_id}/{page_number}.png\n"
