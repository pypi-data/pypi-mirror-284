...
class RVC:
    ...
    def from_pretrained(
        ...
        **from_pretrained_kwargs,
        ...
        """
        return RVC(name, sr, hubert, rmvpe, accelerator, from_pretrained_kwargs)

    def convert(self, audio: str | Dataset | np.ndarray, protect=0.33):
        """
        Converts the input audio to the target voice using the pre-trained model.
        ...
        if isinstance(audio, str):
            return self.convert_file(audio, protect)
        if isinstance(audio, Dataset):
            return self.convert_dataset(audio, protect)
        return self.convert_from_wav16k(audio, protect)

    def convert_dataset(self, dataset: Dataset, protect=0.33):
        """
        Converts a dataset of audio samples to the target voice using the pre-trained model.
        ...
        for i, data in enumerate(dataset):
            ...
            pitchf = data["f0nsf"]
            pitch = data["f0"]
            yield self.convert_from_features(phone, pitchf, pitch, protect)

    def convert_file(self, audio: str, protect=0.33) -> np.ndarray:
        """
        Converts a single audio file to the target voice using the pre-trained model.
        ...
        return self.convert_from_wav16k(wav16k, protect)

    def convert_from_wav16k(self, wav16k: np.ndarray, protect=0.33) -> np.ndarray:
        """
        Converts a 16kHz waveform to the target voice using the pre-trained model.
        ...
        Returns:
            np.ndarray: The converted audio in the target voice.
        """
        wav16k = np.pad(wav16k, (SR_16K, SR_16K), mode="reflect")
        logger.info(f"Padded audio with shape {wav16k.shape}")

        pitchf, pitch = self.rmvpe.extract_f0_from(wav16k)
        phone = self.hubert.extract_feature_from(wav16k)

        return self.convert_from_features(phone, pitchf, pitch, protect)

    def convert_from_features(
        self, phone: np.ndarray, pitchf: np.ndarray, pitch: np.ndarray, protect=0.33
    ) -> np.ndarray:
        """
        Converts audio features (phone, pitchf, pitch) to the target voice using the pre-trained model.
        ...
        with torch.no_grad(), self.accelerator.device:
            ...
            return audio_segment[self.sr : -self.sr]
