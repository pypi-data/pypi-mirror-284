...
class ImportStatement(Statement):
    def __init__(
        self, code: str, start: int, end: int, path: Token, _as: Token | None = None
    ):
...
class IntroduceVariableStatement(Statement):
    def __init__(self, code: str, start: int, end: int, name: Token, varType: Token):
...
class ReturnStatement(Statement):
    def __init__(
        self, code: str, start: int, end: int, keyword: Token, expr: List[Token]
    ):
...
class InlineStatement(Statement):
    def __init__(self, code: str, start: int, end: int, expr: List[Token]):
...
def parse_iterate(
    ...
    if t0.value == "import":
        ...
        statement = ImportStatement(
            code=t0.code, start=t0.start, end=(_as or t1).end, path=t1, _as=_as
        )
    ...
    if t0.value == "define":
        ...
        statement = IntroduceVariableStatement(
            code=t0.code, start=t0.start, end=t2.end, name=t2, varType=t1
        )
        statements.append(statement)
        return True

    if t0.value == "fn":
        # fn <name>( <arguments> ): <type> { <body> }
        t1 = next_token(tokens, index)
        t2 = next_token(tokens, index)
        t3 = next_token(tokens, index)
    ...
    statement = InlineStatement(
        code=t0.code, start=t0.start, end=expr[-1].end, expr=expr
    )
    statements.append(statement)
    index[0] = ind
    return True
