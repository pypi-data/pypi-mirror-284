...
class ByteHouseAdapter(SQLAdapter):
    ...
    @available
    def get_sql_insert_values_data(self, table):
        values_list = []

        for row in table.rows:
            # Prepare the value list for each row
            row_values = ", ".join(
                "NULL" if d is None else f"'{c.csvify(d)}'" for c, d in zip(table._column_types, row)
            )
            values_list.append(f"({row_values})")

        # Combine all value lists into a single VALUES clause
