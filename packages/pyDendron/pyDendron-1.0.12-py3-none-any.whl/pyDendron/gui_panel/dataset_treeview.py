...
class DatasetTreeview(Viewer):
    ...
    def __init__(self, dataset, parameters, **params):
        ...
        offsets_items = [('Normalize offsets', 'o'), ('Offsets to Years', 'o2y'), ('Years to offsets', 'y2o'), ('Get GPS code', 'localisation'), ('Current column value to children', 'propchild'), ('Current values to selected', 'propselect')]
        self.bt_tools = pn.widgets.MenuButton(name='Tools', items=offsets_items, icon='adjustments', button_type='primary', align=('start', 'end'))
        self.bt_tools.on_click(self.on_tools)

        self.bt_chronology = pn.widgets.Button(name='Average', icon='tournament', button_type='primary', align=('start', 'end'))
        ...
        self.bt_package = pn.widgets.MenuButton(name='Create package', items=package_items, split=True, icon='cube-plus', button_type='primary', align=('start', 'end'))
        self.bt_package.on_click(self.on_package)
        
        create_items = [('Create & add selection', 'a'), ('Regexp create & add selection', 'b')]
        self.bt_create = pn.widgets.MenuButton(name='Create set', items=create_items, split=True, icon='folder-plus', button_type='primary', align=('start', 'end'))
        ...
        self.bt_goto = pn.widgets.Button(name='Go to', icon='direction-sign', button_type='primary', align=('start', 'end'), width=bt_size)
        self.bt_goto.on_click(self.on_goto)

        self.row_cmd = pn.Row(self.bt_chronology, self.bt_tools, self.bt_package, self.bt_create, self.w_set_name)
    ...
    def on_paste(self, event):
        ...
        if len(self.path) == 0:
            logger.warning('cannot perform paste in root')            
        try:
            self._layout.loading = True
