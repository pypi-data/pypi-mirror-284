...
from pyDendron.app_logger import logger, catch_bokeh_log
...
class DendronApp:
    ...
    def __init__(self, dendron_info):
        ...
        self.tabs = tabs = pn.Tabs(self.treeview, self.tab_package, self.ploter, self.crossdating, self.tools, ('Debug', self.debug),
                dynamic=True, margin=0, styles={'padding' : '0px', 'font-size': '18px'})
    ...
    def get_template(self):
        ...
        template.header.objects = [self.bt_notification(), self.parameters.get_widgets(), self.bt_gps()]
        return template
...
def cb_disconnection():
    logger.info(f'cb_disconnection')
    cb_auto_save()
    app.parameters.on_save_param(None)
    data = (app.dataset.filename, dendron_info)
    pn.state.cache['context'] = data
    logger.info(f'Save context: {data}')

def cb_connection():
    from pyDendron.app_logger import logger
    logger.info(f'cb_connection')
    if 'context' in pn.state.cache:
        data = pn.state.cache['context']
        logger.info(f'Load context: {data}')
...
if __name__.startswith('bokeh_app'):
    ...
    catch_bokeh_log(cb_connection, cb_disconnection)
    # pn.state.on_session_destroyed(cb_end_session)
    #pn.state.on_session_created(cb_session_created)
