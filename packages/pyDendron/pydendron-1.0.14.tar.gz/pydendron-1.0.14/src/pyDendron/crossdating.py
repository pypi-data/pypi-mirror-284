...
def worker(instance, i):
    print('worker', list(sys.modules.keys()))
    print('-'*20)
...
def set_worker(arg1, arg2, arg3):
    ...
    data = arg2
    global master_data 
    master_data = data if self_crossdating else arg3
...
class CrossDating(param.Parameterized):
    ...
    def _run_thread(self, data, master_data):
        ...
        with ProcessPoolExecutor(max_workers=self.num_threads, initializer=set_worker, initargs=(self.self_crossdating, data, master_data)) as executor:
            self.futures = [executor.submit(worker, worker_class, i) for i, idx in enumerate(data.index[:last_data])]
            res_lst = []
        ...
        return self.results
            ...
            #         if self.stop:
            #             print("Interruption des tâches demandée par l'utilisateur.")
            #             for f in futures:
            #                 f.cancel()  # Annuler les futures en cours
            #             return
