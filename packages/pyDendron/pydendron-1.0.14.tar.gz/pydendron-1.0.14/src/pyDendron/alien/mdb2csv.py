# based on https://github.com/jbn/pandas_access
...
if os.name != 'nt':
    ...
    CMD_MDB_SCHEMA = 'mdb-schema'
else:
    CMD_MDB_EXPORT = 'mdb-export.exe'
    CMD_MDB_SCHEMA = 'mdb-schema.exe'
...
def _extract_dtype(data_type):
    ...
    if data_type.startswith('double') or data_type.startswith('single'):
        return 'Float32'
    elif data_type.startswith('long') or data_type.startswith('integer'):
        return 'Int32'
...
def read_schema(rdb_file, encoding='utf8'):
    ...
    """
    output = subprocess.check_output([CMD_MDB_SCHEMA, rdb_file])
...
def read_table(rdb_file, table_name, sep='@', *args, **kwargs):
    """
    ...
    cmd = [CMD_MDB_EXPORT, '-d', sep, '-e', '-Q', rdb_file, table_name]
    #print(cmd)
    proc = subprocess.Popen(cmd, stdout=subprocess.PIPE)
    return pd.read_csv(proc.stdout, sep=sep, *args, **kwargs)

# rdb_file='/Users/meignier/Dendro/dataset/sylphe/didier/SylpheDidier.mdb'
# table_name='Sequence'
# df = read_table(rdb_file, table_name, converters_from_schema=False, usecols=sylphe_col_sequences.keys())
# print(df)