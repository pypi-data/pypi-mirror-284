...
class PointsSimplifier:
    ...
    def calculate_length_between_points_in_line(points: list, start_index: int, end_index: int) -> float:
        ...
        for i in range(start_index, end_index):
            p1 = points[i]
            p2 = points[i + 1]
            distance = LineBase.calculate_two_points_distance(p1, p2)
            length += distance
        return length
    ...
    def simplify_points(points: list, max_distance: float = 0.3, max_length: float = 1000) -> list:
        ...
        while end_index < num:
            ...
            length = PointsSimplifier.calculate_length_between_points_in_line(points, start_index, end_index)
            if length > max_length:
                ...
                for k in range(start_index + 1, end_index):
                    mid_point = points[k]
                    distance = PointsSimplifier.calculate_distance_from_point_to_line(mid_point, start_point, end_point)
...
class LinkPointsSimplifier:
    @staticmethod
    def simplify_link(points: list, lanes_points: list, max_distance: float = 0.3, max_length: float = 1000):
        simplified_index = PointsSimplifier.simplify_points(points, max_distance, max_length)
