...
from .LinkPointsDivider import PointsDivider
...
class PointsSplitter:
    ...
    def split_line_by_distance_and_mode(line: list, given_distance: float, mode: int):
        assert len(line) >= 2
        assert mode in [0, 1]  # 0:start, 1:end
        ...
        divided_points, divide_infos = PointsDivider.divide_line_by_distances(line, [given_distance])
        saved_line = divided_points[1] if mode == 0 else divided_points[0]

        return saved_line, divide_infos[0]

    @staticmethod
    def split_line_by_index_and_ratio(line: list, split_info: tuple, mode: int = 1):
        assert len(line) >= 2
        divided_points = PointsDivider.divide_line_by_indexes_and_ratios(line, [split_info])
        saved_line = divided_points[1] if mode == 0 else divided_points[0]
        return saved_line
...
class LinkPointsSplitter:
    ...
    def split_link(points: list, lanes_points: list, given_distance: float, mode: int) -> (list, list):
        # 分割的路段点位，分割信息
        splitted_points, splitted_info = PointsSplitter.split_line_by_distance_and_mode(points, given_distance, mode)
        # 分隔的车道点位
        splitted_lanes_points = [
            {
                "left": PointsSplitter.split_line_by_index_and_ratio(lane_points["left"], splitted_info, mode),
                "center": PointsSplitter.split_line_by_index_and_ratio(lane_points["center"], splitted_info, mode),
                "right": PointsSplitter.split_line_by_index_and_ratio(lane_points["right"], splitted_info, mode),
            }
