...
class LanesPointsCalculator:
        ...
        for idx, lane in enumerate(all_lanes):
            ...
            for other_idx, other_lane in enumerate(all_lanes):
                ...
                if idx != other_idx:
                    ...
                    # 判断起点和其他车道的起点是否满足条件
                    distance = LineBase.calculate_two_points_distance(start_point, other_start)
                    if distance <= threshold_distance_max:
                        found_connection_start = True

                    # 判断终点和其他车道的终点是否满足条件
                    distance = LineBase.calculate_two_points_distance(end_point, other_end)
                    if distance <= threshold_distance_max:
    ...
    def calculate_distance_proportions(one_lane):
        ...
        for i in range(1, len(one_lane)):
            distance = LineBase.calculate_two_points_distance(one_lane[i], one_lane[i - 1])
    ...
    def calculate_interpolated_points(all_lanes, merged_proportions):
        ...
        for lane in all_lanes:
            ...
            for proportion in merged_proportions:
                ...
                interpolated_point = LineBase.interpolate_point_on_segment(lane[i], lane[i + 1], alpha)
    ...
    @staticmethod
    def calc_lanes_with_same_points(all_lanes, threshold_length=3, threshold_distance_max=5, threshold_distance_min=1, force=False):
        # 检查最长的车道与最短的车道的长度差是否在一定范围内
        mean_length = LanesPointsCalculator.check_lane_length(all_lanes, threshold_length)
        if not mean_length:
        ...
        if not LanesPointsCalculator.check_lane_points(all_lanes, threshold_distance_max, threshold_distance_min):
        ...
        # 计算合并后的距离比例列表
        merged_proportions = LanesPointsCalculator.calculate_merged_proportions(all_lanes)

        # 计算插值后的坐标点
        interpolated_points = LanesPointsCalculator.calculate_interpolated_points(all_lanes, merged_proportions)
