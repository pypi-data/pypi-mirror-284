    ...
    def save_avi(self, seconds =  1, name = "" ,  basecamera_string= None ,
        ...
        # ..0.01 means EVERY FRAME .. seconds is in principle different from 'timelapse' value
            ...
            if not self.aviopened_laps:
                ...
                filename = os.path.expanduser("~/DATA/"+ f"{me}_laps{fseconds:04d}_{filename}{str(config.CONFIG['netport'])}_{mycodec}.mkv" )
                for i in range(1):
                ...
                #
                self.skwriter_laps = skvideo.io.FFmpegWriter( f"{filename}.mp4",  outputdict={
                    '-vcodec': 'libx264',  #use the h.264 codec
                    '-crf': '0',           #set the constant rate factor to 0, which is lossless
                    '-tune':'stillimage', #
#                    '-movflags': '',
