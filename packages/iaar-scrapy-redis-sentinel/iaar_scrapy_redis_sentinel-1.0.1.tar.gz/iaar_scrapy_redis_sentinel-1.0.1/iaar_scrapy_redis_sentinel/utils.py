...
from iaar_scrapy_redis_sentinel import defaults, inner_ip, iaar_log
...
def send_message2mq(queue_name, queue_data, priority=0, delay_seconds=""):
    ...
    try:
        ...
        iaar_log.error(f"send message to mq error, queue_name: {queue_name}, inner ip: {inner_ip}, : {traceback.format_exc()}").tracker()
...
def pop_mq(queue_name):
    try:
        ...
        iaar_log.error(f"pop mq error, queue_name: {queue_name}, inner_ip: {inner_ip}, : {traceback.format_exc()}").tracker()
...
def create_queue(queue_name):
    try:
        ...
        iaar_log.error(f"create mq error, queue_name: {queue_name}, inner ip: {inner_ip},: {traceback.format_exc()}").tracker()
