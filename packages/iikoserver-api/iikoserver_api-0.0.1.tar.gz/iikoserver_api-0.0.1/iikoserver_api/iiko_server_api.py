...
class IikoServerApi:
    ...
    def export_outgoing_invoice(self, from_datetime: datetime, to_datetime: datetime) -> Optional[OutgoingInvoice]:
        params = {
            'key': self.token,
        ...
        document = xml_dict_data['outgoingInvoiceDtoes']
        if document:
        ...
        return OutgoingInvoice(**dict_data)

    def export_incoming_invoice(self, from_datetime: datetime, to_datetime: datetime) -> Optional[IncomingInvoice]:
        params = {
            'key': self.token,
            'from': from_datetime.strftime('%Y-%m-%d'),
            'to': to_datetime.strftime('%Y-%m-%d'),
        ...
        xml_dict_data = xmltodict.parse(data.decode())
        document = xml_dict_data['incomingInvoiceDtoes']
        if document:
            document = document['document']
        else:
            return
        ...
        return IncomingInvoice(**dict_data)
...
if __name__ == '__main__':
    api = IikoServerApi('https://corex-demo.iiko.it/resto/api/', 'rsinfo', 'dVs4PBaoqu')
    with api:
        api.export_incoming_invoice(datetime.now() - timedelta(days=5), datetime.now())
        api.export_outgoing_invoice(datetime.now() - timedelta(days=5), datetime.now())
