...
def parse_cmp_vars(reference: str, new: str, cmp_vars: str) -> list[tuple[str, str]]:
    ...
    _, ref_url = split_protocol(reference)
    _, new_url = split_protocol(new)
    ref_protocol = reference[: reference.index(str(ref_url))]
    new_protocol = new[: new.index(str(new_url))]
    ...
    return [(ref_protocol + ref, new_protocol + new) for ref, new in list_prods]
