...
from microservice_story_manager import MicroserviceManager, MicroserviceMetricsManager


class GeneticMicroserviceAssigner:
    def __init__(self, user_story_manager, microservice_manager, fitness_function: Callable, minimize: bool = True):
        self.user_story_manager = user_story_manager
        self.microservice_manager = microservice_manager
        self.fitness_function = fitness_function
        self.minimize = minimize

        from microservice_story_manager import MicroserviceMetricsManager
    ...
    def setup_genetic_algorithm(self):
        fitness_class_name = f"Fitness{'Min' if self.minimize else 'Max'}"
        individual_class_name = f"Individual{'Min' if self.minimize else 'Max'}"

        if fitness_class_name not in creator.__dict__:
            creator.create(fitness_class_name, base.Fitness, weights=(-1.0,) if self.minimize else (1.0,))
        ...
        toolbox.register("individual", tools.initIterate, getattr(creator, individual_class_name),
    ...
    def optimize_assignments(self, generations=50, population_size=100) -> Tuple[object, float, Dict, Dict[str, List[str]]]:
        from microservice_story_manager import MetricsVisualizer
...
for func, name, minimize in functions_to_compare:
    ...
    optimized_manager, fitness_value, detailed_metrics, story_assignments = assigner.optimize_assignments(generations=5,
                                                                                                          population_size=10)
