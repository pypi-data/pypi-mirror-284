...
from microservice_story_manager.user_story_manager import UserStoryManager, UserStory
...
class MicroserviceManager:
    ...
    def add_story_to_microservice(self, microservice_id: str, story_id: str) -> None:
        ...
        if story_id not in self.user_story_manager.get_all_stories():
            raise ValueError(get_error_message('story_not_in_backlog', self.project_language).format(story_id))

        for ms in self.microservices.values():
            if story_id in ms.stories:
                raise ValueError(get_error_message('story_already_assigned', self.project_language).format(story_id))
