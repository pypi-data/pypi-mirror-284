...
def _pack(offload_dir: str):
    ...
    tensor_packs.append(pack_tensors(originals, fakes, offload_dir))
    # Free memory
...
def _move(nvidia_uuid: str):
    try:
        ...
        tqdm = None
    total = sum([fake.untyped_storage().size() for fake in cuda_aliases]) # pyright: ignore [reportAttributeAccessIssue]
    progress = tqdm(total=total, unit='B', unit_scale=True) if tqdm is not None else nullcontext()
        ...
        for fake, original in cuda_aliases.items():
            if original is not None:
                ...
                if original_id not in moved:
                    ...
                    update(fake.untyped_storage().size()) # pyright: ignore [reportAttributeAccessIssue]
        for fake, original in cuda_aliases.items():
            if original is not None:
                fake.data = moved[AliasId.from_tensor(original)]
        # Disk -> CUDA
        for tensor_pack in tensor_packs:
            pack_to_cuda(tensor_pack, callback=update)
    bitsandbytes.move()

def move(nvidia_uuid: str):
    with ThreadPoolExecutor(1) as e:
        e.submit(_move, nvidia_uuid).result()
