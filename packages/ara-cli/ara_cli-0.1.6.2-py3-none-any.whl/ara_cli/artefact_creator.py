...
class ArtefactCreator:
    ...
    def update_artefact_parent(self, file_path, parent_file):
        parent_classifier = parent_file.split('.')[-1]
        if not Classifier.is_valid_classifier(parent_classifier):
            print("Parent artefact extension not recognized. Contributes section will not be updated.")
            return
        ...
        parent_path = f"{sub_directory}/{parent_file}"
        ...
        with open(file_path, 'w') as file:
            for line in lines:
                if line.startswith("Contributes to"):
                    line = f"Contributes to {parent_title} {parent_type}"
                elif line.startswith("Illustrates"):
                    line = f"Illustrates {parent_title} {parent_type}"
                file.write(line)
    ...
    def run(self, filename, classifier, template_path=None, contributes_to=None):
        ...
        if contributes_to:
            self.update_artefact_parent(file_path, contributes_to)
