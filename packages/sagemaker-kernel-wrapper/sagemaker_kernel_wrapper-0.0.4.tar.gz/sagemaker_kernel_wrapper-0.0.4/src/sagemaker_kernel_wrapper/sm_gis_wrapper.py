...
def _get_tags_string(tag_retriever, domain_id, user_profile, space_name):
...
def exec_kernel():
    ...
    try:
        ...
        runtime_environment = RuntimeEnvironment(client)
        ...
        if "SM_JOB_DEF_VERSION" not in os.environ:
            tags_str = _get_tags_string(
                ...
                app_medata.get_user_profile_name(),
        ...
        aws_glue_role_arn = runtime_environment.get_execution_role_arn(
            domain_id=app_medata.get_domain_id(), user_profile=app_medata.get_user_profile_name()
        )
