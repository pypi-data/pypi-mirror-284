...
from importer_local.importer_sync_conflict_resolution import ImporterSyncConflictResolution
from importer_local.importer_sync_conflict_resolution import UpdateStatus as ImporterUpdateStatus
...
class GoogleContactsSync(GoogleContactsPush):
    ...
    def _insert_contact_details_to_db(self, contact_dict: dict, user_external_id: int,
        ...
        try:
            ...
            contact_person_result_dict = self._insert_link_contact_persons(
                contact_dict=contact_dict) or {}
            ...
            contact_profile_info = self._insert_contact_profiles(
                contact_dict=contact_dict) or {}
            ...
            self._insert_organization_profile(
                contact_dict=contact_dict)
            ...
            GoogleContactsSync._insert_link_contact_notes_and_text_blocks(contact_dict=contact_dict)

            # insert link contact_phones
            self._insert_link_contact_phones(contact_dict=contact_dict)

            # inset link contact_user_externals
            self._insert_link_contact_user_external(
                contact_dict=contact_dict)

            # insert link contact_internet_domains
            self._insert_link_contact_domains(
                contact_dict=contact_dict)
    ...
    def _sync_organization(self, contact_dict: dict):
        update_status_and_information_list = self.organizations_local.get_update_status_and_information_list(
            last_modified_timestamp=contact_dict.get("last_modified_timestamp"),
        ...
        if not update_status_and_information_list:
            ...
            for update_status_and_information in update_status_and_information_list:
                if update_status_and_information.get("main_profile_id") is None:
                    continue
                update_status = update_status_and_information.get("update_status")
                organization_name = update_status_and_information.get("organization_name")
                ...
                job_title_str = self.select_one_value_by_column_and_value(
                    schema_name="job_title", view_table_name="job_title_ml_with_deleted_and_test_data_view",
                    select_clause_value="job_title_ml.title", column_name="job_title_ml_id",
                ...
                #     organization_id = upsert_organization_result.get("organization_id")
                #     organzation_ml_ids_list = upsert_organization_result.get("organization_ml_ids_list")
                #     update_status_and_information["organization_id"] = organization_id
                #     update_status_and_information["organization_ml_ids_list"] = organzation_ml_ids_list
                if update_status == UpdateStatus.UPDATE_DATA_SOURCE:
                    ...
                    job_title_id = update_status_and_information.get("job_title_id")
                    organization_name = update_status_and_information.get("organization_name")
                    ...
                    contact_dict["organizations_job_titles_dicts_list"].append(update_status_and_information)
            self._insert_organization(contact_dict=contact_dict)
            if is_update_data_source_found:
                self.contact_to_push_dict['organizations'] = []
                for organization_job_title_dict in contact_dict.get("organizations_job_titles_dicts_list"):
