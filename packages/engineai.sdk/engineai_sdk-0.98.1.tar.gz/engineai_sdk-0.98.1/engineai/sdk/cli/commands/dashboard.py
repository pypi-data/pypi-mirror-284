...
def _validate_version(
    api: DashboardAPI, account_slug: str, app_id: str, slug: str, version: str
) -> None:
    try:
        version_list = api.list_dashboard_versions(account_slug, app_id, slug)
...
def _validate_run(
    api: DashboardAPI, account_slug: str, app_id: str, slug: str, version: str, run: str
) -> None:
    try:
        run_list = api.list_dashboard_runs(account_slug, app_id, slug, version)
    except (APIServerError, UnauthenticatedError) as e:
        write_console(f"{e}\n", 1)
...
def _validate_slug(
    api: DashboardAPI, account_slug: str, app_id: str, slug: str
) -> None:
    try:
        dashboard_versions = api.list_dashboard_versions(account_slug, app_id, slug)
...
def _validate_account_slug(account_slug: Optional[str] = None) -> None:
    account_slug_var = get_env_var("ACCOUNT_SLUG")
    if not account_slug_var and account_slug is None:
        write_console("Please set an account slug where the app belong\n", 1)
    return account_slug or account_slug_var
...
def _validate_inputs(
    ...
    account_slug = _validate_account_slug()
    ...
    _validate_slug(api, account_slug, app_id, slug)

    if version:
        _validate_version(api, account_slug, app_id, slug, version)
    if run:
        _validate_run(api, account_slug, app_id, slug, version, run)
    return account_slug, app_id
...
def ls(app_id: str) -> None:
    ...
    api = DashboardAPI()
    try:
        dashboards = api.list_user_dashboards(app_id)
...
def activate(ctx: click.Context, version: Optional[str], run: Optional[str]) -> None:
    ...
    api = DashboardAPI()
    account_slug, app_id = _validate_inputs(api, ctx.obj["SLUG"], version, run)

    try:
        if version is None and run is None:
            version_list = api.list_dashboard_versions(
                account_slug, app_id, ctx.obj["SLUG"]
            )
            ...
            run_list = api.list_dashboard_runs(
                account_slug, app_id, ctx.obj["SLUG"], version
            )
        ...
        write_console(
            ...
            f"{PLATFORM_URL.get(api.url)}/{account_slug}/{app_id}/"
...
def list_runs(ctx: click.Context) -> None:
    ...
    try:
        run_list = api.list_dashboard_runs(
            account_slug, app_id, ctx.obj["SLUG"], ctx.obj["VERSION"]
        )
        _show_runs(run_list)
    except (APIServerError, UnauthenticatedError) as e:
        if e.args[-1] in API_USER_ERRORS:
