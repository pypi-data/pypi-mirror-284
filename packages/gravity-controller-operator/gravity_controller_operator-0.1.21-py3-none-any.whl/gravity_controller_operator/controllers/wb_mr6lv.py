...
class ARMK210ControllerDI(WBMR6LVControllerABC):
    map_keys_amount = 7
    starts_with = 0
    ...
    def get_phys_points_states(self):
        ...
        while not response:
            response = self.controller.read_discrete_inputs(
                self.starts_with, self.map_keys_amount, slave=self.slave_id).bits
            # if not response:
            #    response = {"error": "None response from controller"}
        return response


class ARMK210ControllerRelay(WBMR6LVControllerABC, RelayControllerInterface):
    map_keys_amount = 5
    starts_with = 0
