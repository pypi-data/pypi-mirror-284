...
from transitions.extensions.diagrams import NestedGraphTransition
from transitions.core import listify, EventData, Event
from pigeon import Pigeon
from pigeon.utils import setup_logging, call_with_correct_args


class Transition(NestedGraphTransition):
    def _change_state(self, event_data):
        dest = event_data.transition.dest
        if len(event_data.machine._get_state_obj(dest).states):
            event_data.machine.callbacks(
                event_data.machine._get_child_machine(dest).on_machine_enter,
                event_data,
            )
...
class BaseMachine(Machine):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.parent = None
        self._children = {}
        self._on_machine_enter = self._get_callables(on_machine_enter)
        super().__init__(
            *args,
    ...
    @property
    def on_machine_enter(self):
        return self._on_machine_enter

    @on_machine_enter.setter
    def on_machine_enter(self, val):
        self._on_machine_enter = listify(val)
...
class RootMachine(BaseMachine):
    ...
    def _get_child_machine(self, state):
        state_obj = self._get_state_obj(state)
        child_state = state + self.separator + list(state_obj.states.keys())[0]
        return self._get_machine(child_state)
