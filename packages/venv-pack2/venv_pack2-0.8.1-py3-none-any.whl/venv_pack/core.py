...
def rewrite_shebang(data, file, prefix):
    if on_win and file.source.endswith('.exe'):
        ...
        if data.endswith(b"UVUV"):
            interpreter_path_length = int.from_bytes(data[-8:-4], byteorder='little')
            interpreter_path = data[-8-interpreter_path_length:-8]
            interpreter = os.path.basename(interpreter_path)
            return (data[:-8-interpreter_path_length]+interpreter+len(interpreter).to_bytes(4, byteorder='little')+b"UVUV",
                    True,
                    file.target)
