...
from lanq.model.llm.base import BaseLLM
from lanq.model.rule.base import BaseRule
from lanq.utils import log
from lanq.model.llm.common.base_llm import BaseLLMModel
    ...
    def load_model(cls):
        ...
        for file in os.listdir(os.path.join(this_module_directory, 'rule')):
            ...
            if os.path.isfile(path) and file.endswith('.py') and not file == '__init__.py':
                try:
                    importlib.import_module('lanq.model.rule.' + file.split('.')[0])
        ...
        for file in os.listdir(os.path.join(this_module_directory, 'llm')):
            ...
            if os.path.isfile(path) and file.endswith('.py') and not file == '__init__.py':
                try:
                    importlib.import_module('lanq.model.llm.' + file.split('.')[0])
                except ModuleNotFoundError as e:
                    log.debug(e)
