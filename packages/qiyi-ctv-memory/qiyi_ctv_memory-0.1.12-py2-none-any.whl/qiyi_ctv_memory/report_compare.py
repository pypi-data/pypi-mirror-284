...
def compare(firstFile, secondFile, outputFile):
    ...
    second_total, second_groups, second_group_counts, second_groupRecords = record_summary.record_summary(
        secondFile, None)
    recordSos = {}
    for key in second_group_counts:
        first_count = first_group_counts[key]
        second_count = second_group_counts[key]
    ...
    if len(recordSos) > 0:
        for so in recordSos:
            result = "    " + so + "分配次数增长超过20%，前值：" + first_group_counts[so] + "  新值：" + second_group_counts[so] + "\n"
