...
def compare(firstFile, secondFile, outputFile):
        ...
        if key in first_groupRecords:
            for second_record in second_groupRecords[key]:
                for first_record in first_groupRecords[key]:
                    if first_record.stack == second_record.stack and second_record.size > first_record.size:
    ...
    path, inputFileName = os.path.split(argParams.secondFile)
    ...
    compare_groups = sorted(compare_groups.items(), key=lambda x: x[1], reverse=True)
    compare_groups = dict(compare_groups)
    for key in compare_groups:
        if writer is not None:
            summary_tmp = '%s\t%s\n' % (format(compare_groups[key], ',').rjust(13, ' '), key)
