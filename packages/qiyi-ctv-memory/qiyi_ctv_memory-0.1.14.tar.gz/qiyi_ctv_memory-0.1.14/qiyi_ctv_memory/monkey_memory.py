...
def trigger_native_dump(deviceIp, package_name, outputPath, outputFile, memoryLevel, shared_msg):
...
def trigger_total_memory_dump(outputFile, shared_msg, process_total_mb):
    print("trigger_total_memory_dump")
    json_list = []
    if os.path.exists(outputFile):
        with open(outputFile, 'r', encoding='utf-8') as f:
            content = f.read()
...
def parse_java(adb, hprof, name, outputPath, mappingPath, out_size, outputFile, memoryLevel,
    ...
    javaHeap.parse_json_and_find_bug(json, outputFile, out_size, shared_msg)
...
def check_memory(mappingPath, is_timed, outputPath, deviceIp, total_memory_mb, process_total_mb,
    ...
    if is_timed:
        ...
        trigger_native_dump(deviceIp, package_name, outputPath, outputFile, memoryLevel, shared_msg)
        ...
        # Check total process memory
            ...
            trigger_total_memory_dump(outputFile, shared_msg, process_total_mb)
