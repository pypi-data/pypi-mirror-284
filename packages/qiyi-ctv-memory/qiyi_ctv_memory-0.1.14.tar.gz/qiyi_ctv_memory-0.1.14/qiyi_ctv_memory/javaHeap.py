...
def parse_json_and_find_bug(input_file, output_file, out_size, shared_msg):
            ...
            for entry in data.get("Classs", []):
                ...
                if instance == "android.graphics.Bitmap" :
                    if size > 15 :
                        ...
                        msg += u"\nandroid.graphics.Bitmap total size is larger than 15MB({}MB)\n".format(str(size)) + top_bitmap_str
                elif instance == "java.lang.String" and size > 8:
                    msg += u"java.lang.String total size is larger than 8MB({}MB)\n".format(str(size))
                elif instance == "com.gala.apm2.trace.core.AppMethodBeat" and size > 8:
                    msg += u"AppMethodBeat total size is larger than 8MB({}MB)\n".format(str(size))
                elif size > 3:
                    msg += u"{} total size is larger than 3MB({}MB)".format(instance,str(size))
        new_node = {
