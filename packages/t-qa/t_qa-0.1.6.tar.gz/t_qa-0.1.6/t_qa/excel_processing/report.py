...
class _Report:
    ...
    def dump(self, run_data: RunData) -> None:
        ...
        all_cells = self._get_run_data_dump_cells(run_data=run_data, headers=headers)
        try:
            self.google_sheet._check_or_create_headers(headers=headers)
            self.google_sheet.write_to_google_sheet(all_cells=all_cells, headers=headers)
    ...
    def _get_run_data_dump_cells(self, run_data: RunData, headers: list[DumpCell]) -> list[DumpCell]:
        """Dump the run data."""
        row = 2
        all_cells = []
    ...
    def get_column_number(self, header_name: str, headers: list[DumpCell]) -> int:
        ...
        for cell in headers:
            if cell and cell.value == header_name:
