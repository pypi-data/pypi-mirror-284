...
def get_model_name(name,source):
    ...
    if source.upper() == "EXPERIMENT":
        return True, f"EXPERIMENT_{projectID}_FDC_{name.upper()}"
...
def get_sf_dataset_columns_names(model_obj):
    ...
    feature_names = model_obj.input_cols
    target_name = model_obj.label_cols
    prediction_names = model_obj.output_cols
    
    return feature_names, target_name, prediction_names
...
def validate_datasets_for_sf(model_obj,flavour, snowflake_df, x_train, y_train, 
                             x_test, y_test, y_pred, prob):
    try:
        if flavour == ModelConstants.snowflake_model_flavour:
            ...
            feature_names, target_name ,prediction_col_name = get_sf_dataset_columns_names(model_obj)
            ...
            if not set(prediction_col_name).issubset(snowflake_df.columns):
                raise ValueError(f"Prediction columns {prediction_col_name} not found in snowflake dataframe")
            
            return True, "Snowflake Model datasets are validated successfully."

        else:
            if x_train is None or not isinstance(x_train, (pd.DataFrame)):
                raise ValueError("please provide x_train DataFrame")
            validate_column_names(x_train)

            if y_train is None or not isinstance(y_train, (pd.DataFrame, pd.Series)):
                raise ValueError("please provide y_train dataset")
            validate_column_names(y_train)

            if x_test is None or not isinstance(x_test, (pd.DataFrame)):
                raise ValueError("please provide x_test DataFrame")
            validate_column_names(x_test)

            if y_test is None or not isinstance(y_test, (pd.DataFrame, pd.Series)):
                raise ValueError("please provide y_test dataset")
            validate_column_names(y_test)

            if y_pred is None or not isinstance(y_pred, (pd.DataFrame, pd.Series)):
                raise ValueError("please provide y_pred dataset")
            validate_column_names(y_pred)

            if prob is not None and not isinstance(prob, (pd.DataFrame, pd.Series)):
                raise ValueError("please provide prob")
            validate_column_names(prob)
            
            if hasattr(model_obj,"feature_names_in_") and not model_obj.feature_names_in_.tolist() == x_train.columns.to_list():
                raise Exception("Feature names in model object and x_train columns are not same")

            if not prob is None and not isinstance(prob,pd.DataFrame):
                raise ValueError("prob must be a pandas DataFrame with class probabilities for each class column")

            return True, "Sklearn Model datasets are validated successfully."
        
    except Exception as e:
        return False,f"Failed to validate datasets: {str(e)}"
