...
class snowflakesession:
    ...
    def _connection_details(self):
        try:
            if UserConfigs.connection_details in os.environ or UserConfigs.connection_details.upper() in os.environ:
                connection_info = os.environ.get(UserConfigs.connection_details, os.environ.get(UserConfigs.connection_details.upper()))
                self.connection_params = json.loads(connection_info.replace("\\", ""))

            elif not self.connection_params and  not "connection_details" in os.environ:
...
def get_session():
    try:
        if UserConfigs.connection_details in os.environ or UserConfigs.connection_details.upper() in os.environ:
            connection_info = os.environ.get(UserConfigs.connection_details, os.environ.get(UserConfigs.connection_details.upper()))
            connection_details = json.loads(connection_info.replace("\\", ""))
            session_obj = snowflakesession(connection_details)
            return session_obj.get_session()
        else:
            session_obj = snowflakesession()
            return session_obj.get_session()
    except Exception as msg:
        return "Failed to create snowflake session. {msg}"
