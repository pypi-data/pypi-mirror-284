...
def get_config():
    ...
    config.read(
        [
            ...
            os.path.join(
                f'{os.environ.get("XDG_CONFIG_HOME", os.path.expanduser("~/.config"))}',
                "colorls/colorls.toml",
            ),
            os.path.expanduser("~/.colorls.toml"),
            os.path.join(
                Path(__file__).parent.absolute(), "config/colorls.toml"
            ),
        ],
        encoding="utf8",
    )
...
def _get_entries(directory, args):
    ...
    if args.ignore_backups:
        remove_list += list(p.glob("*~"))
    entries = [c for c in contents if c not in remove_list]
