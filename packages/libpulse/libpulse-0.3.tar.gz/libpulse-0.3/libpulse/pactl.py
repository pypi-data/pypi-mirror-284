...
from libpulse.libpulse import (
    LibPulse, LibPulseError, PulseStructure, struct_ctypes,
    ...
    PA_DEVICE_TYPE_SINK, PA_SUBSCRIPTION_MASK_ALL,
...
def operation_success(result, c_context):
    ...
    if result != PA_OPERATION_DONE:
        ...
        sys.exit(f'Failure: {error}')

def print_volume(info):
    c_volume = C_Volume(info.volume.channels, info.volume.values)
    c_channel_map = C_ChannelMap(info.channel_map.channels,
...
def volume_relative_adjust(cv, c_volume):
    ...
    if sflag in (VOL_LINEAR, VOL_DECIBEL):
        c_cv = C_Volume(cv.channels, cv.values)
...
class C_Object:
    def __init__(self, struct_name):
        self.struct_ctype = struct_ctypes[struct_name]
    ...
    def __setattr__(self, name, val):
        if name in self.members:
            if self.members[name]:
                ...
                raise LibPulseError(
                    f"Cannot change attribute '{name}' as it is an array.\n"
                    f'But you can modify the items of the existing array.'
                )
        else:
            object.__setattr__(self, name, val)

    def byref(self):
        return ct.byref(self.c_obj)

    def to_pulse_sructure(self):
        return PulseStructure(self.c_obj, self.struct_ctype)

class C_Volume(C_Object):
    members = { 'channels': True,   # The attribute can be set to a new value.
                'values': False}

    def __init__(self, channels, values):
        super().__init__('pa_cvolume')
        length = len(values)
        assert length <= PA_CHANNELS_MAX
        values += [0 for i in range(PA_CHANNELS_MAX - length)]
        self.c_obj = self.struct_ctype(channels, C_UINT_ARRAY_32(*values))

class C_ChannelMap(C_Object):
    members = { 'channels': True,   # The attribute can be set to a new value.
                'map': False}

    def __init__(self, channels, map):
        super().__init__('pa_channel_map')
        length = len(map)
        assert length <= PA_CHANNELS_MAX
        map += [0 for i in range(PA_CHANNELS_MAX - length)]
        self.c_obj = self.struct_ctype(channels, C_INT_ARRAY_32(*map))
...
def parse_volumes(volumes):
    ...
    c_volume = C_Volume(length, values)
