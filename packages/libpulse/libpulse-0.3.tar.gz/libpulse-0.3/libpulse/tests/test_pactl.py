...
class PactlTests(IsolatedAsyncioTestCase):
    ...
    async def test_operation_null_pointer(self):
        try:
            ...
            self.assertTrue(str(e) ==
                    'LibPulseOperationError("NULL \'pa_operation\' pointer")')
    ...
    async def test_invalid_argument(self):
        try:
            ...
            self.assertTrue(str(e) == "Failure: b'No such entity'")
    ...
    async def subscribe_helper(self, delay):
        async def subscribe():
            eof = "Event 'remove' on client"
            async with asyncio.timeout(1):
                result = await subcommand('result', 'subscribe',
                                          eof, as_str=True)
        ...
        subprocess.run(['pactl', 'send-message', '/core', 'list-handlers'],
    ...
    async def test_subscribe(self):
        ...
        while True:
            try:
                await self.subscribe_helper(delay)
                break
            except AssertionError as e:
                if delay >= 1.0:
                    raise RuntimeError(
                                f'Failed with a {delay} seconds delay') from e
                delay += increment
