...
def run(config, ps, yr, layer, use_calib_adc):
    ...
    ps.set(v=measurement_config["v_max"], i=measurement_config["i_config"][layer])
    ...
    currents = sorted(
        np.concatenate(
            [
                np.linspace(
                    measurement_config["i_min"][layer],
                    measurement_config["i_max"][layer],
                    measurement_config["n_points"][layer],
                ),
                np.array(measurement_config["extra_i"]),
            ]
        ),
        reverse=True,
    )
    for i in currents:
        ...
        ps.set(v=measurement_config["v_max"], i=i)
