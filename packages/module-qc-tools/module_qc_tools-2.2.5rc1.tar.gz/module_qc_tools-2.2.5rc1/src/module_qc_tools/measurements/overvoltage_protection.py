...
def run(config, ps, yr, layer, use_calib_adc):
    ...
    if yr.running_emulator():
        ps.on(v=measurement_config["v_max"], i=measurement_config["i_config"][layer])
    else:
        ps.set(
            v=measurement_config["v_max"],
            i=measurement_config["i_config"][layer],
            check=False,
        )
        ps.on()
        ps.checkTarget(
    ...
    ps.set(v=measurement_config["v_max"], i=measurement_config["i_ovp"][layer])
    ...
    for chip in yr._enabled_chip_positions:
        i_mea[chip]["SetCurrent"] = [measurement_config["i_ovp"][layer]]
