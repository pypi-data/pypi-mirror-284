...
def run(config, ps, yr, layer, use_calib_adc):
    ...
    """
    nominal_config = copy.deepcopy(config["tasks"]["GENERAL"])
    ...
    data = [
        qcDataFrame(
            ...
            + [f"Vmux{v_mux}" for v_mux in measurement_config["v_mux"]]
            + [f"Imux{i_mux}" for i_mux in measurement_config["i_mux"]],
            units=["C", "A", "A"]
            + ["V" for v_mux in measurement_config["v_mux"]]
            + ["V" for i_mux in measurement_config["i_mux"]],
        )
    ...
    i_usp = measurement_config["i_usp"][layer]
    i_config = nominal_config["i_config"][layer]
    if yr.running_emulator():
        ps.on(v=measurement_config["v_max"], i=i_usp)
    else:
        ps.set(v=measurement_config["v_max"], i=i_usp, check=False)
        ps.on()
        ps.checkTarget(v=measurement_config["v_max"], i=i_usp)
        ...
        for chip in yr._enabled_chip_positions:
            ...
            for i_mux in measurement_config["i_mux"]:
                mea_chips = read_vmux(
                    meter,
                    yr,
                    measurement_config,  # Or should it be nominal?
                    i_mux=i_mux,
                    chip_position=chip,
                    use_adc=use_calib_adc,
                )
    ...
    if yr.running_emulator():
        ...
        ps.set(v=nominal_config["v_max"], i=i_config, check=False)
        ps.on()
        ps.checkTarget(v=nominal_config["v_max"], i=i_config)
