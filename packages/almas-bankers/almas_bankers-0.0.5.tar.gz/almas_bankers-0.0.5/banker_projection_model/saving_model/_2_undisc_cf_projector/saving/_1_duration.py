...
def duration_calculation(merged_df, persistency_rate=1):
    ...
    merged_df["proj_y"] = proj_y_value(merged_df)
...
def proj_y_value(merged_df):
    proj_y_values = (merged_df["proj_m"] // 12) + 1
    return proj_y_values
...
def fees_amount(merged_df):
    ...
    return (
        sa_death * merged_df["inforce_ind"] * ((sa_loading_pm / 1000) / 12)
        + merged_df["admin_fee_amt"] * merged_df["inforce_ind"]
    )
