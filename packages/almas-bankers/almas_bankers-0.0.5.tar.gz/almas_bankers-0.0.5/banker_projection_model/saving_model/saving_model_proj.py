...
def saving_model_process(
    ...
    if chunk_size <= 0:
        ...
        output["inception_year"] = (output["inception_date"].dt.year).astype(
            str
        )
        output["inception_month"] = (output["inception_date"].dt.month).astype(
            str
        )
        ...
        for chunk_start in range(0, len(model_points), chunk_size):
            ...
            output["inception_year"] = (
                output["inception_date"].dt.year
            ).astype(str)
            output["inception_month"] = (
                output["inception_date"].dt.month
            ).astype(str)
            result_df, not_found_columns = saving_module(output)
