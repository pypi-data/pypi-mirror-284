...
namespace CommandIDs {
   ...
   */
  export const launchPanel = 'jcl:launch-dashboard';

  /**
   * Launch a jcl custom layout.
   */
  export const launchLayout = 'jcl:launch-layout';

  /**
   * Attempt to find an active jcl cluster.
   */
  export const populateDashboardUrl = 'jcl:populate-dashboard-url';

  /**
   * Attempt to find an active jcl cluster.
   */
  export const populateAndLaunchLayout = 'jcl:populate-and-launch-layout';

  /**
   * Inject client code into the active editor.
   */
  export const injectClientCode = 'jcl:inject-client-code';

  /**
   * Launch a new cluster.
   */
  export const launchCluster = 'jcl:launch-cluster';

  /**
   * Shutdown a cluster.
   */
  export const stopCluster = 'jcl:stop-cluster';

  /**
   * Scale a cluster.
   */
  export const scaleCluster = 'jcl:scale-cluster';

  /**
   * Toggle the auto-starting of clients.
   */
  export const toggleAutoStartClient = 'jcl:toggle-auto-start-client';
}

const PLUGIN_ID = 'jcl-labextension:plugin';

/**
 * The jcl dashboard extension.
 */
...
async function activate(
  ...
  const id = 'jcldata-dashboard-launcher';

  // Attempt to find a link to the jcl dashboard
  ...
  void Promise.all([settingRegistry.load(PLUGIN_ID), state.fetch(id)]).then(
    async res => {
      ...
      if (!settings) {
        console.warn('Unable to retrieve jcl-labextension settings');
        return;
      }
  ...
  app.commands.addCommand(CommandIDs.launchPanel, {
    ...
    caption: 'Launch a jcl dashboard',
    execute: args => {
      ...
      dashboard.id = `jcl-dashboard-${Private.id++}`;
  ...
  app.commands.addCommand(CommandIDs.launchLayout, {
    label: 'Launch jcl Dashboard Layout',
    caption: 'Launch a pre-configured jcl Dashboard Layout',
    ...
    execute: async () => {
      ...
      for (let k of order) {
        ...
        if (!dashboard) {
          console.warn(`Non-existent dashboard found in jcl layout spec ${k}`);
          continue;
        }
  ...
  app.commands.addCommand(CommandIDs.populateDashboardUrl, {
    label: 'Populate jcl Dashboard URL',
    caption: 'Attempt to populate the URL for an active jcl cluster',
    execute: async args => {
  ...
  app.commands.addCommand(CommandIDs.populateAndLaunchLayout, {
    label: 'Populate jcl Dashboard URL and launch the default layout',
    caption:
      'Attempt to populate the URL for an active jcl cluster and then launch the default layout',
    execute: async args => {
  ...
  app.commands.addCommand(CommandIDs.injectClientCode, {
    label: 'Inject jcl Client Connection Code',
    execute: async () => {
  ...
  app.contextMenu.addItem({
    ...
    selector: '.jcl-ClusterListingItem',
    rank: 10
  });
  app.contextMenu.addItem({
  ...
  app.contextMenu.addItem({
    ...
    selector: '.jcl-ClusterListing-list',
    rank: 1
  });
}
