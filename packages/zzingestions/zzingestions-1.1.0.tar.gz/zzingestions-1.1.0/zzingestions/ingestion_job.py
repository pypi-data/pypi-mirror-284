...
class IngestionJob:
    ...
    def __set_class_docstring(self):
        readme_file = os.path.join(os.path.dirname(os.getcwd()), "README.md")
    ...
    def run(self, kwargs:dict={}) -> None:
        ...
        try:
            if self.__fl_parallel:
                self.__parallel_completed = self.process_parallel_read()
    ...
    def __set_work_parameters(self) -> None:
        ...
        self.__fl_optimize_target_table = self.__get(self.parameters, "optimize_target_table", True)
        self.__fl_vacuum_target_table = self.__get(self.parameters, "vacuum_target_table", True)
    ...
    def get_secret(self, secret_name:str):
        ...
        if hasattr(self, "__secrets") and self.__secrets.get(secret_name):
    ...
    def process_parallel_read(self) -> dict:
        """
    ...
    def __prepare_create_statement(self, create_mode:str, with_data:bool, partition_by:list, path:str, target_table:str):
        ...
        self.debug_print(f"def PREPARE_CREATE_STATEMENT >> source_table = {source_table}")
        ...
        return f"{create_modes[create_mode]} {target_table} {partition_data} location '{path}' as select * from {source_table} {filter_data}"
    ...
    def __prepare_delete_statement(self, delete_keys:list, target_table:str):
        ...
        self.debug_print(f"def PREPARE_DELETE_STATEMENT >> source_table = {source_table}")
        self.debug_print(f"def PREPARE_DELETE_STATEMENT >> on_clause = {on_clause}")
        
        return f"delete from {target_table} as target where exists (select 1 from {source_table} as source where {on_clause})"
    ...
    def __prepare_insert_statement(self, insert_mode:str, partition_by:list, target_table:str):
        ...
        self.debug_print(f"def PREPARE_INSERT_STATEMENT >> source_table = {source_table}")
        self.debug_print(f"def PREPARE_INSERT_STATEMENT >> partition_data = {partition_data}")
        
        return f"insert {insert_mode} {target_table} {partition_data} select * from {source_table}"
    ...
    def __prepare_merge_statement(self, merge_keys:list, merge_filter:list, target_table:str):
        ...
        self.debug_print(f"def PREPARE_MERGE_STATEMENT >> source_table = {source_table}")
        self.debug_print(f"def PREPARE_MERGE_STATEMENT >> on_clause = {on_clause}")
        
        return f"merge into {target_table} as target using {source_table} as source on {on_clause} when matched then update set * when not matched then insert *"
    ...
    def __write_delta_column_comment(self, df_cols:list, print_level:int) -> None:
        ...
        if comments != False:
            ...
            new_comments = {k:v for k,v in valid_comments.items() if v != current_comments[k]}
    ...
    def __write_delta(self, df:DataFrame, print_level:int) -> None:
        ...
        self.pprint("Creating temp table as source", lvl=print_level)
        temp_source_table = self.__write_options["name"] + "_temp"
        df.write.saveAsTable(temp_source_table, format="delta", mode="overwrite", overwriteSchema="true")
        self.pprint(f"Temp table created: {temp_source_table}", lvl=print_level+1, dots=0)
        ...
        self.pprint("Removing temp table", lvl=print_level)
        self.drop_table(temp_source_table)
        self.pprint(f"Table {temp_source_table} successfully dropped", lvl=print_level+1, dots=1)
