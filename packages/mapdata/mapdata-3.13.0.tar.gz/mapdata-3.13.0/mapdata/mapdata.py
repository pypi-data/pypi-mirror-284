...
class MapUI(object):
	...
	def coord_candkeys(self):
		# Returns a (possibly empty) list of column names that are candidate keys for coordinate pairs.
		coordkey_ids = []
		curs = data_db.cursor()
...
class FindCandKeysDialog(object):
	def __init__(self, parent, table_rows, column_specs):
		...
		self.dups_btn = ttk.Button(btn_frame, text="Duplicates", state=tk.DISABLED, command=self.show_dups)
		self.dups_btn.grid(row=0, column=2, sticky=tk.W, padx=(3,3))
		self.seldups_btn = ttk.Button(btn_frame, text="Select", state=tk.DISABLED, command=self.select_dups)
		self.seldups_btn.grid(row=0, column=3, sticky=tk.W, padx=(3,3))
		self.dlg.bind("<Alt-h>", self.do_help)
	...
	def get_data(self):
		...
		if len(column_list) > 0:
			...
			if self.sel_only_var.get() == "1":
				...
				rowids = self.parent.get_sel_rowids()
			else:
				dataset = self.parent.get_all_data(column_list)
				rowids = self.parent.get_all_rowids()
	...
	def check_cols(self):
		...
		sqlcmd = f"select {colstr}, count(*) as duplicates from mapdata group by {colstr} having count(*) > 1;"
		cur = data_db.cursor()
...
class AddRowIDDialog(object):
	...
	def ck_colname(self, varname, ix, mode):
		...
		if cname.strip() == '' or cname in self.curr_colnames or db_colnames([cname])[0] in self.db_names:
			self.ok_btn["state"] = tk.DISABLED
		else:
			self.ok_btn["state"] =  tk.NORMAL
	def do_select(self, *args):
...
class PlotDialog(object):
	def __init__(self, parent, column_specs):
		...
		date_columns = sorted([c[0] for c in self.column_specs if c[1] in ("date", "timestamp", "timestamptz")])
		self.dataset = None
		self.data_labels = None
		...
		self.type_sel = ttk.Combobox(ctrl_frame, state="readonly", textvariable=self.type_var, width=20, height=15,
				values=["Box plot", "Breaks groups", "Breaks optimum", "Bubble plot", "Count by category", "CV by category", "Empirical CDF", "Histogram", "Kernel density (KD) plot", "Line plot", "Mean by category", "Min-max by bin", "Min-max by category", "Normal Q-Q plot", "Scatter plot", "Stripchart", "Total by category", "Violin plot", "Y range plot"])
		...
		if self.plot_data is not None and len(self.plot_data) > 0 and len(self.plot_data[0]) > 0:
			if plot_type == "Count by category":
				...
				if len(values) > 0:
					...
					if not self.rotated:
						self.plot_axes.bar(grp_lbls, values)
						self.plot_axes.set_xlabel(self.xlabel or self.groupby_var.get())
						self.plot_axes.set_ylabel(self.ylabel or lbl + loglbl + self.x_var.get())
					else:
						self.plot_axes.barh(grp_lbls, means)
						self.plot_axes.set_xlabel(self.ylabel or lbl + loglbl + self.x_var.get())
...
class CorrMatrixDialog(object):
	def __init__(self, parent, column_specs):
		...
		self.canceled = False
		self.help_btn = ttk.Button(btn_frame, text="Help", command=self.do_help, underline=0)
		self.help_btn.grid(row=0, column=0, sticky=tk.W, padx=(6,6))
		self.data_btn = ttk.Button(btn_frame, text="Source Data", state=tk.DISABLED, command=self.show_data, underline=0)
		self.data_btn.grid(row=0, column=1, sticky=tk.W, padx=(3,3))
		self.corr_data_btn = ttk.Button(btn_frame, text="Correlations", state=tk.DISABLED, command=self.show_corr_data, underline=0)
		self.corr_data_btn.grid(row=0, column=2, sticky=tk.W, padx=(3,3))
		self.dlg.bind("<Alt-h>", self.do_help)
		close_btn = ttk.Button(btn_frame, text="Close", command=self.do_close, underline=0)
...
class CosineSimilarityDialog(object):
	def __init__(self, parent, column_specs):
		...
		self.sim_data_btn = ttk.Button(btn_frame, text="Similarities", state=tk.DISABLED, command=self.show_sim)
		self.sim_data_btn.grid(row=0, column=3, sticky=tk.W, padx=(3,3))
		close_btn = ttk.Button(btn_frame, text="Close", command=self.do_close, underline=0)
		close_btn.grid(row=0, column=4, sticky=tk.E, padx=(6,6))
		self.dlg.bind("<Alt-c>", self.do_close)
		self.dlg.bind("<Escape>", self.do_close)
	...
	def redraw(self):
		...
		if self.agg_data is not None and len(self.agg_data) > 1 and len(self.agg_data[0]) > 1:
			...
			self.cos_table = [[], [], []]
			groups = sorted(list(set(self.dataset[self.n_dataset_columns])))
...
class NMFUnmixingDialog(object):
	def __init__(self, parent, column_specs, id_columns):
		...
		self.data_btn = ttk.Button(btn_frame, text="Source Data", state=tk.DISABLED, command=self.show_data, underline=0)
		self.data_btn.grid(row=0, column=1, sticky=tk.W, padx=(3,3))
		self.addcols_btn = ttk.Button(btn_frame, text="Add Columns", state=tk.DISABLED, command=self.add_columns, underline=0)
		self.addcols_btn.grid(row=0, column=2, sticky=tk.W, padx=(3,3))
		self.dlg.bind("<Alt-h>", self.do_help)
		close_btn = ttk.Button(btn_frame, text="Close", command=self.do_close, underline=0)
		close_btn.grid(row=0, column=3, sticky=tk.E, padx=(6,6))
		self.dlg.bind("<Alt-c>", self.do_close)
	...
	def recalc(self):
		...
		if self.dataset is not None and len(self.dataset[0]) > 2:
			...
			n = len(self.dataset[0]) * self.n_dataset_columns
			if self.em_var.get() == 0:
				test_ems = list(range(1, min(12, self.n_dataset_columns, len(self.dataset[0]))))
				aics = []
				unmixes = []
				diagnostics = []
				for em_no in test_ems:
					...
					k = em_no * (self.n_dataset_columns + len(self.dataset[0]) - 1) - 1
					fit_aic = -2 * math.log(fit_rss/n) + 2 * k + ((2 * k + (k + 1)) / (n - k - 1))
					aics.append(fit_aic)
					unmixes.append((W, H))
					# Diagnostics: Total residual, RSS, RMSE, AICc, Frobenius norm of matrix difference
					diagnostics.append([em_no, fp_display(arraysum(fit_ds)-arraysum(ds), 4), fp_display(fit_rss,5),
						fp_display(math.sqrt(mean_squared_error(ds, fit_ds)),4),
						fp_display(frobnorm,4), fp_display(fit_aic,4)])
				if len(aics) > 1:		# If more than 1 EM was tested, choose the one with the lowest AICc
					um_aics = aics[1:]
					min_aic_index = um_aics.index(min(um_aics)) + 1
					self.em_var.set(test_ems[min_aic_index])
				else:
					self.em_var.set(1)
					min_aic_index = 0
				self.em_sel["values"] = test_ems
				self.em_sel["state"] = tk.NORMAL
				W, H = unmixes[min_aic_index]
	...
	def add_columns(self, *args):
		...
		if ok:
			...
			em_names = em_hdrs[1:-1]
			em_table = rows_to_columns(em_tbl)[:-1]
			...
			data_rowids = [item[0] for item in data_rowids]
			for col_idx in range(len(em_names)):
				col_name = col_prefix + "_" + em_names[col_idx]
				db_col_name = db_colnames(col_name)
				# Update the database
				...
				for i in range(len(key_col_vals)):
					cur.execute(f"update mapdata set {db_col_name} = '{em_table[col_idx][i]}' where {key_col_id} = '{key_col_vals[i]}';")
				# Update the Treeview
				if new_column:
	...
	def tab_changed(self, *args):
		...
		if self.tab_no == 2:
			self.addcols_btn["state"] = tk.NORMAL
		else:
			self.addcols_btn["state"] = tk.DISABLED
	def save_data(self, *args):
...
class CategSimDialog(object):
	def __init__(self, parent, column_specs, id_columns):
		...
		prompt_frame.grid(row=0, column=0, sticky=tk.N+tk.EW, pady=(3,3))
		prompt_frame.columnconfigure(0, weight=1)
		msg_lbl = ttk.Label(prompt_frame, width=65, text="Select two or more variables from the left to see the similarity matrix.  Use Ctrl-click or Shift-click to select multiple rows.")
		msg_lbl.grid(row=0, column=0, sticky=tk.W, padx=(6,6), pady=(3,3))
		def wrap_msg(event):
			msg_lbl.configure(wraplength=event.width - 5)
		msg_lbl.bind("<Configure>", wrap_msg)
		...
		self.data_btn = ttk.Button(btn_frame, text="Source Data", state=tk.DISABLED, command=self.show_data, underline=0)
		self.data_btn.grid(row=0, column=1, sticky=tk.W, padx=(3,3))
		self.sim_data_btn = ttk.Button(btn_frame, text="Similarities", state=tk.DISABLED, command=self.show_sim_data, underline=0)
		self.sim_data_btn.grid(row=0, column=2, sticky=tk.W, padx=(3,3))
		self.dlg.bind("<Alt-h>", self.do_help)
		close_btn = ttk.Button(btn_frame, text="Close", command=self.do_close, underline=0)
		close_btn.grid(row=0, column=3, sticky=tk.E, padx=(6,6))
		self.dlg.bind("<Alt-c>", self.do_close)
	...
	def redraw(self):
		...
		if self.data_labels is not None and len(self.data_labels) > 1:
			self.loading_dlg.display("Calculating categorical similarities")
			self.dlg.bind("<Alt-l>", self.set_labeling)
			...
			caxes = self.plot_axes.matshow(simmat, cmap="BuGn", vmin=0.0, vmax=1.0)
			self.plotfig.colorbar(caxes)
			self.plot_axes.set_xticks(range(ncases), labels=self.data_ids, rotation=25)
			self.plot_axes.set_yticks(range(ncases), labels=self.data_ids)
			if self.show_labels:
				for i in range(ncases):
					for j in range(ncases):
						v = simmat[i,j]
						c = "white" if abs(v) > 0.40 else "black"
						self.plot_axes.text(j, i, f"{simmat[i,j]:.2f}", ha="center", va="center", color=c)
			self.plot_axes.set_title(simtype)
			self.plotfig_canvas.draw()
