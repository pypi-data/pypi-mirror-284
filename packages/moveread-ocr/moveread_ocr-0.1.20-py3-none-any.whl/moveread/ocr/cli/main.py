...
def predict(
  ...
  labels: bool = typer.Option(False, '-l', '--labels', help='Whether to include labels in the output'),
):
  ...
  cli.predict(
    ...
    batch_size=batch_size, logstream=stderr() if verbose else None, exp=exp, labels=labels
  )
