...
try:
    ...
    # classpath is (probably) wrong, add latest jar file to path
    ...
    if II_SYSTEM is not None:
        tmp_ingres_jdbc_jar_path = os.path.join(II_SYSTEM, 'ingres', 'lib', 'iijdbc.jar')
        """
        if tmp_ingres_jdbc_jar_path not in sys.path:
            print 'appends...'
            sys.path.append(tmp_ingres_jdbc_jar_path)
            #This doesn't seem to allow zxJDBC  to see the jdbc driver :-(
        print 'sys.path', sys.path
        """
    else:
        # assume/hope jdbc driver is in the current directory
        tmp_ingres_jdbc_jar_path = os.path.join('iijdbc.jar')
    
    # try and add Ingres JDBC jar file to CLASSPATH automatically
    jarLoad = classPathHacker()
    a = jarLoad.addFile(tmp_ingres_jdbc_jar_path)
