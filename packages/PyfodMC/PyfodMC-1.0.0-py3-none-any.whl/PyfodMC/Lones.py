...
class Lones(HelperFunctions):
    ...
    def assign_lone(self):
        ...
        for s,sym in enumerate(self.atomic_sym):
            ...
            if s not in self.lone.keys():
                ...
                if len(self.bond_partners[s]) == 0: #sum(self.con_mat[0,s,:]) + sum(self.con_mat[1,s,:]) == 0:
                    self.lone[s] = f"{len(self.fods[s][0][-1])}-{len(self.fods[s][1][-1])}"
                else:
                    ...
                    if (sum(self.con_mat[0,s,:]) + sum(self.con_mat[1,s,:]))/2. + self.charge[s] + core_charge < atomic_numbers[sym]:
                        up_lone = int(numpy.ceil((atomic_numbers[sym] - core_charge - self.charge[s] - (sum(self.con_mat[0,s,:]) + sum(self.con_mat[1,s,:]))/2.)/2.))
                        dn_lone = int(numpy.floor((atomic_numbers[sym] - core_charge - self.charge[s] - (sum(self.con_mat[0,s,:]) + sum(self.con_mat[1,s,:]))/2.)/2.))
                        self.lone[s] = f"{up_lone}-{dn_lone}"
    

    # CLEAN THIS UP!! THIS IS TERRIBLE (but should work)
    def make_multiplelones(self,
                           lone_no=1,
                           idx_atom=0,
                           lone_center=[]):
        '''
        ...
        bonds = [self.dist_vecs[idx_atom][partner] for partner in self.bond_partners[idx_atom]]

        # Define Motif
        motif = self.get_motif(no_points=lone_no,r=self.r_bond*self.scale_r,core_or_bond='bond')
        ...
        for keys,values in self.bond_orientation.items():
            ...
            if idx_atom in [int(keys.split("-")[0]),int(keys.split("-")[1])] and self.is_linear_planar[idx_atom] not in ["planar4both"]:
                org_vec = values
                ...
                if lone_no > 2:
                     ...
                     if numpy.dot(perp_motif,lone_center)/(numpy.linalg.norm(perp_motif)*numpy.linalg.norm(lone_center)) == -1.0:
                ...
                if lone_no == 4:
                    ...
                    linear_rot = self.rotmat_around_axis(axis=lone_center,angle=numpy.pi/3)
                else:
                    linear_rot = self.rotmat_around_axis(axis=lone_center,angle=numpy.pi/lone_no)
        ...
        if self.is_linear_planar[idx_atom] in ['linear4both'] or (self.is_linear_planar[idx_atom] in ['planar4both'] and lone_no == 2):
            ...
            # For more than 2; rotate into bond
                ...
                perp_vec = numpy.cross(rot_pos[0],rot_pos[1])/numpy.linalg.norm(numpy.cross(rot_pos[0],rot_pos[1]))
        ...
        # if lone_up == 2 -> do the same as for linear; otherwise, put lones at lone_center .... see above
            ...
            perp_vec = numpy.cross(bonds[0],bonds[1])/numpy.linalg.norm(numpy.cross(bonds[0],bonds[1]))
        ...
        # ADD planar4bonds!!!!!
            ...
            if len(self.bond_partners[idx_atom]) == 1:
                ...
                outofaxis = numpy.cross(bonds[0],[0,0,1])#/numpy.linalg.norm(numpy.cross(vec_bond,[0,0,1]))
                if outofaxis.tolist() == [0,0,0]:
                    outofaxis = numpy.cross(bonds[0],[1,0,0])/numpy.linalg.norm(numpy.cross(bonds[0],[1,0,0]))
                else:
                ...
                perp_vec = numpy.cross(bonds[0],lone_center)/numpy.linalg.norm(numpy.cross(bonds[0],lone_center))
        ...
        # Do linear4bonds: Dimers! Invert motif in case it is alinged with bond vector!
            ...
            if lone_no == 2:
                linear_rot = self.rotmat_around_axis(axis=bonds[0],angle=numpy.pi/2.)
            ...
            # Else
                ...
                if numpy.dot(perp_vec,lone_center)/(numpy.linalg.norm(perp_vec)*numpy.linalg.norm(lone_center)) == -1.0:
                    rot_pos2 = rot_pos.copy()
                else:
                    rotmat = self.rotmat_vec1vec2(perp_vec,lone_center)
                    rot_pos2 = numpy.array([numpy.matmul(rotmat,po) for po in rot_pos])
        ...
        # NEED TO CHECK WHETHER BOND PARTNERS ARE LINEAR/PLANAR
        elif numpy.any([self.is_linear_planar[s2] in ["planar4bonds","planar4both"] for s2 in self.bond_partners[idx_atom]]):
            ...
            planar = [s2 for s2 in self.bond_partners[idx_atom] if self.is_linear_planar[s2] in ["planar4bonds","planar4both"]][0]
            bonds_planar = [self.dist_vecs[planar][partner]/numpy.linalg.norm(self.dist_vecs[planar][partner]) for partner in self.bond_partners[planar]]
            outofplane = numpy.cross(bonds_planar[0],bonds_planar[1])/numpy.linalg.norm(numpy.cross(bonds_planar[0],bonds_planar[1]))
            # Go through bond vectors etc etc bla. Invert logic for bonds -> get in-plane direction
            inplane = numpy.cross(outofplane,lone_center)/numpy.linalg.norm(numpy.cross(outofplane,lone_center))
        ...
        elif numpy.any([self.is_linear_planar[s2] in ["linear4bonds","linear4both"] for s2 in self.bond_partners[idx_atom]]):
            # There should be exactly one bond partner here, and this one has exactly 2 bond partners
            linear = [s2 for s2 in self.bond_partners[idx_atom] if self.is_linear_planar[s2] in ["linear4bonds","linear4both"]][0]
    ...
    def create_lone(self):
        ...
        for s,sym in enumerate(self.atomic_sym):
            ...
            # else:
            # for all lone FODs
            if s in self.lone.keys() and self.bond_partners[s] != [] and self.lone[s] != "0-0":
                ...
                bonds = [self.dist_vecs[s][partner] for partner in self.bond_partners[s]]
                # Define lone center
                if self.is_linear_planar[s] in ['planar4both']:
                    # Get out-of-plane direction via cross product. rotate tmp to align with that
                    lone_center = numpy.cross(bonds[0],bonds[1])/numpy.linalg.norm(numpy.cross(bonds[0],bonds[1]))
                    lone_center *= self.r_bond
                elif self.is_linear_planar[s] in ['linear4both']:
                    lone_center = numpy.cross(bonds[0],[0,0,1])#/numpy.linalg.norm(numpy.cross(vec_bond,[0,0,1]))
                    if lone_center.tolist() == [0,0,0]:
                        lone_center = numpy.cross(bonds[0],[1,0,0])/numpy.linalg.norm(numpy.cross(bonds[0],[1,0,0]))
                        lone_center *= self.r_bond
                    else:
                    ...
                    lone_center = 1./(no_lone/2.)*vecc*self.r_bond
                ...
                # ALSO, check whether any neighbor is planar -> if so, have to rotate FODs accordinly in-plane!
                    ...
                    new_FOD_up = self.make_multiplelones(lone_no=lone_up,
                                                         idx_atom=s,
                                                         lone_center=lone_center)
                ...
                if lone_dn == lone_up:
                    ...
                    # define motif
                        ...
                        new_FOD_dn = self.make_multiplelones(lone_no=lone_dn,
                                                             idx_atom=s,
                                                             lone_center=lone_center)
