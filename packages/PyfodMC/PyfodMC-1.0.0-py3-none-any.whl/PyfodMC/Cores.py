    ...
    def create_core(self):
        '''
           Create the core FOD positions via rotations
            ...
            Rotate to minimize 1/r to all other bond/lone FODs
        '''
        cores = []

        for s,sym in enumerate(self.atomic_sym):
            # If there is a core, and it is more than 1 (up or dn channel)
            if (len(self.fods[s][0]) > 1 or len(self.fods[s][1]) > 1) and self.bond_partners[s] != []:
                ...
                if len(tetra) != 0:
                    if self.is_linear_planar[s] in ["planar4both","planar4bonds"]:
                        ...
                        bonds = [self.dist_vecs[s][partner]/numpy.linalg.norm(self.dist_vecs[s][partner]) for partner in self.bond_partners[s]]
                        ...
                        # Rotate
                        rotpos = numpy.array([numpy.matmul(rotmat,po) for po in fods_to_rot])
                        ...
                        angle = numpy.arccos(numpy.dot(vec2rot,tmp_vec)/(numpy.linalg.norm(vec2rot)*numpy.linalg.norm(tmp_vec)))
                        ...
                        _, dist_mat = get_distances(tetra+self.atomic_pos[s],self.atomic_pos[self.bond_partners[s][0]],cell=self.cell,pbc=self.pbc_xyz)
                        ...
                        fods_to_rot = numpy.array([numpy.matmul(rotmat,po)*factor for po in rotpos]) + self.atomic_pos[s]
                        ...
                        #  think about it
                        # We should always have nmore than 2 bond partners here!
                        ...
                        # 1. plane normal
                        plane_normal = numpy.cross(tetra[1],tetra[2])/numpy.linalg.norm(numpy.cross(tetra[1],tetra[2]))
                        # 2. project bonds[1] into that plane
                        proj_bond = bonds[1] - numpy.dot(numpy.dot(bonds[1],plane_normal),plane_normal)
                        # actual angle between this and tetra[1]
                        angle = numpy.arccos(numpy.dot(tetra[1],proj_bond)/(numpy.linalg.norm(tetra[1])*numpy.linalg.norm(proj_bond)))
                        ...
                        rotpos2 = numpy.array([numpy.matmul(rotmat,po) for po in rotpos])
                        # Final cores; INVERT THE POSITIONS HERE!!!
                        fods_to_rot = numpy.array([-1.0*po for po in rotpos2]) + self.atomic_pos[s]
                        #fods_to_rot = numpy.array([numpy.matmul(rotmat,po) for po in fods_to_rot]) + self.atomic_pos[s]
                        cores.append([[fods_to_rot[:core_up]],[fods_to_rot[core_up:]]])
                        # overwrite original pos -> won;t be written
                        self.fods[s][0] = [[],[]]
                ...
                # Note: hould barely trigger
                    ...
                    # Three cycles
                    for rot in range(3):
                        for angle_x in numpy.linspace(-numpy.pi/factor,numpy.pi/factor,step+1): # +1 to take pi into account
                            for angle_y in numpy.linspace(-numpy.pi/factor,numpy.pi/factor,step+1): # +1 to take pi into account
                                for angle_z in numpy.linspace(-numpy.pi/factor,numpy.pi/factor,step+1): # +1 to take pi into account
                                    # Create rot mat for all! matmul
                                    rotmat_x = self.rotmat_around_axis(axis=[1,0,0],angle=angle_x)
                                    rotmat_y = self.rotmat_around_axis(axis=[0,1,0],angle=angle_y)
                                    rotmat_z = self.rotmat_around_axis(axis=[0,0,1],angle=angle_z)

                                    tmp = numpy.matmul(rotmat_z,rotmat_y)
                                    rotmat = numpy.matmul(tmp,rotmat_x)

                                    # rotate ALL up and dn core FODs of the given atom
                                    rot_pos = numpy.array([numpy.matmul(rotmat,po) for po in fods_to_rot]) 
                                    # Get distances to bond/lone
                                    _, distmat = get_distances(rot_pos+self.atomic_pos[s],flatten_bond_lone,cell=self.cell,pbc=self.pbc_xyz) 
                        ...
                        fods_to_rot = numpy.array([numpy.matmul(best_rotmat,po) for po in fods_to_rot])
                        factor *= step

                    # Final cores
                    fods_to_rot = numpy.array([numpy.matmul(best_rotmat,po) for po in fods_to_rot]) + self.atomic_pos[s]
                    cores.append([[fods_to_rot[:core_up]],[fods_to_rot[core_up:]]])
