...
class PyfodMC(Database,Bonds,Lones,Cores):
    def __init__(self):
        '''
            ...
            invert_updn ... User input: If True, invert all UP with all DN FODs for atoms; for molecules, do it for core FODs          default: True
            pbc         ... User input: If True, fodMC assumes periodic boundary conditions in all directions                          default: False
            ...
            core_charge ... Place holder to assing the number of core charges to each atom; will be used in lone FOD determination     default: []
        '''
    ...
    def create_atomic_guess(self):
        ...
        '''
        # From Database
        fods = self.get_initial_FODs(species=self.atomic_sym[0],
                                     charge=self.charge[0],
                                     config=self.config[0],
                                     fix1s=self.fix1s,
                                     invert_updn=self.invert_updn,
                                     ecp=self.ecp[0],
        ...
        print(f"Charge = {charge}  spin = {spin}   mu = {dip[0]:10.4f} {dip[1]:10.4f} {dip[2]:10.4f}")

    def create_molecular_guess(self):
        '''
            Create molecular guess
        '''
        for s,sym in enumerate(self.atomic_sym):
            fods = self.get_initial_FODs(species=sym,
        ...
        for sym,cnt in zip(syms,cnts):
            if cnt == 1:
                ...
                formula += f"{sym}{cnt}"
        print(f"Generate molecular guess for {formula}")
        print(f"Charge = {charge}  spin = {spin}   mu = {dip[0]:10.4f} {dip[1]:10.4f} {dip[2]:10.4f}")


    def create_pbc_guess(self):
        '''
            Create pbc guess
        '''
        for s,sym in enumerate(self.atomic_sym):
        ...
        for sym,cnt in zip(syms,cnts):
            if cnt == 1:
                ...
                formula += f"{sym}{cnt}"
        print(f"Generate periodic guess for {formula}")
        print(f"Charge = {charge}  spin = {spin}   mu = {dip[0]:10.4f} {dip[1]:10.4f} {dip[2]:10.4f}")
...
if __name__ == "__main__":
    ...
    myfodmc.bonds        = {"0-0":["O-Ni","N-Ni","Ni-Ni"]}
    myfodmc.lone        = {"0-0":["Ni"],"3-3":["O"]}

    myfodmc.fix1s       = True
    myfodmc.invert_updn = False
    #myfodmc.pbc         = True

    #myfodmc.charge      = {2:["Ni"],-1:["O"],1:["C"]}
    #myfodmc.switch_updn = {True:[120,121]}
    #myfodmc.ecp         = {"sc":[0,1,'O'],"lc":["Ni"]}
    #myfodmc.config      = {'4s2':["Ni"]}
