...
class Bonds(HelperFunctions):
    ...
    def assign_bonds(self):
        '''
           Assign bond matrix according to user inputs; also, set all bonds to 1 if unassigned 
            ...
            Store distance matrix, distance vectors, and connectivity matrix for later usage in self
        '''
        #  use bond_type:[pairs of atoms, or atomic types]
        ...
        # Exclude entries that are atomic types -> see that end of this function
        dummy = dict()
        for key,value in self.bonds.items():
            for val in value:
                #Error check for atomic types; deal with them later
                if val.split("-")[0] in chemical_symbols and val.split("-")[0] not in self.atomic_sym:
                    raise ValueError(f"\nBonds for element {val.split('-')[0]} specified; this element does not exist in your system: {self.atomic_sym}!")
                elif val.split("-")[1] in chemical_symbols and val.split("-")[1] not in self.atomic_sym:
                    raise ValueError(f"\nBonds for element {val.split('-')[1]} specified; this element does not exist in your system: {self.atomic_sym}!")
                # If we have indices
                elif val.split("-")[0] not in self.atomic_sym:
                    val1 = int(val.split("-")[0])
                    val2 = int(val.split("-")[1])
                    # Make sure the indices make sense
                    if val1 >= len(self.atomic_pos):
                        raise ValueError(f"\nBonds for atomic index {val1} specified ({val}); however, atomic indices only range from 0 to {len(self.atomic_pos)-1}")
                    if val1 >= len(self.atomic_pos) or val2 >= len(self.atomic_pos):
                        raise ValueError(f"\nBonds for atomic index {val2} specified ({val}); however, atomic indices only range from 0 to {len(self.atomic_pos)-1}")
                    ...
                    # sanity check: if keys are identical -> not a valid input
                        ...
                        raise ValueError(f"\nBonds cannot be formed between the same atom; check inputs of bonds; entry found: {val,key}")
                    else:
                        dummy[val] = key
        ...
        for key,value in self.bonds.items():
            if key.split("-")[0] in self.atomic_sym:
                ...
                if key.split("-")[1] not in self.atomic_sym:
                    ...
                    idx1 = numpy.where(numpy.array(self.atomic_sym) == key.split("-")[0])[0]
                    idx2 = numpy.where(numpy.array(self.atomic_sym) == key.split("-")[1])[0]
                    for id1 in idx1:
                        for id2 in idx2:
                            if id1 > id2:
        ...
        self.dist_vecs, self.dist_mat = get_distances(self.atomic_pos,cell=self.cell,pbc=self.pbc_xyz)
        ...
        for s1,sym1 in enumerate(self.atomic_sym):
            for s2,sym2 in enumerate(self.atomic_sym):
                ...
                cov_mat[s1,s2] = covalent_radii[atomic_numbers[sym1]] + covalent_radii[atomic_numbers[sym2]] + 0.2
        ...
        for idx1,idx2 in zip(mask[0],mask[1]):
            if idx1 != idx2:
                ...
                if f"{idx1}-{idx2}" not in self.bonds.keys():
                    # Form bonds specified via atomic types!
                    if f"{idx1}-{idx2}" in additional_indices:
                        self.bonds[f"{idx1}-{idx2}"] = additional_indices[f"{idx1}-{idx2}"]
                    else:
        ...
        self.con_mat = numpy.zeros((2,len(self.atomic_sym),len(self.atomic_sym)),dtype=int)
        for key,value in self.bonds.items():
        ...
        self.r_bond = max([max(self.dist_mat[mask_up]),max(self.dist_mat[mask_dn])])/2.
    ...
    def make_multiplebonds(self,
        ...
        vec_bond = self.dist_vecs[idx_atom1,idx_atom2]
        #
        motif = self.get_motif(no_points=bond_order,r=self.r_bond*self.scale_r,core_or_bond='bond')
        ...
        for keys,values in self.bond_orientation.items():
            ...
            if idx_atom1 in [int(keys.split("-")[0]),int(keys.split("-")[1])] and self.is_linear_planar[idx_atom1] not in ["planar4bonds","planar4both"] and self.is_linear_planar[idx_atom2] not in ["planar4bonds","planar4both"]:
                org_vec = values
                ...
                if bond_order > 2:
                     perp_motif = numpy.cross(rot_pos[0],rot_pos[1])/numpy.linalg.norm(numpy.cross(rot_pos[0],rot_pos[1]))
                     # If angle is 180* -> do not do anything -> would flip motif!
                     if numpy.dot(perp_motif,vec_bond)/(numpy.linalg.norm(perp_motif)*numpy.linalg.norm(vec_bond)) == -1.0:
                ...
                if bond_order == 4:
                    ...
                    linear_rot = self.rotmat_around_axis(axis=vec_bond,angle=numpy.pi/3)
                else:
                    linear_rot = self.rotmat_around_axis(axis=vec_bond,angle=numpy.pi/bond_order)
                rot_pos3 = numpy.array([numpy.matmul(linear_rot,po) for po in rot_pos2])
                # Add this orientation to self.bond_orientations
                self.bond_orientation[f"{idx_atom1}-{idx_atom2}"] = rot_pos3[0]/numpy.linalg.norm(rot_pos3[0])
        ...
        if self.is_linear_planar[idx_atom1] in ["planar4bonds","planar4both"]:
            ...
            bonds = [self.dist_vecs[idx_atom1][partner]/numpy.linalg.norm(self.dist_vecs[idx_atom1][partner]) for partner in self.bond_partners[idx_atom1]]
            outofplane = numpy.cross(bonds[0],bonds[1])/numpy.linalg.norm(numpy.cross(bonds[0],bonds[1]))
            ...
            # Get out-of-plane direction via cross product. rotate tmp to align with that
            bonds = [self.dist_vecs[idx_atom2][partner]/numpy.linalg.norm(self.dist_vecs[idx_atom2][partner]) for partner in self.bond_partners[idx_atom2]]
            outofplane = numpy.cross(bonds[0],bonds[1])/numpy.linalg.norm(numpy.cross(bonds[0],bonds[1]))
            # Rotate tmp into outofplane
            rotmat = self.rotmat_vec1vec2(vec2rot,outofplane)
        # any linear -> check whether this is the first or the second bond -> invert 2nd one!
            ...
            outofaxis = numpy.cross(vec_bond,[0,0,1])#/numpy.linalg.norm(numpy.cross(vec_bond,[0,0,1]))
            if outofaxis.tolist() == [0,0,0]:
                outofaxis = numpy.cross(vec_bond,[1,0,0])/numpy.linalg.norm(numpy.cross(vec_bond,[1,0,0]))
            else:
            ...
            if len(self.bond_partners[idx_atom1]) >= len(self.bond_partners[idx_atom2]):
                tpartn = [partner for partner in self.bond_partners[idx_atom1] if partner != idx_atom2]
            else:
                tpartn = [partner for partner in self.bond_partners[idx_atom2] if partner != idx_atom1]
            bond = self.atomic_pos[tpartn[0]] - self.atomic_pos[tpartn[1]]
            orientation = numpy.cross(vec_bond,bond)/numpy.linalg.norm(numpy.cross(vec_bond,bond))
        ...
        if perp_motif.tolist() == [0,0,0]:
            self.bond_orientation[f"{idx_atom1}-{idx_atom2}"] = rot_pos[0]/numpy.linalg.norm(rot_pos[0])
        ...
        self.bond_orientation[f"{idx_atom1}-{idx_atom2}"] = rot_pos2[0]/numpy.linalg.norm(rot_pos2[0])
    ...
    def create_bonds(self):
        ...
        no_bonds_sort = numpy.argsort([len(partners) for partners in self.bond_partners])
        ...
        #   re-use bond_partners from is_linear_planar
            ...
            bonds_to_consider = [partner for partner in  self.bond_partners[s] if partner not in bonds_sort[:idx_sort+1]]

            for s2,(con_up,con_dn) in enumerate(zip(self.con_mat[0][s],self.con_mat[1][s])):
