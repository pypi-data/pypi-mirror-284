...
# print documantation info to JSON. It is a choice with 2 options, schema or cli
@click.option("-d", "--docinfo", type=click.Choice(choices=["schema", "cli"], case_sensitive=False), help="Print documantation info to JSON.")
...
def main(ctx, version, docinfo):
    ...
    - name: Generate dsl-schema.json
    run: storm -d schema > ../storm-doc/static/json/dsl-schema.json
...
########################### DISPLAY #############################
SCHEMA_FORMAT_HELP="""The output format. Write a Markdown file or show an HTML page. If you want to export schema documentation in pure JSON format, use `storm -d schema` instead of this command."""
