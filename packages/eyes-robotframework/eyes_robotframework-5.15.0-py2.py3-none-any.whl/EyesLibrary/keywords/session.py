...
class SessionKeywords(LibraryComponent):
    ...
    def locate(self, *args):
        # type: (*str) -> Union[Region, Dict[str, List[Region]]]
        """
        Locates visual elements using the Applitools Eyes visual locator functionality.

            | =Arguments=   | =Description=                                                   |
            | locator_name  | *Mandatory* - The name of the visual locator to search for      |
            | First         | *Optional* - Return only the first matching result              |
            | All           | *Optional* - Return all matching results                        |
            | name:value    | *Optional* - Add additional locator name(s)                     |
            | image:path    | *Optional* - Specify an image path for the locator              |

        *Returns:*
        - If 'First' is specified or by default: A Region object or None if not found.
        - If 'All' is specified: A dictionary of locator names to lists of Region objects.

        *Example:*
            | ${result}=    | Eyes Locate | my-locator | First |
            | ${results}=   | Eyes Locate | locator1 | name:locator2 | All |
            | ${results}=   | Eyes Locate | main-locator | image:path/to/image.png | All |
        """
        if not args:
            raise ValueError("At least one locator name must be provided")

        locator = VisualLocator.name(args[0])
        ...
        for arg in args[1:]:
            if arg.lower() == "first":
                ...
                locator = locator.name(arg.split(":", 1)[1].strip())
            elif arg.startswith("image:"):
                locator = locator.image(arg.split(":", 1)[1].strip())
            else:
                raise ValueError(f"Unknown argument: {arg}")

        return self.current_eyes.locate(locator)
