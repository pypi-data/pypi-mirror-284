...
from sklearn.cluster._k_means_fast import CHUNK_SIZE
from sklearn.cluster._k_means_fast cimport _relocate_empty_clusters_dense
from sklearn.cluster._k_means_fast cimport _relocate_empty_clusters_sparse
from sklearn.cluster._k_means_fast cimport _average_centers, _center_shift


np.import_array()


cdef convert_to_python(floating *ptr, int n):
    cdef int i
    lst=[]
    for i in range(n):
        lst.append(ptr[i])
...
def lloyd_iter_chunked_dense_reg(
        ...
        int n_threads,
        floating lam,                              # IN
        int[::1] edges_indptr,                     # IN
        int[::1] edges_indices,                    # IN
    ...
    cdef:
        ...
        floating *pairwise_distances_ctr
    ...
    pairwise_distances_ctr = <floating*> calloc(n_clusters * n_clusters, sizeof(floating))
    if edges_indptr is not None:
        _update_pairwise_distances_ctr(centers_old, centers_squared_norms, pairwise_distances_ctr)
    ...
    with nogil, parallel(num_threads=n_threads):
        ...
        for chunk_idx in prange(n_chunks, schedule='static'):
            ...
            _update_chunk_dense(
                ...
                edges_indptr[start: end + 1],
                edges_indices,
                lam,
                pairwise_distances_ctr,
                labels,
                labels[start: end],
                centers_new_chunk,
    ...
    # print(convert_to_python(&centers_new[0, 0], n_clusters * n_features))


cdef void _update_pairwise_distances_ctr(
        floating[:, ::1] centers_old,         # IN
        floating[::1] centers_squared_norms,  # IN
        floating *pairwise_distances_ctr      # OUT
        ) nogil:
    cdef:
    ...
    _gemm(RowMajor, NoTrans, Trans, n_clusters, n_clusters, n_features,
        -2.0, &centers_old[0, 0], n_features, &centers_old[0, 0], n_features,
        1.0, pairwise_distances_ctr, n_clusters)
    
    for i in range(n_clusters):
        for j in range(n_clusters):
            pairwise_distances_ctr[i * n_clusters + j] += centers_squared_norms[i] + centers_squared_norms[j]
...
cdef void _update_chunk_dense(
        ...
        int[::1] edges_indices,               # IN
        floating lam,                         # IN
        floating *pairwise_distances_ctr,     # IN
        int[::1] labels_all,                  # IN
        int[::1] labels,                      # OUT
    ...
    for i in range(n_samples):
        for j in range(n_clusters):
            ...
            if edges_indptr is not None:
                new_lam = lam / (edges_indptr[i + 1] - edges_indptr[i])
                for k in range(edges_indptr[i], edges_indptr[i + 1]):
                    pairwise_distances[i * n_clusters + j] += new_lam * pairwise_distances_ctr[j * n_clusters + labels_all[edges_indices[k]]]
