...
SELECT * FROM `project_id.dataset_id.your_table_name` ORDER BY your_date_column DESC LIMIT 100;
    ...
    def astc(self, url_column, transcription_column, classify=None, classification_model='roberta-large-mnli'):
        ...
        def process_url(session, url, classify_labels):
        ...
        for url in tqdm(urls, desc="Processing URLs"):
            transcription, labels_dict, url, error = process_url(session, url, classify)
            if error:
                print(f"Failed to process {url}: {error}")
