...
def businfo(province,city, keyword,key,key_fwd,jscode,save_path, region_xzq=None,polygon_mult=None):
    relistname = []
    # 判断结果相似性
    ...
    def station(html, keywords, bus_station, buslines_information, buslines_polyline, city):
        ...
        count = 0
        # print(buslines[0]['name'])
        # if len(buslines)>0:
        #     if buslines[0]['name']:
        #         if buslines[0]['name'] in ['M441路(深业花园公交总站--梅山中学)','M120区间3路(福保桂花苑南--下沙总站)',"高快巴市108号线(中兴通讯2--龙岗区政府)"]:
        #             print(buslines[0]['name']+'buslines长度为:'+str(len(buslines))+'；返回线路数量为：'+html['count'])
        #     else:
        #         print(keywords)
        # else:
        #     print('{}返回了空值！'.format(keywords))
    ...
    # 使用行政区方法
        ...
        # citybus_name=pd.read_csv('xianlu.csv')['0'].to_list()
        # citybus_name=['M120区间3路','M441路','(停运)高快巴市108号线']
        # print(citybus_name)
        # print(len(citybus_name))
    """
