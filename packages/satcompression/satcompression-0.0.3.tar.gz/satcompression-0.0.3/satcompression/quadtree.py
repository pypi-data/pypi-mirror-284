...
class QuadTreeNodeCategorical:
    ...
    def _create_child_node(self, position: tuple, size: tuple) -> "QuadTreeNodeCategorical":
        """Creates a new quad tree node with the given position and size.
        ...
        Returns:
            QuadTreeNode: A new quad tree node.
        """
        return QuadTreeNodeCategorical(position, size)
    ...
    def subdivide(self) -> tuple:
        ...
        # Create the child nodes.
        self.bottom_left_node: QuadTreeNodeCategorical = self._create_child_node(
            position=(x, y), size=(split_width, split_height)
        )

        self.bottom_right_node: QuadTreeNodeCategorical = self._create_child_node(
            position=(x + split_width, y), size=(width - split_width, split_height)
        )

        self.top_left_node: QuadTreeNodeCategorical = self._create_child_node(
            position=(x, y + split_height), size=(split_width, height - split_height)
        )

        self.top_right_node: QuadTreeNodeCategorical = self._create_child_node(
            position=(x + split_width, y + split_height),
            size=(width - split_width, height - split_height),
        )
    ...
    def draw_self(self, image_data: np.array):
        ...
        # I Want a categorical map based on the width and height of the node
        bands = len(self.color)
        categorical_info = np.array(width).repeat(bands).reshape(-1, 1, 1)
        image_data[..., start_y:end_y, start_x:end_x] = categorical_info        
