...
class QuadTreeNode:
    ...
    def subdivide(self) -> tuple:
        ...
        self.bottom_right_node: QuadTreeNode = self._create_child_node(
            position=(x + split_width, y),
            size=(width - split_width, split_height)
        )

        self.top_left_node: QuadTreeNode = self._create_child_node(
            position=(x, y + split_height),
            size=(split_width, height - split_height)
        )
    ...
    def draw_self(self, image_data: np.array):
        ...
        image_data[..., start_y: end_y, start_x: end_x] = np.array(self.color).reshape(-1, 1, 1)
