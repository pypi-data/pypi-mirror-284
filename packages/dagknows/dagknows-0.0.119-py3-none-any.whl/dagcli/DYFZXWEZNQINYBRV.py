...
def getParamValue(paramname):
    if paramname in PARAM_VALUES: return PARAM_VALUES[paramname]
    return 

def getEnvVar(varname): return os.environ.get(varname)
...
class job_o9w1CTgDboB5ugqRh3b1(BaseJobRunner):
    def __init__(self, *args):
        super().__init__(*args)
        self.runbook_task_id = ''
        self.starting_child_path = ''
        self.starting_task_id = 'mSjt5jZOnD7wSnfEwGmS'
        self.starting_func_name = 'task_mSjt5jZOnD7wSnfEwGmS'
        self.starting_task_param_values = {}
        self.special_param_values = {}

    ### defs for each task goes here

    def task_mSjt5jZOnD7wSnfEwGmS(self, context):
        """ Check Elasticsearch Cluster Health
        This script checks the health of an Elasticsearch cluster by making an HTTP GET request to the cluster's health endpoint. It uses basic authentication with a username and password to access the cluster. The script outputs the JSON response from the Elasticsearch cluster health API.
        """
        self.debugprint("ENTERING task_mSjt5jZOnD7wSnfEwGmS....", context, context.vars)
        print = context.ctxprint
        # TODO - import other "magic" variables - eg _xyz
    
        try:
        
            if 'elastic_url' in context.vars: elastic_url = typecasted(context.vars['elastic_url'], 'str')
            self.debugprint("SETTING INPUT PARAMS task_mSjt5jZOnD7wSnfEwGmS elastic_url = ", context.vars.get('elastic_url', None), "HasIt?: ", 'elastic_url' in context.vars)
            if 'elastic_user_name' in context.vars: elastic_user_name = typecasted(context.vars['elastic_user_name'], 'str')
            self.debugprint("SETTING INPUT PARAMS task_mSjt5jZOnD7wSnfEwGmS elastic_user_name = ", context.vars.get('elastic_user_name', None), "HasIt?: ", 'elastic_user_name' in context.vars)
            if 'elastic_password' in context.vars: elastic_password = typecasted(context.vars['elastic_password'], 'str')
            self.debugprint("SETTING INPUT PARAMS task_mSjt5jZOnD7wSnfEwGmS elastic_password = ", context.vars.get('elastic_password', None), "HasIt?: ", 'elastic_password' in context.vars)
            ...
            print(response.json())
            if context.skip_sub_tasks: return context.taskFinished(True, locals(), [{'name': 'response', 'param_type': 'Response', 'default_value': '', 'required': False}])
            return context.taskFinished(True, locals(), [{'name': 'response', 'param_type': 'Response', 'default_value': '', 'required': False}])
            
        except Exception as e:
        
            return context.taskFailed(locals(), sys.exc_info())
            
ALLTASKS = {'mSjt5jZOnD7wSnfEwGmS': {'id': 'mSjt5jZOnD7wSnfEwGmS', 'conv_id': '', 'title': 'Check Elasticsearch Cluster Health', 'task_type': '', 'task_category': '', 'script_type': 'python', 'script': {'lang': 'python', 'code': 'import os\nimport requests\nfrom requests.auth import HTTPBasicAuth\n\nPARAM_VALUES = {\n    \'elastic_url\': getEnvVar(\'ELASTIC_URL\'),\n    \'elastic_user_name\': getEnvVar(\'ELASTIC_USER_NAME\'),\n    \'elastic_password\': getEnvVar(\'ELASTIC_PASSWORD\')\n}\n\n\nelastic_url = getParamValue(\'elastic_url\')\nelastic_user_name = getParamValue(\'elastic_user_name\')\nelastic_password = getParamValue(\'elastic_password\')\n\nurl = f"{elastic_url}/_cluster/health"\nresponse = requests.get(url, auth=HTTPBasicAuth(elastic_user_name, elastic_password))\n\nprint(response.json())\n'}, 'description': "This script checks the health of an Elasticsearch cluster by making an HTTP GET request to the cluster's health endpoint. It uses basic authentication with a username and password to access the cluster. The script outputs the JSON response from the Elasticsearch cluster health API.", 'description_type': '', 'num_voters': 0, 'num_parents': 1, 'parent_tasks': ['DzXJqqbjNZK3SNnA3Spr'], 'extras': {}, 'tags': [], 'workspace_ids': [], 'input_params': [{'name': 'elastic_url', 'param_type': 'str', 'default_value': '', 'required': False}, {'name': 'elastic_user_name', 'param_type': 'str', 'default_value': '', 'required': False}, {'name': 'elastic_password', 'param_type': 'str', 'default_value': '', 'required': False}], 'output_params': [{'name': 'response', 'param_type': 'Response', 'default_value': '', 'required': False}], 'permissions': [], 'workspace_status': [], 'commands': [], 'created_at': 1720549751.996961, 'updated_at': 1720549753.9152813, 'creator': '1', 'sub_tasks': [], 'source_info': {'source_url': '', 'source_name': '', 'source_id': '', 'source_version': '', 'last_synced_at': 1720549751.997061}, 'ref_task_ids': [], 'created_at_bucket': 28675828, 'metadata': {'last_vectorized': 1720549751.9974694, 'vec_version': 0, '_seq_no': 13475, '_primary_term': 3}}}

JOB_ID = 'o9w1CTgDboB5ugqRh3b1'
...
token = "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..mH1mIAGh4--1S0Gv-JvhToY3Mm-7FmBu5QMngEZaTmwYLK_iD7Y1foKO0FqmzZi7yeX7_bRGqAq2J9l0x5kXTUo13aUl2crW0C-DPJxIz92MJ5H1RIqpduRU-XI0RBo5dJuLmLUVynvgvbaiCuNaswY1Cgz-QQBRcHlu4jaY4mnhH_ga5gF1B_lAK3sMfAGm5JsGqhUYfYIY-mJmHV13RMD3hR9oy--ZV_eKPmInGH8EmRVb3HSIDuCXNJKLUW1jKYqL-Pl2bvfY0CHjf5Fgr-PnQyv0DpEwewcmGVV8JRp2ydbo-ravGBk2pWlJD0NrS2gHLUAn_8mzX5tULt4fJw"
