...
class EAction:
    ...
    def to_bytes(self):
        ...
        return result + self.id + self.pk + self.sign + self.action + self.input + self.output + self.error
    ...
    def from_bytes(self, packed_data):
        ...
        self.action = buffer.read(actionLength) if actionLength > 0 else b''
