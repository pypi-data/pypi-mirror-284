...
class CApiFlow:
    ...
    async def onEActionInput(
        ...
        try:
            ...
            eAction = IuApiRequest.toEAction(data=dataERequest)

            IuLog.doVerbose(__name__, f"eAction:{eAction}")
            
            bytesChunk = eAction.input
            
            if  eAction.id not in self.mapERequestInfo.keys():          
                eRequestInfoTmp = ERequestInfo()
                eRequestInfoTmp.id = eAction.id
                eRequestInfoTmp.length = eAction.length
                eRequestInfoTmp.doStartTime()
                self.mapERequestInfo.doSet(eRequestInfoTmp)
                
            eRequestInfo:ERequestInfo = self.mapERequestInfo.doGet(eAction.id)
            
            lengthOffset = eAction.offsetEnd - eAction.offsetStart
            if  lengthOffset == eAction.length:
                eRequestInfo.isChunk = False
                eRequestInfo.chunkCount = 1             
                
                eActionInput = IuApiRequest.toEActionInput(eAction.data)
                return (eRequestInfo, eActionInput)
                       
            else:
                file_path = f"{self.pathTemp}/{eAction.id.hex()}.eam"
                
                if os.path.exists(file_path):
                    # File exists, check its size
                    current_size = os.path.getsize(file_path)
                    if current_size != eAction.length:
                ...
                if not os.path.exists(file_path):
                    with open(file_path, 'wb') as file:
                        file.write(b'\0' * eAction.length)
                        IuLog.doDebug(__name__, f"file created: {file_path} {eAction.length}={os.path.getsize(file_path)}")
                
                async with aiofiles.open(file_path, mode='r+b') as file:
                    await file.seek(eAction.offsetStart)
                    await file.write(bytesChunk)
                    await file.close()
               
                eRequestInfo.lengthCurrent += lengthOffset
                
                if  eRequestInfo.lengthCurrent == eAction.length:
