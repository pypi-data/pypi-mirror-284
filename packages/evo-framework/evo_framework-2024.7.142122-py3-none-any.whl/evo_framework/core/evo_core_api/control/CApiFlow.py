...
class CApiFlow:
    ...
    async def onEActionInput(
        ...
        try:
            ...
            eHeader = IuApiRequest.toEHeader(data=dataERequest)

            IuLog.doVerbose(__name__, f"eRequest:{eHeader}")
            
            bytesChunk = eHeader.data
            
            if  eHeader.id not in self.mapERequestInfo.keys():          
                eRequestInfoTmp = ERequestInfo()
                eRequestInfoTmp.id = eHeader.id
                eRequestInfoTmp.length = eHeader.length
                eRequestInfoTmp.doStartTime()
                self.mapERequestInfo.doSet(eRequestInfoTmp)
                
            eRequestInfo:ERequestInfo = self.mapERequestInfo.doGet(eHeader.id)
            
            lengthOffset = eHeader.offsetEnd - eHeader.offsetStart
            if  lengthOffset == eHeader.length:
                eRequestInfo.isChunk = False
                eRequestInfo.chunkCount = 1             
                
                eActionInput = IuApiRequest.toEActionInput(eHeader.data)
                return (eRequestInfo, eActionInput)
                       
            else:
                file_path = f"{self.pathTemp}/{eHeader.id.hex()}.eam"
                
                if os.path.exists(file_path):
                    # File exists, check its size
                    current_size = os.path.getsize(file_path)
                    if current_size != eHeader.length:
                ...
                if not os.path.exists(file_path):
                    with open(file_path, 'wb') as file:
                        file.write(b'\0' * eHeader.length)
                        IuLog.doDebug(__name__, f"file created: {file_path} {eHeader.length}={os.path.getsize(file_path)}")
                
                async with aiofiles.open(file_path, mode='r+b') as file:
                    await file.seek(eHeader.offsetStart)
                    await file.write(bytesChunk)
                    await file.close()
               
                eRequestInfo.lengthCurrent += lengthOffset
                
                if  eRequestInfo.lengthCurrent == eHeader.length:
                    async with aiofiles.open(file_path, mode="rb") as file:
                        dataERequest = await file.read()
                        IuLog.doDebug(__name__, f"COMPLETE CHUNK: {len(dataERequest)} {file_path}")
                    
                    #os.remove(file_path)
                    
                    eRequestInfo.isChunk=False  
                    eActionInput = IuApiRequest.toEActionInput(dataERequest)
    ...
    async def onEActionOutput(self, eAction: EAction) -> bytes:
        try:
            ...
            eHeader = IuApiRequest.newEHeader(id=eActionOutput.id, data=dataEResponse)
            
            return eHeader.toBytes()
            ...
            if not eRequestInfo.isChunk:
                try:
                    ...
                    async for eActionOutput in self.onActionStream(eApi, eActionTask):
                        ...
                        IuLog.doInfo(
                            __name__,
                            f"{eActionInput.action} id:{eActionOutput.id} len:{len(dataResponse)} {eRequestInfo.doGetElapsedPartial()}",
                        )
                        yield dataResponse
