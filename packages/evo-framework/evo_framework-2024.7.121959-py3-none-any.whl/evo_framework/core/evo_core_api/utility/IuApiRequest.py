 ...
 # ---------------------------------------------------------------------------------------------------------------------------------------      
    ...
    def toEActionInput(         
        ...
        try:
            ...
            eActionIput = IuApi.toEObject(EAction(), data)

            if isinstance(eActionIput, EAction):
                if len(eActionIput.id) < 32: #sha256
                    raise Exception(f"ERROR_NOT_VALID|{eActionIput.id!r}")

                # TODO:decrypt data with eRequest chiper
                
                if eActionIput.enumApiCompress == EnumApiCompress.NONE:
                    pass
                
                elif eActionIput.enumApiCompress == EnumApiCompress.LZ4:
                    eActionIput.input = lz4.frame.decompress(eActionIput.input)
                    
                elif eActionIput.enumApiCompress == EnumApiCompress.GZIP:
                    eActionIput.input = gzip.decompress(eActionIput.input)
                

                hash = IuCryptHash.toSha256Bytes(eActionIput.input)

                IuLog.doVerbose(
                    __name__,
                    f"CHUNK HASH: {eActionIput.id.hex()} {hash!r}",
                )
               
                if not eActionIput.pk:
                    raise Exception("ERROR_NOT_VALID|PK")
                
                IuLog.doVerbose(__name__, f"checkSign: {eActionIput}")
                ...
                if publicKey is None:
                    publicKey= eActionIput.pk
                
                isValid = IuCryptEC.verify_data(
                    hash, eActionIput.sign, publicKey
                )
                
                
                IuLog.doVerbose(
                    __name__,
                    f"checkSign: {eActionIput.id!r} {signSha256!r} isValid:{isValid}",
                )
...
# ---------------------------------------------------------------------------------------------------------------------------------------      
    @staticmethod
    def newEHeader(id:bytes, data:bytes, offsetStart:int=0, offsetEnd:int=-1, length:int=-1 ) -> EHeader:
        try:  
            eHeader = EHeader()
            eHeader.id = id
            eHeader.offsetStart = offsetStart
...
# ---------------------------------------------------------------------------------------------------------------------------------------      
    @staticmethod
    def toEHeader(data:bytes) -> EHeader:
        try:  
           # eHeader = IuApi.toEObject(EHeader(), data )
            eHeader = EHeader() 
            eHeader.fromBytes(data)
            
            IuLog.doVerbose(__name__, f"toEHeader:{eHeader}")
            
            if eHeader.id is None:
                raise Exception("ERROR_NOT_VALID|EHeader")

            if len(eHeader.id) < 32: #sha256
                raise Exception(f"ERROR_NOT_VALID|EHeader.id|{eHeader.id!r}")
            
            offsetData = eHeader.offsetEnd - eHeader.offsetStart
            lengthData = len(eHeader.data)
            
            IuLog.doVerbose(__name__, f"offsetData:{offsetData} {lengthData}")
            
            if offsetData != lengthData:
                raise Exception("ERROR_NOT_VALID|DATA_LENGTH")
