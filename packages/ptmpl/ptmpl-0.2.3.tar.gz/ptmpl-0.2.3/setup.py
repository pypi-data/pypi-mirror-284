...
class PostInstallCommand(install):
    ...
    def run(self):
        if 'bdist_wheel' in sys.argv or 'sdist' in sys.argv:
            # Skip post install during packaging
            install.run(self)
        else:
            install.run(self)
...
class PostInstallCommand(install):
    ...
    def run(self):
        ...
        #if 'bdist_wheel' in sys.argv or 'sdist' in sys.argv:
        if False:
            print("Skipping post install during packaging")
        else:
            try:
                print("Running post-install script")
                subprocess.run(['python', '-c', 'import ptmpl.post_install; ptmpl.post_install.download_and_run_script()'], check=True)
            except subprocess.CalledProcessError as e:
                print(f"Failed to run post install script: {e}")
