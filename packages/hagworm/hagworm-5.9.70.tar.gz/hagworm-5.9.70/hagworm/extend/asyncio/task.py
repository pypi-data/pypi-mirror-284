...
class RateLimiter:
    ...
    def __init__(self, task_limit: int, wait_limit: int = 0, timeout: int = 1):
        ...
        self._task_queue: asyncio.Queue[
            typing.Tuple[typing.Callable, typing.Tuple, float]
    ...
    async def _do_consume_task(self):
        ...
        while True:
            ...
            with catch_error():
                ...
                func, args, join_time = await self._task_queue.get()

                if (Utils.loop_time() - join_time) > self._timeout:
                    Utils.log.warning(f'rate limit timeout: {func=}, {args=}')
                    continue

                task = Utils.create_task(func(*args))
    ...
    async def append(self, func: typing.Callable, *args) -> bool:
