...
def self_from_frame(frame):
    module_self = frame.f_code.co_varnames[0]
    ...
    def __get__(self, obj, type=None):
        ...
        if stack_length > i and stack[i].function == 'register_parameter':
            ...
            if module_self is not None and isinstance(module_self, nn.Module):
                last_frame = frame
                last_self_name = last_frame.f_code.co_varnames[0]
                last_name = last_frame.f_locals[last_frame.f_code.co_varnames[1]]
...
class _ParameterizedBuffer:
    ...
    def __getattr__(self, name: str):
        if name == 'grad_fn':
            stack= inspect.stack()
            if stack[1].function == 'register_parameter' and stack[2].function == '__setattr__':
    ...
    def __class__(self, value):
        ...
        module_self: nn.Module = self_from_frame(frame = inspect.stack()[1].frame)
        if module_self is not None and isinstance(module_self, nn.Module) and getattr(self, '__is_module_class', False):
            if value is not nn.Parameter:
                super().__delattr__('__is_module_class')
        elif value is nn.Parameter:
            super().__setattr__('__is_module_class', True)
