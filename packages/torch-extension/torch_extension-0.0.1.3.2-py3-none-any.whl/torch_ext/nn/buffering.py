...
class _ParameterizedBuffer:
    ...
    def __class__(self):
        ...
        if stack_length > i and (stack[i].function == '__setattr__' or stack[i].function == 'register_parameter'):
            ...
            __class = super().__getattribute__('__class')
            if module_self is not None and isinstance(module_self, nn.Module) and (getattr(self, '__is_module_class', False) or (__class is torch.Tensor)):
                return nn.Parameter
        return __class if __class is not None else super().__getattribute__('__class')
