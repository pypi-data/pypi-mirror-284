...
class WebullTrading:
    ...
    async def get_ticker_id(self, ticker):
        """Converts ticker name to ticker ID using the saved dataframe."""
        try:
            result = self.df.loc[self.df['ticker'] == ticker, 'id']
            if not result.empty:
            ...
            print(f"Error while retrieving ticker ID for {ticker}: {e}")
            return None
    ...
    async def async_get_td9(self, ticker, interval):
        try:
            ...
            headers = await screen.get_headers()
