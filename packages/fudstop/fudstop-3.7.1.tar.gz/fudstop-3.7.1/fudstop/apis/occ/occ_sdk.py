...
class occSDK:
    ...
    async def stock_info(self, symbol:str, has_options:bool=True):
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get(f"https://private-authorization.ivolatility.com/lookup/stockInformation?stockId={stockId}&region=1&rtMode=RT&hasOptions={has_options}", headers=self.headers)
