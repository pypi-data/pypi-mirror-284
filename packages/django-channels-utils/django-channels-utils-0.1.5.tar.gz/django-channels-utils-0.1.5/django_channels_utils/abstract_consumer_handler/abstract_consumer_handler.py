...
class HandlerMeta(type):
    def __new__(cls, name, bases, class_dict):
        ...
        if not hasattr(new_cls, 'handlers'):
            new_cls.handlers = {}
        if not HANDLER_ABSTRACT_CLASS_NAME in bases:
            return new_cls
        print("bases", bases)
        if HANDLER_ABSTRACT_CLASS_NAME in bases and not hasattr(new_cls, 'type'):
            raise TypeMissingError(name)
        if HANDLER_ABSTRACT_CLASS_NAME in bases and not hasattr(class_dict, 'handle'):
