...
class IdentityVerification:
    ...
    def extract_document_info(self, image, side, document_type, country):
        ...
        if side=='front':
            ...
            logging.info(f"\n--------------Time taken for Front ID Extraction in IDV package: {time.time() - st} seconds")
            return document_data

        if side=='back':
            document_data = self.extract_back_id_info(image, country)
            logging.info(f"\n--------------Time taken for Back ID Extraction in IDV package: {time.time() - st} seconds")

        elif document_type == 'passport':
            document_data = self.exract_passport_info(image, country)
            logging.info(f"\n--------------Time taken for Passport Extraction in IDV package: {time.time() - st} seconds")
    ...
    def extract_front_id_info(self, front_id, country):
        ...
        if country == 'IRQ':
                ...
                try:
                    st = time.time()
                    processed_front_id = self.image_conversion(front_id)
                    logging.info(f'\n----------------Time taken for image conversion: {time.time() - st} seconds')
                    ...
                    front_id_text_desc = front_id_text[0].description
                    logging.info(f'\n----------------Time taken for vision: {time.time() - st} seconds')
                    ...
                    front_blurred, front_glare = self.get_blurred_and_glared_for_doc(image)
                    logging.info(f'\n----------------Time taken for fraud detection attributes: {time.time() - st} seconds')

                    st = time.time()
                    front_face_locations, front_face_encodings = load_and_process_image_deepface(front_id)
                    logging.info(f'\n----------------Time taken for face extraction: {time.time() - st} seconds')
                    # front_face_locations, front_face_encodings = self.load_and_process_image_fr(front_id)
                    ...
                    logging.info(f'\n----------------Time taken for iraq data formatting and final extraction: {time.time() - st} seconds')
    ...
    def extract_back_id_info(self, back_id, country):
        ...
        if country == 'IRQ':
                ...
                try:
                    ...
                    logging.info(f'\n--------------Time taken for back id data formatting final extraction: {time.time() - st}')
    ...
    def extract_ocr_info(self, data, video, country, report_names, back_img = None):
        ...
        logging.info(f"\n--------------Time taken for Extract Ocr Function in IDV package: {time.time() - st} seconds")
