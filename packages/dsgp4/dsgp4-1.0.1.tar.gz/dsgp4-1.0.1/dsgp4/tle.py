...
def load_from_lines(lines, opsmode='i'):
    ...
    if (len(line) >= 64 and
        ...
        data['_bstar'] = torch.tensor(float(line[53]+'.'+line[54:59])*pow(10, int(line[59:61])),dtype=torch_type)
        data['_ndot'] = torch.tensor(float(line[33:43])/(xpdotp*1440.0),dtype=torch_type)
        data['_nddot']= torch.tensor(float(line[44] + '.' + line[45:50])/(xpdotp*1440.0*1440),dtype=torch_type)
    ...
    if (len(line) >= 69 and
        ...
        data['line2'] = line
        #for SGP4:
        data['_inclo']=torch.tensor(np.deg2rad(float(line[8:16])),dtype=torch_type)
        data['_nodeo']=torch.tensor(np.deg2rad(float(line[17:25])),dtype=torch_type)
        data['_ecco'] =torch.tensor(float('0.'+line[26:33].replace(' ', '0')),dtype=torch_type)
        data['_argpo']=torch.tensor(np.deg2rad(float(line[34:42])),dtype=torch_type)
        data['_mo']=torch.tensor(np.deg2rad(float(line[43:51])),dtype=torch_type)
        data['_no_kozai'] =torch.tensor(float(line[52:63]) / xpdotp,dtype=torch_type);
    ...
    data['_epochyr'] = torch.tensor(year,dtype=torch_type)
    data['_jdsatepoch'] = torch.tensor(util.jday(year,mon,day,hr,minute,sec)[0],dtype=torch_type);
    data['_jdsatepochF'] = torch.tensor(util.jday(year,mon,day,hr,minute,sec)[1],dtype=torch_type);
...
def load_from_data(data, opsmode='i'):
    ...
    data['date_string'] = date_string
    #for SGP4:
    data['_bstar'] = torch.tensor(float(line1[53]+'.'+line1[54:59])*pow(10, int(line1[59:61])),dtype=torch_type)
    data['_ndot'] = torch.tensor(float(line1[33:43])/(xpdotp*1440.0),dtype=torch_type)
    data['_nddot']= torch.tensor(float(line1[44] + '.' + line1[45:50])/(xpdotp*1440.0*1440),dtype=torch_type)
    ...
    #for SGP4:
    data['_inclo']=torch.tensor(np.deg2rad(float(line2[8:16])),dtype=torch_type)
    data['_nodeo']=torch.tensor(np.deg2rad(float(line2[17:25])),dtype=torch_type)
    data['_ecco'] =torch.tensor(float('0.'+line2[26:33].replace(' ', '0')),dtype=torch_type)
    data['_argpo']=torch.tensor(np.deg2rad(float(line2[34:42])),dtype=torch_type)
    data['_mo']=torch.tensor(np.deg2rad(float(line2[43:51])),dtype=torch_type)
    data['_no_kozai'] =torch.tensor(float(line2[52:63]) / xpdotp,dtype=torch_type);
...
class TLE():
    ...
    def set_time(self, date_mjd):
        ...
        sec_whole, sec_fraction = divmod(sec, 1.0)
        d['_epochyr'] = torch.tensor(d['epoch_year'],dtype=torch_type)
        d['_jdsatepoch'] = torch.tensor(util.jday(d['epoch_year'],mon,day,hr,minute,sec)[0],dtype=torch_type);
        d['_jdsatepochF'] = torch.tensor(util.jday(d['epoch_year'],mon,day,hr,minute,sec)[1],dtype=torch_type);
        tle = TLE(d)
