...
class TextChat(App):
    ...
    async def send_message(self, event) -> None:
        ...
        if not input:
            ...
            try:
                ...
                elif now.minute > 9 and not await self.irc_client._intercept_join(event.value) and not await self.irc_client._intercept_part(event.value):
                    self.irc_client.connection.privmsg(self.tab.name, event.value)
    ...
    def add_to_tree(self, channel, user_list):
        ...
        if self.app.channel_list is not None and channel != self.app.channel_list.data['id']:
                ...
                channels_list = tree.root.add(channel, data={"id": channel})
                self.app.channel_list = channels_list
                print(f'{self.app.channel_list.data=}')
    ...
    def remove_from_tree(self):
        tree = self.app.query_one(Tree)
        tree_node = self.app.channel_list.id
        tree_thing = tree.get_node_by_id(tree_node)
        tree_thing.remove()
        print(f'{tree_thing.id=}')
