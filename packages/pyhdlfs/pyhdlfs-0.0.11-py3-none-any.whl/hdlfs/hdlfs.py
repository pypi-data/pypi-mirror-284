...
class HDLFSConnect:
    ...
    def upload(self, destination: str, data: str):
        ...
        response = upload(endpoint=self.endpoint, certificate=self.cert_file, password=self.key_file, destination=destination, data=data)
        return response
    
    def append(self, path: str, data: str, verbose=False):
        # response = append(endpoint=self.endpoint, certificate=str(self.cert_file.name), password=str(self.key_file.name), \
        #                   path=path, data=data,verbose=verbose)
        response = append(endpoint=self.endpoint, certificate=self.cert_file, password=self.key_file, path=path, data=data,verbose=verbose)
        return response
    
    def download(self, path: str, length=None, offset=0):
        # response = get(endpoint=self.endpoint, certificate=self.cert_file.name, password=self.key_file.name, path=path) 
        response = get(endpoint=self.endpoint, certificate=self.cert_file, password=self.key_file, path=path,length=length, offset=offset) 
        return response.text
    
    def list(self, path: str):
        # response = list_path(endpoint=self.endpoint, certificate=self.cert_file.name, password=self.key_file.name, path=path)
        response = list_path(endpoint=self.endpoint, certificate=self.cert_file, password=self.key_file, path=path)
    ...
    def listr(self, path: str):
        ...
        response = list_path_recursive(endpoint=self.endpoint, certificate=self.cert_file, password=self.key_file, path=path)
    ...
    def delete(self, path: str) -> list:
        ...
        if self.file_status(path)['FileStatus']['type'] == 'FILE':
            return [delete(endpoint=self.endpoint, certificate=self.cert_file, password=self.key_file, path=path)]
        else:
            response = list()
            files = self.listr(path)
            for f in files:
                response.append(self.delete(os.path.join(path,f['pathSuffix'])))
            return response 
    ...
    def rename(self, path: str, destination: str):
        ...
        response = rename(endpoint=self.endpoint, certificate=self.cert_file, password=self.key_file, path=path, destination=destination)
        return response
    ...
    def copy(self, path: str, destination: str):
        ...
        response = copy(endpoint=self.endpoint, certificate=self.cert_file, password=self.key_file, path=path, destination=destination)
        return response 
    ...
    def file_status(self, path: str):
        ...
        response = file_status(endpoint=self.endpoint, certificate=self.cert_file, password=self.key_file, path=path)
        return response     
    
    def whoami(self):
        # response = whoami(endpoint=self.endpoint, certificate=self.cert_file.name, password=self.key_file.name)
        response = whoami(endpoint=self.endpoint, certificate=self.cert_file, password=self.key_file)
        return response 
...
def main():
    ...
    parser.add_argument("command", choices=['upload','upload', 'append', 'download', 'list', 'listr','delete', 'rename', 'copy', 'exists'],
    ...
    match args.command:
        ...
        case 'append':
            ...
            response = hdlconnect.append(path=args.path, data=args.target,verbose=True)
