...
inline AvxMC2x2F ToAvx(const ScalarMC2x2F& input) {
  return AvxMC2x2F(input[0], input[1], input[2], input[3]);
}
inline ScalarMC2x2F ToScalar(AvxMC2x2F input) {
  return ScalarMC2x2F(input[0], input[1], input[2], input[3]);
}
...
BOOST_AUTO_TEST_CASE(to_avx) {
  ...
  BOOST_TEST(input[0] == ToScalar(result)[0]);
  BOOST_TEST(input[1] == ToScalar(result)[1]);
  BOOST_TEST(input[2] == ToScalar(result)[2]);
  BOOST_TEST(input[3] == ToScalar(result)[3]);
}
...
BOOST_AUTO_TEST_CASE(norm) {
  static_assert(noexcept(
      AvxMC2x2F{static_cast<const std::complex<float>*>(nullptr)}.Norm()));
  ...
  for (const AvxMC2x2F& input : inputs)
    BOOST_CHECK_EQUAL(input.Norm(), 2022428.0f);
}
...
BOOST_AUTO_TEST_CASE(non_member_norm) {
  ...
  for (const AvxMC2x2F& input : inputs)
    BOOST_CHECK_EQUAL(input.Norm(), Norm(input));
}
