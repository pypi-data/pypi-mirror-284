...
class Pattern:
    ...
    CamelToSnakeReplacements = lib.re.compile(
        r'[A-Z0-9]([0-9]{1,128}|[a-z]+|([0-9][a-z]){1,128})'
        )
    """Matches all Title Case and numeric components."""

    camelCase = lib.re.compile(r'^[a-z]+((\d)|([A-Z0-9][a-z0-9]{1,128})){0,128}$')
    """
    ...
    snake_case = lib.re.compile(r'^[a-z0-9_]{1,128}$')
    """Matches strict [lower] snake_case (i.e. python casing)."""

    NumberPattern = lib.re.compile(
        r'^(([+-]?(([0-9]{1,128}(_?[0-9]{1,128}){0,128})?(\.([0-9]{1,128}(_?[0-9]{1,128}){0,128})?)?))+(e[+-]?([0-9]{1,128}(_?[0-9]{1,128}){0,128}){1,128})?)'  # noqa
        r'(j|(([+-](([0-9]{1,128}(_?[0-9]{1,128}){0,128})?(\.([0-9]{1,128}(_?[0-9]{1,128}){0,128})?)?))+(e[+-]?([0-9]{1,128}(_?[0-9]{1,128}){0,128}){1,128})?j))?$'  # noqa
        )
    """
