...
def configure():
    ...
    print(
        "\nEnter one or more search paths, then press enter to finish. Search paths are absolute or relative "
        + "paths which will be scanned recursively by the upload agent for upload config files.\n"
    )
    ...
    new_config_file = UploadAgentConfig(
        upload_config_filename=upload_config_filename, search_paths=search_paths
    )
