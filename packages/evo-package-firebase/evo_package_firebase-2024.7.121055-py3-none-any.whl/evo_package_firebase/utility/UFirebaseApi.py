...
#<
# ---------------------------------------------------------------------------------------------------------------------------------------
    async def doSet(self, collection:str, iD: str, data:bytes, isEncrypt:True):
        try:
...
# ---------------------------------------------------------------------------------------------------------------------------------------
    async def doGet(self, collection:str, id: str ) -> bytes:
        try:
            ...
            doc = doc_ref.get()
            if not doc.exists:
                raise Exception(f"ERROR_GET|{collection}_{id}")
            ...
            if mapInfo is None:
                raise Exception(f"ERROR_GET|MAPINFO{collection}_{id}")
            ...
            if dataEncrypt is None:
                raise Exception(f"ERROR|dataEncrypt_{collection}_{id}")
            ...
            if data is None:
                raise Exception(f"ERROR|data_{collection}_{id}")
