...
_UNION_SERVERLESS_API_KEY_ENV_VARS: List[str] = ["UNION_SERVERLESS_API_KEY", "UNIONAI_SERVERLESS_API_KEY"]
...
def _get_endpoint_for_login(endpoint: Optional[str]) -> str:
    ...
    if _unionmeta_byoc_is_installed():
        # byoc
        if endpoint is None:
            msg = "--endpoint must be passed in for login"
        ...
        if endpoint is not None:
            msg = "--endpoint should not be passed in for login"
            raise click.ClickException(msg)
...
_get_union_serverless_api_env_var = partial(_get_env_var, env_vars=_UNION_SERVERLESS_API_KEY_ENV_VARS)
...
def _decode_unionai_serverless_api_key(serverless_api_value: str) -> Config:
    ...
    try:
        ...
        raise ValueError("Unable to read UNION_SERVERLESS_API_KEY") from e
...
def _get_config_obj(config: Optional[str] = None, default_to_unionai_semantics: bool = False) -> Config:
    ...
    if config is not None:
        ...
        if (serverless_api_value := _get_union_serverless_api_env_var()) is not None and serverless_api_value != "":
