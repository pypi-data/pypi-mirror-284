    ...
    if action_type == "tiktokcircle" or action_type == "tiktokpuzzle" or action_type == "tiktok3D":
        try:
            for i in range(0, number_captcha_attempts):
                is_exist_capctha_whirl = driver.find_elements("xpath", '//div[contains(@class,"captcha_verify_container")]/div/img[1][contains(@style,"transform: translate(-50%, -50%) rotate")]')
                ...
                if len(is_exist_capctha_whirl) < 1 and len(is_exist_capctha_slide) < 1 and len(is_exist_3d_capctha) < 1:
                    ...
                    get_refresh_buttton = driver.find_elements("xpath", '//a[contains(@class,"refresh")]/span[contains(@class,"refresh")][text()]')
                    if len(get_refresh_buttton) > 0:
                        update_captcha_img = driver.find_element(By.XPATH, '//a[contains(@class,"refresh")]/span[contains(@class,"refresh")][text()]')
                    else :
                    ...
                    if len(is_exist_capctha_whirl) > 0:
                        get_slider_square = driver.find_element(By.XPATH, '//div[contains(@class,"slidebar")]')
                        img_width = get_slider_square.value_of_css_property("width")
                        img_width = int(img_width.replace('px', ''))
                        get_slider_captcha = driver.find_element(By.XPATH, '//div[contains(@class,"secsdk-captcha-drag-icon")]//*[name()="svg"]')
                        ...
                        full_img_url = driver.find_element(By.XPATH, '//div[contains(@class,"captcha_verify_container")]/div/img[1][contains(@style,"transform: translate(-50%, -50%) rotate")]').get_attribute("src")
                    ...
                    if len(is_exist_capctha_slide) > 0:
                        ...
                        img_width = get_slider_square.value_of_css_property("width")
                        img_width = int(img_width.replace('px', ''))
                        get_slider_captcha = driver.find_element(By.XPATH, '//div[contains(@class,"captcha_verify_img")]/img[contains(@class,"captcha_verify_img_slide")]/following::div/following::div//div[contains(@class,"captcha-drag-icon")]')
                        get_coordinates_slider_captcha = get_slider_captcha.location
                        slider_x_coordinate = int(get_coordinates_slider_captcha['x'])
                        slider_y_coordinate = int(get_coordinates_slider_captcha['y'])
