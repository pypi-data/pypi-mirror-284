...
class IPProxyPool(ClientSession):
    def __init__(
        self,
        target: str,
    ...
    def request(self, method: str, url: str, **kwargs) -> ClientResponse:
        if len(self.endpoints) == 0:
            raise RuntimeError("To send requests using the IPProxyPool class, "
                               "first call [your IPProxyPool instance].start() "
                               "or use async with IPProxyPool(...) as session:")
