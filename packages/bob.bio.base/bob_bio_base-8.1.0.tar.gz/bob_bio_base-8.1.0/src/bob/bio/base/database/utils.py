...
def search_in_archive_and_open(
    ...
    if ".tar" in archive_path.suffixes:
        ...
        for member in tar_arch:
            ...
            logger.debug(
                "No file matching '%s' were found in '%s'.",
                pattern,
                archive_path,
            )
            return None
...
def extract_archive(
    ...
    if ".tar" in archive_path.suffixes:
        ...
        if sub_dir is not None:
            warnings.warn(
                f"Ignored sub directory ({sub_dir}). Not supported for `.bz2` "
                "files.",
                RuntimeWarning,
            )
...
def search_and_open(
    ...
    **kwargs,
    ...
    If ``base_dir`` points to an archive, the pattern will be searched inside
    that archive.
...
def _infer_filename_from_urls(urls=Union[list[str], str]) -> str:
    ...
    if not all(n == names[0] for n in names):
        raise ValueError(
            "Cannot infer file name when urls point to different files "
            f"({names=})."
        )
...
def download_file(
    ...
    extract: bool = False,
    makedirs: bool = True,
    checksum_mismatch_download_attempts: int = 2,
) -> Path:
    ...
    ----------
    urls
        The remote location of the server. If multiple addresses are given, we
        will try to download from them in order until one succeeds.
    destination_directory
        A path to a local directory where the file will be saved. If omitted,
        the file will be saved in the folder pointed by the ``bob_data_dir`` key
        in the user configuration.
    destination_sub_directory
        An additional layer added to the destination directory (useful when
        using ``destination_directory=None``).
    destination_filename
        The final name of the local file. If omitted, the file will keep the
    ...
    checksum_mismatch_download_attempts
        Number of download attempts when the checksum does not match after
        downloading, must be 1 or more.

    Returns
    -------
    The path to the new local file (or the parent directory if ``extract`` is
    True).
    ...
    ValueError
        - When ``destination_filename`` is omitted and URLs point to files with
            different names.
        - When the checksum of the file does not correspond to the provided
            ``checksum``.
    """
    ...
    if checksum_mismatch_download_attempts < 1:
        logger.warning(
            "'Checksum_mismatch_download_attempts' must be greater than 0 "
            "(got %d). Setting it to 1.",
            checksum_mismatch_download_attempts,
        )
        checksum_mismatch_download_attempts = 1
    ...
    if needs_download:
        for current_download_try in range(checksum_mismatch_download_attempts):
            if isinstance(urls, str):
                urls = [urls]

            for tries, url in enumerate(urls):
                logger.debug("Retrieving file from '%s'.", url)
                try:
                    response = requests.get(url=url, timeout=10)
                except requests.exceptions.ConnectionError as e:
                    if tries < len(urls) - 1:
                        logger.info(
                            "Could not connect to %s. Trying other URLs.",
                            url,
                        )
                    logger.debug(e)
                    continue
                ...
                if response.ok:
                    logger.debug("Got file from %s.", url)
                    break
                elif tries < len(urls) - 1:
                    logger.info(
                        "Failed to get file from %s, trying other URLs.", url
                    )
                    logger.debug("requests.response was:\n%s", response)
            else:
                raise RuntimeError(
                    "Could not retrieve file from any of the provided URLs! "
                    f"({urls=})"
                )
