...
:LastEditTime: 2024-07-15 17:50:29
...
class OrderGoodsModelEx(OrderGoodsModel):
    ...
    def get_refund_goods_list(self,order_id,goods_list = []):
        ...
        sql = f"SELECT a.goods_id,a.sku_id,a.sku_name,a.goods_picture,a.goods_code,a.goods_name,a.buy_count,a.real_pay_price,IF(c.status IS NULL,0,1) AS refund_status FROM order_goods_tb a LEFT JOIN order_refund_goods_tb b ON a.order_id = b.order_id AND a.goods_id=b.goods_id AND a.sku_id = b.sku_id LEFT JOIN order_refund_tb c ON b.refund_order_id = c.refund_order_id AND c.`status`!=4  WHERE {condition}"
