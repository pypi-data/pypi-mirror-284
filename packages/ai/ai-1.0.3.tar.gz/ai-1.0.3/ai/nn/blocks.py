...
class Resnet(nn.Module):
    def __init__(self, dim, bias=False):
        ...
        conv += [
            ...
            nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=0, bias=bias),
...
class SE(nn.Module):
    def __init__(self, dim):
        ...
        self.fc1 = nn.Linear(dim, round(dim / 16), bias=False)
        self.fc2 = nn.Linear(round(dim / 16), dim, bias=False)
...
class ResnetSoftAdaLIN(nn.Module):
    def __init__(self, dim, bias=False):
        super(ResnetSoftAdaLIN, self).__init__()
        self.pad1 = nn.ReflectionPad2d(1)
        self.conv1 = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=0, bias=bias)
        self.norm1 = SoftAdaLIN(dim)
        self.relu1 = nn.ReLU(True)

        self.pad2 = nn.ReflectionPad2d(1)
        self.conv2 = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=0, bias=bias)
...
class ResnetAdaLIN(nn.Module):
    def __init__(self, dim, bias=False):
        super(ResnetAdaLIN, self).__init__()
        self.pad1 = nn.ReflectionPad2d(1)
        self.conv1 = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=0, bias=bias)
