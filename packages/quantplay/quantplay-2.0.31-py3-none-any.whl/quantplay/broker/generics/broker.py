...
class Broker:
    ...
    def validate_exchange(self, exchange: ExchangeType) -> None:
        if exchange not in ["NSE", "NFO", "BFO", "BSE"]:
            raise InvalidArgumentException(f"exchange {exchange} not supported")

    def validate_existance(self, input, message: str):
        if not input:
    ...
    def underlying_config(self, underlying_symbol: str, expiry=None):
    ...
    def symbol_max_lots(self, exchange, symbol: str):
        try:
    ...
    def modify_orders_till_complete(
        self, orders_placed, sleep_time: float = 10, max_modification_count: int = 10
    ):
        modification_count = {}
