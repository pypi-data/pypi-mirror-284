...
class Noren(Broker):
    ...
    def modify_order(self, order_to_modify: ModifyOrderRequest) -> str:
        ...
        order_to_modify["order_type"] = self.get_order_type(order_to_modify["order_type"])
        ...
        try:
            logger.info(f"[MODIFYING_ORDER] {order_to_modify}")
            response = self.api.modify_order(  # type:ignore
                orderno=order_id,
                exchange=existing_details["exchange"],
                tradingsymbol=existing_details["tradingsymbol"],
                newprice_type=order_to_modify["order_type"],
                newquantity=order_to_modify["quantity"],
                newprice=order_to_modify["price"],
                newtrigger_price=order_to_modify["trigger_price"],
            )
            logger.info(
