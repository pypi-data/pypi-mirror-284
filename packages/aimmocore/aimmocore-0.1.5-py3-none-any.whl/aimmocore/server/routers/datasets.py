...
class DatasetExportRouter(HTTPEndpoint):
    ...
    async def get(self, request: Request):
        ...
        file_path, file_name = await svc.export_dataset_files(dataset_id, file_name, filter_value)
        if not file_path:
            return JSONResponse({"message": "No data to export"}, status_code=404)
        return FileResponse(file_path, filename=file_name, media_type="application/json")
...
class DatasetFileRouter(HTTPEndpoint):
    ...
    async def get(self, request: Request):
        ...
        result = await svc.get_dataset_file(dataset_id, file_id)
        if result:
...
class DatasetFileListRouter(HTTPEndpoint):
    ...
    async def get(self, request: Request):
        ...
        filter_value = parse_filter_value(request.query_params.get("filterValue", None))
        items, total_size = await svc.get_dataset_files(query, filter_value)
        return JSONResponse(
            DatasetFilePageList(
                items=[DatasetFile(**r) for r in items],
                page=query["page"],
...
class EmbeddingRouter(HTTPEndpoint):
    ...
    async def get(self, request: Request):
        ...
        results = await svc.get_dataset_embeddings(dataset_id)
