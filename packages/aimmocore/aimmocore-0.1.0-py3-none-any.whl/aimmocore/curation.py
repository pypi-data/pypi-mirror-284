...
class Curation:
    ...
    def curate_dataset(self, name: str, storage_config: StorageConfig) -> str:
        ...
        """
        task = self.event_loop.create_task(self.curate_dataset_async(name, storage_config))
    ...
    async def curate_dataset_async(self, name: str, storage_config: StorageConfig) -> str:
        ...
        dataset_id = await self.process_dataset(name, request_datas)
        await self.post_process_dataset(dataset_id)
        return dataset_id
    ...
    async def process_dataset(self, name, request_datas):
    ...
    async def post_process_dataset(self, dataset_id):
        """Handle post-processing tasks such as thumbnail generation."""
        await self.event_loop.create_task(aci.generate_thumbnail())
    ...
    def tracing_curation_task(self, dataset_id: str, tracing: bool = False) -> str:
        ...
        while retries > 0:
            json_results = self._make_request(url, retries)
        ...
        raise RuntimeError(f"Failed to complete tracing for dataset {dataset_id} after retries")

    def _valid_api_key(self):
        """_summary_"""
        headers = {"Authorization": f"Bearer {self.api_key}"}
        response = requests.get(conf.CURATION_AUTH_ENDPOINT, headers=headers, timeout=conf.REQUEST_TIMEOUT)
        if response.status_code != 200 and response.text:
            logger.error(response.text)
    ...
    def _make_request(self, url: str, retries: int = 3):
        while retries > 0:
            ...
            logger.debug(f"Maybe creating collection...[{response.status_code}]")
            retries -= 1
            time.sleep(2)
        raise RuntimeError("Failed to get a successful response")
