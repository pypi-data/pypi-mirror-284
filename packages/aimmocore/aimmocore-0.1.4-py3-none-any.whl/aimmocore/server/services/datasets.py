...
async def get_dataset_embeddings(dataset_id: str):

    pipeline = [
        {"$match": {"dataset_id": dataset_id, "updated_at": {"$exists": True}}},  # Filter by dataset_id
        {
            "$lookup": {
...
async def export_dataset_files(dataset_id: str, file_name: str, filter_value: dict):
    """Export dataset files to a CSV file."""
    query = {"dataset_id": dataset_id, "per_page": 0}
    dataset_files, _ = await get_dataset_files(query, filter_value)
...
async def get_dataset_files(query: dict, filter_value: dict):
    """_summary_
    ...
    pipeline = [
        ...
        {
            "$lookup": {
                "from": "raw_files",  # 조인 대상 컬렉션을 raw_files로 지정
                "localField": "image_id",  # datasets의 image_id 필드
                "foreignField": "id",  # raw_files의 id 필드
                "as": "raw_files_docs",  # 조인된 결과를 저장할 필드 이름
            }
        },
