...
class iBot(iAgent):
    ...
    async def _get_data(self, **context):
        ...
        if self._set_already(url, timeout=timeout, **params):
    ...
    def _is_already(self, url, timeout=600, **params):
        url += str(params)
        blueprint = hashlib.md5(url.encode("utf-8")).hexdigest()
        last = self.DONE_TASKS.get(blueprint, 0)
        return time.time() - last < timeout

    def _set_already(self, url, timeout=600, **params):
        if not self._is_already(url, timeout=timeout, **params):
            url += str(params)
            blueprint = hashlib.md5(url.encode("utf-8")).hexdigest()
