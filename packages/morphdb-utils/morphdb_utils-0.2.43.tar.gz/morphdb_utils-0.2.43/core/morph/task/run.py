...
"""
        ...
        if os.path.splitext(os.path.basename(self.filename))[1]:
            ...
            self.resource = MorphYaml.find_or_create_resource_by_alias(
                self.filename, self.project_root, self.db_manager
            )
    ...
    def _finalize_run(
        ...
        self.resource = self.resource.save_output(output, self.logger, visualize)
    ...
    def _run_python(self, cell: str) -> None:
        ...
        try:
            ...
            self._finalize_run(
                ...
                output,
                None,
                True if decorator_name == "visualize" else False,
            )
        except Exception as e:
