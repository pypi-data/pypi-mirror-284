...
def retrieve_value(lpad, launch_id, name):
    ...
    launch_q = {'launch_id': launch_id}
    launch_dct = lpad.launches.find_one(launch_q, {'action': True})
    ret_val = launch_dct['action']['update_spec'][name]
    if isinstance(ret_val, (list, tuple)):
        ret_lst = []
        for obj in ret_val:
            if isinstance(obj, dict):
                ret_lst.append(load_object(obj))
            else:
                ret_lst.append(obj)
        return ret_lst
    if isinstance(ret_val, dict):
        return load_object(ret_val)
    return ret_val
