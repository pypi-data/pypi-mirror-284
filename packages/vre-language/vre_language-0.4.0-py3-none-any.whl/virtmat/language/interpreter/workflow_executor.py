...
from virtmat.language.utilities.serializable import get_serializable, DATA_SCHEMA_VERSION
...
def get_input_name(par):
    ...
    if isinstance_m(par, ['ObjectImport']):
        inp_name = par.name
        inp_value = get_serializable(par.value)
...
def _get_fws_parallel(self, inputs, spec_rsc):
    ...
    if isinstance_m(self.parameter, ('Map', 'Filter')):
        fstr = get_fstr(lambda *x: get_serializable(pandas.concat(x)))
        ...
        fstr = get_fstr(lambda *x: [get_serializable(elem) for elem in x])
...
def workflow_model_processor(model, _):
    ...
    if not isinstance(model, str):
        ...
        if model_params['model_instance']['uuid'] is None:
            ...
            fw_p = {'spec._source_code': True, 'fw_id': True}
            ...
            if sorted(fwk['spec']['_source_code']) != sorted(meta_src):
                ...
                logger.info('updated meta node %s: %s', fwk['fw_id'], meta_src)
