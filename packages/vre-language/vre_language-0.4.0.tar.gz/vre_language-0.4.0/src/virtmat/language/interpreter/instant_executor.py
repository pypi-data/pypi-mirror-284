...
def chem_reaction_value(self):
    ...
    props = self.props.value if self.props is not None else None
...
def chem_species_value(self):
    ...
    props = self.props.value if self.props is not None else None
    return chemistry.ChemSpecies(self.name, props=props)
...
def object_to(obj):
    ...
    ioops.store_value(obj.ref.parameter.value, obj.url, obj.filename)
...
def add_io_processor(metamodel):
    """Register the i/o processor on the metamodel instance"""
