...
from virtmat.language.utilities.textx import GrammarString, GRAMMAR_LOC
...
class Session:
    ...
    def get_model_nodes(self, uuid):
        """return the list of nodes in the model"""
    ...
    def get_model_str(self, uuid, model_str=''):
        """updates the current model string with model_str"""
        if uuid is not None:
            ...
            pers_model_str = ''
        return pers_model_str + model_str
    ...
    def _expand_reuse(self, model_str):
        ...
        for obj in reuse_refs:
            ...
            info = get_nodes_info(self.lpad, wf_q, fw_q, proj)[0]
            vars_ = [v for n in info['nodes'] for t in n['spec']['_tasks'] for v in t['outputs']]
            ...
            lines = [line for n in info['nodes'] for line in n['spec']['_source_code']]
        ...
        return model_str_new + '\n' + '\n'.join(list(set(src_src_lines)))
    ...
    def _update_models_old_vars(self, vary, df_old, model_str):
        ...
        for ind, row in df_norm.iterrows():
            ...
            src_update = '\n'.join([pers_model_str, src, model_str])
            self.logger.debug('Source added to a new model: %s', src_update)
            strns.append(src_update)
    ...
    def _parse_vary(self, model_str):
        ...
        add_properties(meta)
        add_obj_processors(meta)
        meta.register_model_processor(vary_model_processor)
        full_model_str = self.get_model_str(self.uuid, model_str)
        model = meta.model_from_str(full_model_str)
        if not isinstance(model, str):
            objs = [o for o in model.statements if textx_isinstance(o, meta['Vary'])]
            len_pers_model = len(full_model_str) - len(model_str)
            new_str = str(model_str)
            for obj in reversed(objs):
                beg = getattr(obj, '_tx_position') - len_pers_model
                end = getattr(obj, '_tx_position_end') - len_pers_model
                new_str = new_str[:beg] + new_str[end:]
            return objs, new_str.replace(';', '\n').strip()
        return [], model_str
...
class SessionManager():
    ...
    def main_loop(self):
        ...
        while True:
            ...
            if not input_str.strip():
                continue
            model = self.parse_input(input_str)
            get_logger(__name__).debug('main_loop: session model: %s', model)
    ...
    def parse_input(self, input_str):
        ...
        return self.metamodel.model_from_str(input_str)
    ...
    def process_magic(self, model):
        ...
        if model.com == 'stop':
            ...
            if model.arg is None:
                ...
                if model.arg in self.session.uuids:
                    self.uuid = model.arg
                else:
                    self.session.stop_runner()
                    try:
                        self.session = Session(self.lpad, uuid=model.arg, **self.kwargs)
                    except ModelNotFoundError as err:
                        print(f'Model not found: {err}')
                        self.session.start_runner()
                    else:
                        self.uuid = model.arg
        elif model.com == 'vary':
            ...
            assert model.com in ('hist', 'history')
