...
class FunctionTask(FireTaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        try:
            params = [fw_spec[i] for i in inputs]
            ...
            raise err
        return self.get_fw_action(get_serializable(f_output))
...
class ExportDataTask(FireTaskBase):
    ...
    def run_task(self, fw_spec):
        obj_val = fw_spec[self.get('varname')]
...
class ScatterTask(FireTaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        assert len(set((len(fw_spec[i]) for i in self['split']))) == 1
        nchunks = len(self['chunk_ids'])

        dcts = [self['spec'].copy() for _ in range(nchunks)]
        for inp in self['split']:
            assert isinstance(fw_spec[inp], (pandas.Series, pandas.DataFrame))
            chunks = numpy.array_split(fw_spec[inp], nchunks)
            for dct, chunk in zip(dcts, chunks):
                dct[inp] = get_serializable(chunk)
