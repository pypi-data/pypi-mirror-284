...
class AivBot:
    ...
    def createParamIn(self,type, data, paramName= 'prompt', paramIn= []):
        ''' 2024.6
    ...
    def push(self,type,data, paramName= 'prompt'):
        ''' 2024.1
            添加输出内容
        ...
        if type.lower() == 'string':
            return self._addStringToTaskOutParam(data, paramName)

        if type.lower() == 'file':
            return self._addFileToTaskOutParam(data, paramName)
    ...
    def getParam(self, paramName= 'prompt', type= None, paramStr = 'paramIn', task = None):
        ''' 2024.6
            获取任务参数 (只能读取非'file'的参数)
            @param paramName 可以指定获取参数的名字, 如 'prompt' (必须与客户端传参匹配)
            @param type:    将要读取的参数类型信息
        ...
        for param in paramList:      
            ...
            if param['name'].lower() == paramName.lower():
                ...
                if type is not None:
                    if type.lower() == 'boolean' or type.lower() == 'bool': # 返回一个值
                        return param['data'][0]
                    elif type.lower() == 'string': # 返回一个值
                    ...
                    if isinstance(param['data'][0], (bool,int,float)):
                        ...
                        logger.warning(f'{param["name"]}参数中未识别的数据类型')
                        return None
