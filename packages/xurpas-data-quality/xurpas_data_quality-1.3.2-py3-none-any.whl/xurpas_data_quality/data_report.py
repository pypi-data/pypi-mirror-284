...
class DataReport:
    def __init__(self, file:str=None, 
        ...
        if isinstance(df, list):
            ...
            self.df = df[0] if isinstance(df[0], pd.DataFrame) else df[0].to_pandas()
            self.errors = df[1] if isinstance(df[1],list) else [df[1]]
    ...
    def _render_error_report(self, name=None):
        report = get_error_report(self.df,self.errors, name)
        return report.render()
