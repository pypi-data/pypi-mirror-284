...
class QueryParser:
    ...
    def parse_match(self, query):
        match_pattern = r"MATCH\s+\((\w+):(\w+)\s*{\s*(.+?)\s*}\)\s*-\[:(\w+)\s*{\s*(.+?)\s*}\]\s*->\s*\((\w+):(\w+)\s*{\s*(.+?)\s*}\)\s*RETURN\s+(.+)"
        ...
        if match:
            ...
            filtered_source_nodes = [node for node in source_nodes if self.match_properties(node.properties, self.parse_properties(source_props))]
            filtered_target_nodes = [node for node in target_nodes if self.match_properties(node.properties, self.parse_properties(target_props))]
            filtered_edges = [edge for edge in edges if self.match_properties(edge.properties, self.parse_properties(rel_props))]
