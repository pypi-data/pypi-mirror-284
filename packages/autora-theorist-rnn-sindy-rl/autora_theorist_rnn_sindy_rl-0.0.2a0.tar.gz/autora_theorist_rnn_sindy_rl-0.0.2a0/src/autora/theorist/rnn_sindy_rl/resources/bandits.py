...
class AgentQ:
  ...
  def q(self):
    ...
    if self._prev_choice != -1:
      q[self._prev_choice] += self._perseveration_bias
    return q
...
class AgentSindy:
  ...
  def set_models(self, sindy_models: Dict[str, ps.SINDy]):
    self._models = sindy_models
    
  def get_models(self) -> Dict[str, ps.SINDy]:
    return self._models
  ...
  def update(self, choice: int, reward: int):
    ...
    reward_update, action_update = self._update_rule(self, self._q[choice], self._h[choice], 1, int(self._prev_choice==choice), reward, 0)
