...
def test_flatten_merge_dicts():
    ...
    # expected = [[3,4,5,6,7], [6,7,8,9,10], [14,15,16]]
    # assert sample.flatten(to={"e", "d"}) == expected, f"Expected {expected}, but got {sample.flatten(to={'e', 'd'})}"
    
    # flattened = sample.flatten(to={"e", "d"}, output_type="list")

    # expected = [[3,4,5,6,7], [6,7,8,9,10], [14,15,16]]
    assert flattened == expected, f"Expected {expected}, but got {flattened}"

    flattened = sample.flatten(to={"g", "d"}, output_type="torch")
    expected = torch.tensor([[6,7,3,4], [9,10,6,7], [15,16,12,13]])
