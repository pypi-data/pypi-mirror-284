...
class Trajectory:
    ...
    def make_relative(self) -> "Trajectory":
        ...
        """
        return Trajectory(
            np.diff(self.array, axis=0), self.freq_hz, self.time_idxs[1:], self.dim_labels, self.angular_dims
        )
    ...
    def resample(self, target_hz: float) -> "Trajectory":
        if self.freq_hz is None:
            raise ValueError("Cannot resample a trajectory without a frequency")
        ...
        if target_hz < self.freq_hz:
            ...
            if len(self.array) < 4:
                raise ValueError("Cannot upsample a trajectory with bicubic interpolationwith less than 4 samples")
    ...
    def transform(self, operation: Callable[[np.ndarray], np.ndarray] | str, **kwargs) -> "Trajectory":
        ...
        try:
            ...
            raise ValueError(f"Operation {operation} failed with kwargs {kwargs}. Signature is {inspect.signature(operation)}") from e
    ...
    def make_pca(self, whiten=True) -> "Trajectory":
        ...
        return Trajectory(
            pca.fit_transform(self.array), self.freq_hz, self.time_idxs, self.dim_labels, self.angular_dims
        )
    ...
    def make_unminmax(
        self, orig_min: np.ndarray | Sample, orig_max: np.ndarray | Sample,
    ) -> "Trajectory":
