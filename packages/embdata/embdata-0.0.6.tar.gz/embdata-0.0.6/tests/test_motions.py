...
def test_bounds():
    ...
    assert xarm.pose.model_field_info("y")["bounds"] == (-0.4, 0.4)
    assert xarm.pose.model_field_info("z")["bounds"] == (-0.175, 0.4)
    assert xarm.pose.model_field_info("roll")["bounds"] == (-np.pi/2, np.pi/2)
    assert xarm.pose.model_field_info("pitch")["bounds"] == (-np.pi/2, np.pi/2)
    assert xarm.pose.model_field_info("yaw")["bounds"] == (-np.pi, np.pi)
