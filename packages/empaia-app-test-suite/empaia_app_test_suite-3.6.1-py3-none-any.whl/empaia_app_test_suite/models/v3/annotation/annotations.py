...
from pydantic import UUID4, Field, ValidationError, conlist, field_validator
...
NppViewing = conlist(Annotated[float, Field(gt=0.0)], min_length=2, max_length=2)
...
class AnnotationIdList(RestrictedBaseModel):
    ...
    low_npp_centroids: Optional[List[conlist(Annotated[int, Field(ge=0)], min_length=2, max_length=2)]] = Field(
        default=None,
...
class AnnotationViewerList(RestrictedBaseModel):
    ...
    low_npp_centroids: List[conlist(Annotated[int, Field(ge=0)], min_length=2, max_length=2)] = Field(
        examples=[[[100, 200], [2000, 5000], [987, 654]]],
        description=(
            "Centroids of all annotations with higher resolution (lower npp_created / npp_viewing values) "
            "than specified by npp_viewing in the query."
