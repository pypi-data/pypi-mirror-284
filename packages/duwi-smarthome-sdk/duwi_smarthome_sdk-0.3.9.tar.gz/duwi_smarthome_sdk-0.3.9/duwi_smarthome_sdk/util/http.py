...
async def _request(
    ...
    try:
        async with session.request(method=method, url=url, headers=headers, params=params, json=json) as response:
            _LOGGER.debug("请求头是 --------------- %s", response.headers)
            _LOGGER.debug("请求体是 --------------- %s", json)
            _LOGGER.debug("返回的状态是 --------------- %s", response.status)
            _LOGGER.info("返回的数据是 --------------- %s", await response.text())
        ...
        return "TimeoutError", 'timeout error', {'error': str(e)}
    finally:
        await session.close()
