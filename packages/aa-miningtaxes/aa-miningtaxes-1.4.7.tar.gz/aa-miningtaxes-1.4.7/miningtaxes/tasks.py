...
def notify_current_taxes_threshold(self):
    ...
    s = Stats.load()
    arr = s.get_admin_char_json()

    for row in arr:
        if row["balance"] >= MININGTAXES_PING_CURRENT_THRESHOLD:
            try:
                chartxt = (
                    row["main_name"].split(">&nbsp;&nbsp;&nbsp;")[1].strip()
                )  # tied to formatting of the admin table
                u = EveCharacter.objects.get(character_name=chartxt).userprofile.user
            except Exception:
                continue
            title = "Taxes are due!"
