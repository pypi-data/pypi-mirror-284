...
class MaskSegmentation(object):
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(bin_sizes, str):
            split_bin_sizes = bin_sizes.split(",")
            try:
                split_bin_sizes = [int(item) for item in split_bin_sizes]
                self.bin_sizes = split_bin_sizes
            except:
                error_msg = "can not conver the bin_sizes string {} to int list,the bin_sizes string should be 1,2,3. like!".format(
                    bin_sizes
                )
                self.logger.error(error_msg)
                raise ValueError(error_msg)
        elif isinstance(bin_sizes, int):
            self.bin_sizes = [bin_sizes]
        elif isinstance(bin_sizes, list):
            self.bin_sizes = bin_sizes
        else:
            error_msg = "we do not know how to parse the the bin_sizes {},we only support string,int,or list".format(
                bin_sizes
            )
            self.logger.error(error_msg)
        ...
        for bin_size in self.bin_sizes:
            if not isinstance(bin_size, int) or bin_size < 0:
                error_msg = (
                    "bin size must be positive integer,but get value:{} type:{}".format(
    ...
    def _is_transcript_omics(omics_str: str) -> bool:
        return omics_str == TRANSCRIPT_OMICS

    @staticmethod
    def _is_protein_omics(omics_str: str) -> bool:
        return omics_str == PROTEIN_OMICS
    ...
    def _single_bin_process(
        ...
        if self._is_gef_format_file(self.gene_file):
            ...
            with h5py.File(self.gene_file, "r") as gene_file_handler:
                ...
                # for large data,we should not use this !
    ...
    def saptialbin_lasso_process(self) -> None:
        ...
        for polygon_meta in polygon_metas:
            ...
            cgef_adj = CgefAdjust()
            flatten_polygons = [item.flatten().tolist() for item in polygons]
            self.logger.info("invoking the cgef adj...")
            # reshape the polygon from shape (n,2) -> (-1)
            self.logger.info("invoke gefpy to run create_Region_bgef")
            cgef_adj.create_Region_Bgef(
                self.gene_file, gef_output_file, flatten_polygons
            )
            for bin_size in self.bin_sizes:
                self.logger.info("start handle bin size {}".format(bin_size))
        ...
        if self._is_gef_format_file(self.gene_file):
            ...
            if has_cell_bin_group:
                ...
                for polygon_meta in polygon_metas:
                    ...
                    cell_adj = CgefAdjust()
                    flatten_polygons = [item.flatten().tolist() for item in polygons]
                    self.logger.info("invole gefpy to run function create_Region_Cgef")
                    cell_adj.create_Region_Cgef(
                        self.gene_file, output_file, flatten_polygons
                    )
            else:
                self.logger.info(
