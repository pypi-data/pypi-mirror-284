...
def parse_example(
    ...
    args = Arguments(f"tests/examples/include/{example}", **kwargs)
    with open(f"tests/examples/include/{example}/{name}.yml") as fp:
...
def test_include_templates() -> None:
    config = parse_example("templates", cache_directory="tests/examples/include")
...
def test_get_all_component_versions_cached(
    ...
    versions = get_all_component_versions(
        Arguments(cache_directory=str(tmp_path)),
...
def test_get_component_location(
    ...
    repo_path = tmp_path / "components" / "gitlab.com-project"
