...
from pyalbert.config import DO_ENCODE_PROMPT, HF_API_TOKEN, LLM_TABLE
...
def prompts_from_llm_table(table: list[dict]) -> dict[str, dict]:
    ...
    templates = {}
    for model in [t["model"] for t in table]:
        templates[model] = PromptConfig.from_hf(model).set_defaults()
