...
from moveread.dfy import Round, Pairings, Game, Tournament, queries
...
def current_round(
  ...
  # if there's no next round or next round (which hasn't yet started) is paired, nothing to do
  if prev+1 >= len(rounds) or round_paired(session, **cp.roundId(tournId, group, rounds[prev+1].name)):
    return Right(None)
