...
def get_logs(vpu_log_dir: str, local_log_cache: str, vpu_sn: str, ssh: SSHClient, scp: SCPClient, vpu_name: str = "") -> None:
    ...
    if SSH_path_exists(ssh, vpu_log_dir):
        ...
        logger.info(
            f"No logs collected from {
                vpu_log_dir}, directory does not yet exist"
        )
...
class Manager:
    ...
    def connect(self, reboot_pause=150) -> bool:
        ...
        if USING_IFM3DPY:
            if len(self.config.possible_initial_ip_addresses_to_try) == 0:
                ...
                if not self._connected:
                    ...
                    logger.info(f"Waiting {reboot_pause}s for VPU to reboot.")
                    heartbeat_rate = 2
    ...
    def append_deployment_timestamp(self, deployment_timestamp_path: str = "~/share/Deployments") -> None:
        cmd = f'echo "{
            Path(self._log_file_path).name[:-4]}" >> {deployment_timestamp_path}'
        logger.info(cmd)
        _stdin, _stdout, _stderr = self._ssh.exec_command(cmd)
        logger.info(
            f">>> {_stderr.read().decode().strip()} {_stdout.read().decode().strip()}")
    ...
    def setup_docker_volume(self, path_for_volume_to_mount: str, volume_name: str) -> None:
        ...
        cmd = f'docker volume create --driver local -o o=bind -o type=none -o device="{
            path_for_volume_to_mount}" {volume_name}'
        _stdin, _stdout, _stderr = self._ssh.exec_command(cmd)
        logger.info(cmd)
    ...
    def import_docker_image(self, image_to_import: str) -> None:
        ...
        cmd = f"cat {
            image_to_import}| docker import - {docker_image_fname[:-4]}"
        logger.info(cmd)
        _stdin, _stdout, _stderr = self._ssh.exec_command(cmd)
    ...
    def pull_docker_image_from_registry(self, docker_registry_host: str, docker_registry_port: int, docker_tag: str, update_tag_on_VPU_to: str = "") -> None:
        ...
        cmd = f"docker pull {docker_registry_host}:{
            docker_registry_port}/{docker_tag}"
        logger.info(cmd)
        _stdin, _stdout, _stderr = self._ssh.exec_command(cmd)
        logger.info(">>>"+_stdout.read().decode().strip() +
                    _stderr.read().decode().strip())
        ...
        if update_tag_on_VPU_to:
            cmd = f"docker tag {docker_registry_host}:{
                docker_registry_port}/{docker_tag} {update_tag_on_VPU_to}"
            logger.info(cmd)
            _stdin, _stdout, _stderr = self._ssh.exec_command(cmd)
            logger.info(">>>"+_stdout.read().decode().strip() +
                        _stderr.read().decode().strip())
    ...
    def initialize_container(self, service: DockerComposeServiceInstance, pipe_duration: float = 0, stop_upon_exit: bool = False, autostart_enabled=True, trigger_to_close_container: str = "") -> None:
        ...
        docker_compose_dir = f"/usr/share/oem/docker/compose/{
            service.service_name}"

        detach_arg = "-d" if pipe_duration == 0 or non_standard_docker_logger_used else ""
        attachment_addendum = f"&& docker attach {
            service.container_name}" if non_standard_docker_logger_used and pipe_duration != 0 else ""
        if autostart_enabled:
            cmd = f"cd {docker_compose_dir} && docker-compose up {
                detach_arg} --remove-orphans {service.service_name} {attachment_addendum}"
        else:
            cmd = f"docker-compose -f {service.docker_compose_dst_on_vpu} up {
                detach_arg} --remove-orphans {service.service_name} {attachment_addendum}"
