...
import db_string_utilities as dsu
...
class DbUtilities:
    ...
    def drop_schema(self, schema_name):
        drop_sql = "DROP SCHEMA IF EXISTS " + dsu.escape_id(schema_name) + " CASCADE;"
        self.execute_modify(drop_sql)

    def drop_table(self, schema_name, table_name):
        drop_sql = "DROP TABLE IF EXISTS " + dsu.escape_id(schema_name) + "." + dsu.escape_id(table_name) + ";"
        self.execute_modify(drop_sql)

    def drop_view(self, schema_name, view_name):
        drop_sql = "DROP VIEW IF EXISTS " + dsu.escape_id(schema_name) + "." + dsu.escape_id(view_name) + ";"
        self.execute_modify(drop_sql)

    def create_schema(self, schema_name, overwrite_existing=False):
        if overwrite_existing:
            self.drop_schema(schema_name)
        create_sql = "CREATE SCHEMA IF NOT EXISTS " + dsu.escape_id(schema_name) + ";"
    ...
    def build_table_insert_statement(self, schema_name, table_name, table_data_row):
        ...
        if (schema_name is not None):
            insert_sql += dsu.escape_id(schema_name) + "."
        insert_sql += dsu.escape_id(table_name)

        columns_list = []
        for col_name in table_data_row:
            columns_list.append(dsu.escape_id(col_name))
    ...
    def create_table(self, schema_name, table_name, table_definition, overwite_existing=False, create_sequence_col=True, primary_key_col=SEQUENCE_COL_NAME):
        ...
        create_sql = "CREATE TABLE IF NOT EXISTS " + dsu.escape_id(schema_name) + "." + dsu.escape_id(table_name) + " ("
        # create a bigserial to track the sequence
        if create_sequence_col:
            create_sql += dsu.escape_id(SEQUENCE_COL_NAME) + " bigserial,"
        ...
        for col_name in table_definition:
            ...
            create_sql += dsu.escape_id(col_name) + " " + col_type + ","
    ...
    def create_view(self, schema_name, view_name, view_sql, overwite_existing=False):
        ...
        create_sql = "CREATE OR REPLACE VIEW " + dsu.escape_id(schema_name) + "." + dsu.escape_id(view_name) + " AS "
    ...
    def create_index_on_table(self, schema_name, table_name, column_name):
        ...
        index_name = dsu.escape_id(column_name + "_idx")
        create_sql = "CREATE INDEX " + index_name + " ON " + dsu.escape_id(schema_name) + "." + dsu.escape_id(table_name) + " (" + dsu.escape_id(column_name) + ");"
        self.execute_modify(create_sql)
    ...
    def add_primary_key_to_table(self, schema_name, table_name, column_name):
        ...
        alter_sql = SQL = "ALTER TABLE " + dsu.escape_id(schema_name) + "." + dsu.escape_id(table_name) + " ADD PRIMARY KEY (" + dsu.escape_id(column_name) + ");"
    ...
    def get_sorted_table_data(self, schema_name, table_name, sort_columns, sort_ascendings, where_clause=None, limit=None, offset=None, as_dataframe=False):
        ...
        if len(sort_columns) > 0 and len(sort_columns) == len(sort_ascendings):
            ...
            for i in range(len(sort_columns)):
                sort_item = dsu.escape_id(sort_columns[i]) + " "
        ...
        return self.execute(
            "SELECT * FROM " + dsu.escape_id(schema_name) + "." + dsu.escape_id(table_name) + " " 
    ...
    def get_table_row_count(self, schema_name, table_name, where_clause=None):
        ...
        return self.flatten_data((self.execute(
            "SELECT COUNT(1) AS row_count FROM " + dsu.escape_id(schema_name) + "." + dsu.escape_id(table_name) + " "
            + where_sql + ";",
    ...
    def does_relation_exist(self, schema_name, table_name):
        relation = dsu.escape_id(schema_name) + '.' + dsu.escape_id(table_name)
