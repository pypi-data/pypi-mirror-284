...
class Kinematic:
    def __init__(self,
        ...
        self._accel = initial_acceleration if initial_acceleration is not None else Acceleration(initial_m_s2_vec=(0, 0))
    ...
    def update(self,
        ...
        # update values
        self._transform.Translation.update(delta_vector=self._velocity.accrue(delta_time_ms, accel=self._accel.CurrentAccel_M_S))
        ...
        logger.debug(f"Mover [{self.Id}] updated Transform: {self._transform}, Velo: {self._velocity}, Accel: {self._accel} [{delta_time_ms}]")
...
class GoalSeeker:
    ...
    def _update_history(self):
        ...
        self._history.append((self.Position.Vector, self.Velocity.CurrentVelocity_M_S, self.Acceleration.CurrentAccel_M_S))

        if len(self._history) > self._history_len:
            self._history.pop(0)


    def _calculate_critical_stopping_v(self,
                         dist_to_goal: float,
    ...
    def _accel_to_stop(self,
                       v,
                       stopping_distance):
        return -v ** 2 / (2 * stopping_distance)
