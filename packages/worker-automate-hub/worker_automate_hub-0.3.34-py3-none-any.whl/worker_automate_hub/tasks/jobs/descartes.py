...
async def descartes(task):
    ...
    try:
        ...
        try:
            ...
            field_modelo_faturamento = pyautogui.locateOnScreen(ASSETS_BASE_PATH + "field_modelo_faturamento.png", confidence=0.9, grayscale=True )
        except:
            field_modelo_faturamento = None

        if field_modelo_faturamento is not None:
            pyautogui.click(field_modelo_faturamento.left + 100, field_modelo_faturamento.top)
            time.sleep(3)
