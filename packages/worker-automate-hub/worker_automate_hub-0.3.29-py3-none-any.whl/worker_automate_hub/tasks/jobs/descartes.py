...
async def descartes(task):
    ...
    try:
        ...
        # app = Application(backend='win32').connect(title="Gestão Empresarial (32 bits)")
        # try:
        #     main_window = app.window(title='^Gestão Empresarial .*')
        ...
        if field_validade is not None:
            ...
            pyautogui.write(f'{datetime.now().strftime("%d/%m/%Y")}')
            pyautogui.press('tab')
    ...
    # #TODO Pode ocorrer tela de 'Selecionar Endereço' - nao implementado pois nao tive um caso

        #Aviso "Deseja alterar a condição de pagamento informada no cadastro do cliente?"
        try:
            payment_condition_warning = pyautogui.locateOnScreen(ASSETS_BASE_PATH + "warning_change_payment_condition.png", confidence=0.9, grayscale=True)
        except:
        ...
        for item in itens:
            ...
            pyautogui.hotkey('tab')
            time.sleep(3)
            console.print(f"\nDigitou almoxarifado {task['configEntrada']['filial']+"99"}")            
            ...
            pyautogui.hotkey('tab')
            time.sleep(5)
            console.print(f"\nDigitou codigo do produto {item['codigoProduto']}")            
            ...
            if warning_price is not None:
                ...
                api_simplifica(task['configEntrada']['urlRetorno'], "ERRO", observacao, task['configEntrada']['uuidSimplifica'], None)
        ...
        time.sleep(5)
        #Seleciona o modelo da nota (em teoria em prod nao precisa mas esta aqui por precaução)
        try:
            field_select_modelo = pyautogui.locateOnScreen(ASSETS_BASE_PATH + "field_select_modelo.png", confidence=0.85, grayscale=True)
            pyautogui.click(pyautogui.center(field_select_modelo))
            time.sleep(5)
            field_to_select_modelo = pyautogui.locateOnScreen(ASSETS_BASE_PATH + "field_to_select_modelo.png", confidence=0.9, grayscale=True)
            pyautogui.click(pyautogui.center(field_to_select_modelo))
        except:
            observacao = {
            ...
            api_simplifica(task['configEntrada']['urlRetorno'], "ERRO", observacao, task['configEntrada']['uuidSimplifica'], None)
            return {"sucesso": False, "retorno": "Erro ao selecionar o modelo"}
        ...
        api_simplifica(task['configEntrada']['urlRetorno'], "SUCESSO", observacao, task['configEntrada']['uuidSimplifica'], nota_fiscal[0])
        return {"sucesso": True, "retorno": observacao}
