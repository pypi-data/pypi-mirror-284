...
class SmartNqClient:
    ...
    def GetFutureData(self, contract_symbol, start_datetime, end_datetime, timeframe, indicators):
        print(f"Indicators received: {indicators}")  # Debug print

        for indicator, indicator_timeframe in indicators:
            print(f"Checking {indicator.name} with timeframe {indicator_timeframe.value} against main timeframe {timeframe.value}")
            if not is_valid_timeframe(indicator_timeframe, timeframe):
                print(f"Invalid: {indicator.name} with timeframe {indicator_timeframe.value}")
                raise ValueError(f"Indicator {indicator.name} timeframe {indicator_timeframe.value} must be less granular than the main requested timeframe {timeframe.value}.")
