...
def device_status(stdscr, idx, flag):
    ...
    if flag:
        ...
        try:
            ping_sg = f"{ping3.ping('api-sg-gl.mecordai.com', timeout=1)*1000:.1f}ms"
        except:
            ping_sg = '--'
        try:
            ping_test = f"{ping3.ping('mecord-beta.2tianxin.com', timeout=1)*1000:.1f}ms"
        except:
            ping_test = '--'
        try:
            ...
            if len(gpu_list) > 0:
                ...
                gpu_load = 0
            real_stdsrc(idx, 0, scr_str(f"Device Usage    CPU : {cpu_load}%    Memory : {mem_load}%    GPU : {gpu_load}%    ↑{up_load}KB/s    ↓{down_load}KB/s    sg: {ping_sg}    test: {ping_test}".ljust(ll*3-4)))
        except:
            real_stdsrc(idx, 0, scr_str(f"Device Usage    CPU : {cpu_load}%    Memory : {mem_load}%    GPU : 获取失败"))
    else:
        try:
            real_stdsrc(idx, 0, scr_str(f"Device Usage    CPU : {mdict['main']['cpu']}%    Memory : {mdict['main']['mem']}%    GPU : {mdict['main']['gpu']}%    ↑{mdict['main']['up']}    ↓{mdict['main']['down']}    sg: {mdict['main']['sg']}    test: {mdict['main']['test']}".ljust(ll*3-4)))
        except:
            ...
            async def get_usage(pid):
                try:
                    if pid == 'main':
                        ...
                        up_load = f"{(_net_up - net_up) / 1024:.2f}KB/s"
                        down_load = f"{(_net_down - net_down) / 1024:.2f}KB/s"
                        try:
                            gpu_list = GPUtil.getGPUs()
                            if len(gpu_list) > 0:
