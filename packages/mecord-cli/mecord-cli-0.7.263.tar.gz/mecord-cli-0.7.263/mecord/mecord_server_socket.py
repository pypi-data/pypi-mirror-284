...
class MecordTaskExecutorThread(Thread):
        ...
        while self.is_running or not self.task_queue.empty():
            try:
                ...
                try:
                    ...
                    is_ok, msg, result, is_counter = task.runTask(data, timeout, country, widget_name, _appendTask, gettask, ext, self.next_task)
                    if is_ok == False:
    ...
    def monitor(self):
        if self.idlePower() == 0:
            self.cpu.append(psutil.cpu_percent(interval=5))
            if len(self.cpu) >= 10:
                for cpu in self.cpu:
                    if cpu <= 90:
                        self.add_flag = True
...
class MecordLongConnectThread(Thread):
    ...
    def on_pong(self, ws, message):
        ...
        if self.ping_time:
            self.latency = f"ping {(time.time() - self.ping_time) * 1000:.2f} ms"  # 转换为毫秒
    ...
    def send_ping(self):
        ...
        self.ws.send("ping", websocket.ABNF.OPCODE_PING)
...
class MecordShortConnectThread(Thread):
    ...
    def run(self):
        ...
        what_time = 3 if self.service_country == "sg" else 5
        while (self.is_running):
            limit = self.executor.idlePower()
            if limit >= 1:
                lock.acquire()
                if os.path.exists(refreshMessage):
                    os.remove(refreshMessage)
                ...
                print(f'{self.name} === wait {limit} tasks, widgets: {self.widgets}')
                try:
                    datas, timeout = xy_pb.GetTask(self.service_country, self.widgets, limit=limit, keep_alive=True)
                    for it in datas:
                        self.executor.appendTask(it, timeout, self.service_country, self.taskCallback, self.next_task, self.mtype)
                except Exception as ex:
    ...
    def next_task(self):
        datas, timeout = xy_pb.GetTask(self.service_country, self.widgets, keep_alive=True)
        for it in datas:
            self.executor.appendTask(it, timeout, self.service_country, self.taskCallback, self.next_task, self.mtype)
