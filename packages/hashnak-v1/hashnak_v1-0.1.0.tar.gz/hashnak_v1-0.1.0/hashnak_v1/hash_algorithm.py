...
class HashNak:
    ...
    # We'll add more methods here as we implement the algorithm
    ...
    def generate_subhashnak(self, dualities):
        subhashnaks = []
        ...
        for duality in dualities:
            ...
            for position, values in duality.items():
                ...
                subhashnak.append(int(area))  # Convert to integer to remove scientific notation
            subhashnaks.append(subhashnak)

        return subhashnaks
    
    def hash_nak(self, input_string):
        swapped = self.swap_input(input_string)
        repaired = self.repair_positions(swapped)
        dualities = self.generate_dualities(repaired)
        subhashnaks = self.generate_subhashnak(dualities)
        
        # Combine all SUBHNs into one string
        hash_string = ''
        for subhashnak in subhashnaks:
            hash_string += ''.join(str(value) for value in subhashnak)
