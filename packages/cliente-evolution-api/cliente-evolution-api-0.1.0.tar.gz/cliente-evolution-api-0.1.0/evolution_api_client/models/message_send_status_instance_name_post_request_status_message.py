...
class MessageSendStatusInstanceNamePostRequestStatusMessage(BaseModel):
    """
    MessageSendStatusInstanceNamePostRequestStatusMessage
    """ # noqa: E501
    type: StrictStr = Field(description="Type of the status message.")
    content: Optional[StrictStr] = Field(default=None, description="The content of the status message.")
    background_color: Optional[StrictStr] = Field(default=None, description="The background color of the status message.", alias="backgroundColor")
    font: Optional[StrictInt] = Field(default=None, description="The font of the status message.")
    all_contacts: Optional[StrictBool] = Field(default=None, description="Indicates whether to send the status message to all contacts.", alias="allContacts")
    status_jid_list: Optional[List[StrictStr]] = Field(default=None, alias="statusJidList")
    __properties: ClassVar[List[str]] = ["type", "content", "backgroundColor", "font", "allContacts", "statusJidList"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['text', 'image', 'video', 'audio']):
            raise ValueError("must be one of enum values ('text', 'image', 'video', 'audio')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of MessageSendStatusInstanceNamePostRequestStatusMessage from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of MessageSendStatusInstanceNamePostRequestStatusMessage from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "font": obj.get("font"),
            "allContacts": obj.get("allContacts"),
            "statusJidList": obj.get("statusJidList")
        })
        return _obj
