...
@app.callback()
def cli_setup(disable_usage_statistics: bool = typer.Option(False, help="If true no usage statistics will be collected."), log_level: Optional[str] = None):
...
async def promote(
    ...
    launchflow_api_key: Optional[str] = typer.Option(None, help=API_KEY_HELP),
    promote_local: bool = typer.Option(False, help="Promote the service locally. If true this will move the docker image to the new environment locally instead of on Cloud Build or Code Build."),
    scan_directory: str = typer.Option(".", help=SCAN_DIRECTORY_HELP),
    verbose: bool = typer.Option(
        False,
        "--verbose",
...
async def run(
    ...
    if local_resources or remote_resources:
        ...
        await create_resources(*resources, environment_name=environment, prompt=True)
