...
class DestroyServicePlan:
    ...
    @property
    def ref(self) -> str:
        service_cls = SERVICE_PRODUCTS_TO_SERVICES[self.existing_service.product]
...
class DestroyResourcePlan:
    ...
    @property
    def ref(self) -> str:
        resource_cls = RESOURCE_PRODUCTS_TO_RESOURCES[self.resource.product]
    ...
    async with lock as lock_info:
        ...
        try:
            ...
            if verbose:
                _dump_verbose_logs(logs_file, f"Destroy {plan.ref} logs")
            progress.console.print(
                f"[red]✗[/red] {plan.ref} failed to delete"
            )
            await plan.resource_manager.save_resource(plan.resource, lock_info.lock_id)
            success = False
...
async def _destroy_service(lock: Lock, plan: DestroyServicePlan, progress: Progress):
    async with lock as lock_info:
        ...
        try:
            ...
            progress.console.print(
                f"[green]✓[/green] {plan.ref} successfully deleted"
            )
            with open(logs_file, "a") as f:
    ...
    if nodes:
        ...
        for node in nodes:
            if isinstance(node, Resource):
                if isinstance(node, DockerResource) and not docker_service_available():
                    raise exceptions.MissingDockerDependency(
                        "Docker is required to destroy local resources."
                    )
