...
async def create_environment(
    ...
    async with await manager.lock_environment(
        ...
        if cloud_provider == CloudProvider.GCP:
            ...
            if existing_environment is not None and existing_environment.gcp_config is not None:
                vpc_connection_managed = existing_environment.gcp_config.vpc_connection_managed
        ...
        if env.status == EnvironmentStatus.READY:
            ...
            rich.print("\n[yellow]NOTE: Rerunning this command with the same environment name will continue creation from where it left off.[/yellow]\n")
            return None
