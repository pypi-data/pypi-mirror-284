...
from launchflow.workflows.deploy_aws_service.deploy_aws_service_utils import (
...
class ReleaseServicePlan(ServicePlan):
    ...
    async def execute_plan(
        ...
        if self.docker_step == "build":
            ...
            docker_result_type = PromoteDockerImageResult
...
async def deploy(
    *nodes: Tuple[Node],
    ...
    flow_result = FlowResult(
        success=success,
        plan_results=create_results + deploy_results,
    ...
    table = Table(show_header=True, show_edge=False, show_lines=False, box=None)
    table.add_column("Service", justify="left", no_wrap=True)
    table.add_column("Host name", style="light_cyan3")
    table.add_column("Type", style="yellow3")
    table.add_column("IP Address", style="pale_green3")
    for result in create_results:
    ...
    for result in create_results:
        if isinstance(result, CreateResourceResult) and is_secret_resource(
    ...
    if secrets:
        ...
        for secret in secrets:
            ...
            console.print(
                f"    $ lf secrets set --environment={environment_name} {secret.name} some-value"
            )
            console.print()
        ...
        try:
            ...
            if isinstance(self.service, GCPService):
                docker_image, logs_file_or_link = await promote_gcp_service_image(
                    ...
                    from_gcp_environment_config=self.from_gcp_environment_config,
                    to_gcp_environment_config=self.to_gcp_environment_config,
                    deployment_id=self.deployment_id,
                ...
                docker_image, logs_file_or_link = await promote_aws_service_image(
                    self.service,
                    from_service_state=self.from_service_state,
                    from_aws_environment_config=self.from_aws_environment_config,
                    to_aws_environment_config=self.to_aws_environment_config,
                    deployment_id=self.deployment_id,
                    promote_local=self.promote_local
                )
                return PromoteDockerImageResult(
    ...
    async def execute_plan(
        ...
        async with self._lock as lock_info:
            ...
            try:
                ...
                # We do this for type hinting purposes
                promote_docker_image_result: PromoteDockerImageResult = (
                ...
                new_to_service_state.docker_image = (
                    promote_docker_image_result.docker_image
                )
                new_to_service_state.service_url = release_service_result.service_url
                if deploy_successful:
...
async def plan_promote_service(
    service: Service,
    from_environment: EnvironmentState,
    to_environment: EnvironmentState,
    ...
    if service.product.cloud_provider() == CloudProvider.GCP:
        if to_environment.gcp_config is None:
            return FailedToPlan(
                service=service,
                error_message="CloudProviderMismatch: Cannot use a GCP Service in an AWS Environment.",
            )
    elif service.product.cloud_provider() == CloudProvider.AWS:
        if to_environment.aws_config is None:
            return FailedToPlan(
                service=service,
                error_message="CloudProviderMismatch: Cannot use an AWS Service in a GCP Environment.",
            )
    ...
    release_plan = ReleaseServicePlan(
        ...
        gcp_environment_config=to_environment.gcp_config,
        aws_environment_config=to_environment.aws_config,
        deployment_id=deployment_id,
        docker_step="promote",
    )
...
async def promote(
    *nodes: Tuple[Node],
    ...
    from_environment_manager = EnvironmentManager(
        project_name=launchflow.project,
        environment_name=from_environment_name,
        backend=config.launchflow_yaml.backend,
    )
    to_environment_manager = EnvironmentManager(
        project_name=launchflow.project,
        environment_name=to_environment_name,
        backend=config.launchflow_yaml.backend,
    )

    from_environment, to_environment = await asyncio.gather(
    ...
    selected_promote_plans: Union[None, List[PromoteServicePlan]] = await select_plans(
    ...
    async with lock_plans(
        *selected_promote_plans, environment_manager=to_environment_manager
        ...
        with Live(Padding(tree, (1, 0, 1, 0)), console=console, refresh_per_second=8):
            promote_results: List[PromoteServiceResult] = await execute_plans(
                selected_promote_plans, tree
            )

    # TODO: Move the print logic below to a shared utility that takes in a FlowResult
    success = all(result.success for result in promote_results)
    flow_result = FlowResult(
        success=success, plan_results=promote_results, failed_plans=failed_plans
    )
    ...
    for result in promote_results:
        ...
        if result.success:
            table.add_row(
                ...
                result.promote_image_result.logs_file_or_link,
            )
        else:
            table.add_row(
