...
class DollarParser(DetectionParser):
    ...
    Note:
        ...
        While this is not a big problem and can still be used to generate statistics like PR- or MRFPPI-curves,
        you might want to normalize these values. You could normalize the results between 0-1 by using the following code,
        but make sure to check the detector that was used to use the correct normalization if necessary.
    ...
    def serialize(self, row):
        return (
            f'{row.image},{round(row.x_top_left)},{round(row.y_top_left)},{round(row.width)},{round(row.height)},'
            f'{round(row.confidence, self.precision)}'
        )
