...
class DollarParser(AnnotationParser):
    ...
    def serialize(self, df):
        ...
        for row in df.itertuples():
            ...
            lost = int(row.lost)
            result += (
                f'{label} {round(row.x_top_left, self.precision)} {round(row.y_top_left, self.precision)} '
                f'{round(row.width, self.precision)} {round(row.height, self.precision)} {occluded} 0 0 0 0 {lost} 0\n'
            )
