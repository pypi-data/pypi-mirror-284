...
def lrp(det, anno, threshold=0.5, ignore=None):
    ...
    Args:
        ...
        threshold (number):
            Threshold to count a detection as true positive (usually IoU); Default **0.5**
        ignore (boolean, optional):
            Whether to consider the ignore flag of annotations when matching detections; Default **True if ignored annotations found**

    Returns:
        pandas.Dataframe:
            DataFrame with 5 columns **('lrp', 'confidence', 'iou', 'fp', 'fn')** that contains the LRP points at different confidence thresholds
    ...
    if not ({'tp', 'fp'}.issubset(det.columns) and {'detection', 'criteria'}.issubset(anno.columns)):
        ...
        log.info(
            'Custom matching function was used. '
            'Make sure to enter same threshold as the matching function used, as this will otherwise result in wrong metrics.'
        )
    ...
    return lrp.loc[~lrp.confidence.duplicated(keep='last')].reset_index(drop=True)  # Only keep last point where detection confidence is the same
