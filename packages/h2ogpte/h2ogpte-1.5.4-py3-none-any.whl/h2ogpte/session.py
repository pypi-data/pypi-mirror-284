            ...
            llm_args:
                Dictionary of kwargs to pass to the llm. Valid keys:
                    ...
                    cost_controls: Optional dictionary
                        max_cost (float) - Controls costs for every LLM call for automatic model routing. If the estimated cost in USD (based on input and output token counts) is higher than this limit, the request will abort. A value of 0 means no limit.
                        ...
                        willingness_to_pay (float) - Controls the willingness to pay extra for a more accurate model for every LLM call when doing automatic routing, in units of USD per +10% increase in accuracy. We start with the least accurate model. For each more accurate model, we accept it if the increase in estimated cost divided by the increase in estimated accuracy is no more than this value divided by 10%, as long as the total cost is below the maximum cost per LLM call specified above.
