...
class Downloader(object):
    ...
    async def session_client(self, verify, http2, proxies):
        if self.client_dict.get(str(proxies)):
            return self.client_dict.get(str(proxies))
        else:
            async with asyncio.Lock():
                ...
                try:
                    ...
                    pass
                self.client_dict.update({str(proxies): client})
    ...
    async def close_session_client(self, proxies):
        if self.client_dict.get(str(proxies)):
            client = self.client_dict.get(str(proxies))
            await client.aclose()
            self.client_dict.pop(str(proxies))
