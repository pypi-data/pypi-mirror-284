...
class Axis:
    def __init__(self, name=None, shortname=None, bottom=0, top=5,
                 var_name_std=None, var_unit=None, lines_num=5, lines_add: dict = None):
        """
        ...
        :param lines_add: a list of custom lines. Each custom line is defined by a dict, using the following keys:
                ...
                'show':Whether full grid-lines should be shown (All), or just tick marks + user defined lines (User)
                'minor_ticks': If defined, minor ticks will appear. 0 means 'auto define', x means x minor ticks.
        """
        self.name = name
...
class Element:
    ...
    def _check_child(self, el):
        if not issubclass(type(el), Element):
            raise Exception(f"Objects of type {type(el).__name__} cannot be added to an EmberGraph")
        if el.parent_type != self.type:
            raise Exception(f"Graphical object type {type(el).__name__} does not indicate "
...
def draw_vaxis(el: Element):
    ...
    liadds = cy.ax.lines_add
    if liadds:
        egr.c.set_stroke_width(0.35 * mm)
        for ic, li in enumerate(liadds):
            if 'line' not in li:
                egr.logger.addwarn("{el.name}: An axis/grid line is incorrectly defined (missing 'line' key)")
                continue
            glcolor = egr.colors[li.get('color', 'lgrey')]
            glhaz = li['line']
            try:
            ...
            if el.has_vaxis:
                ...
                # Add label
                if float(glhaz) not in leftlines:  # Don't add a user defined label if there is already a main label
