...
def approxdef(deg, cl):
    ...
    return dict({'degree': deg, 'lowerB': lb, 'upperB': ub, 'delta': delta})
...
def chebnodegen(cl):
    ...
    n =cl.n; a = cl.s_min; b = cl.s_max
    cl.nodes=(a + b) * 0.5 + (b - a) * 0.5 * np.cos(np.pi * ((np.arange( (n - 1), -1, -1) + 0.5)/n))
