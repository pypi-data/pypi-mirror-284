...
def vfi_plot(cl=[],max_iter=1e2,s_grid=[],initial_guess=[],tol=0,policy=None,policy_iterations=0):
  if not list(s_grid):
      s_grid=cl.s_grid
  ...
  while i < max_iter and 1>tol:
        ...
        while j<=policy_iterations:
          ...
          vf_plot(cl)
          v= cl.v
          cl.x=lambda s:interp(cl.s_grid,tmp[1])(s)
...
def vfi_plot(cl=ca,max_iter=1e2,s_grid=[],initial_guess=[],optimal=0,tol=0,policy=None,policy_iterations=0,ax_pol=1,plot=1):
  if not list(s_grid):
      s_grid=cl.s_grid
  if not list(initial_guess):
      initial_guess=0*s_grid
