...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelность)s - %(сообщение)s')
...
class PluginConfig(XlsxExportConfig):
    ...
    description = 'Export data from XLSX format'
    version = '0.17'
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                return JsonResponse({'filename': excel_file.name})
        ...
        form = UploadFileForm()
        return render(request, 'upload.html', {'form': form, 'projects': projects})
