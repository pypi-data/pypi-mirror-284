...
import png
from asetools.aseprite import Cel, Frame
from PIL import Image


def save_cel_as_png(cel: Cel, fp: BinaryIO) -> None:
...
def pil_image_for_cel(cel: Cel) -> Image.Image:
    fp = BytesIO()
    save_cel_as_png(cel, fp)
    return Image.open(fp).convert("RGBA")


def render_frame(frame: Frame) -> Image.Image:
    cels = [
