...
class TestJsonlite(unittest.TestCase):
    ...
    def test_update_one(self):
        found_id = self.db.find_one({'name': 'Alice'})["_id"]
        result = self.db.update_one({'name': 'Alice'}, {'name': 'Alicia'})
        self.assertEqual(result.matched_count, 1)
        self.assertEqual(result.modified_count, 1)

        found = self.db2.find_one({'_id': found_id})
        self.assertEqual(found['name'], 'Alicia')
    
    def test_update_many(self):
        # updated
        result = self.db.update_many({'age': {"$gt": 20}}, {'name': 'Alicia'})
        ...
        # not exists
        result = self.db.update_many({'age': {"$gt": 80}}, {'name': 'Alicia'})
        self.assertEqual(result.matched_count, 0)
        self.assertEqual(result.modified_count, 0)

        # duplicate update
        result = self.db.update_many({'age': {"$lt": 30}}, {'name': 'Alicia'})
        self.assertEqual(result.matched_count, 2)
        self.assertEqual(result.modified_count, 1)

    def test_update_by_id(self):
        record = self.db.find_one({'name': 'Alice', 'age': 30})
        result = self.db.update_one({"_id": record['_id']}, {'age': 31})
        self.assertEqual(result.matched_count, 1)
        self.assertEqual(result.modified_count, 1)
