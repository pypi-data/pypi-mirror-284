...
:LastEditTime: 2024-07-10 16:46:27
...
class OrderListHandler(SevenBaseHandler):
    ...
    def get_async(self, *args, **kwargs):
        ...
        if is_excel == 1:
            order_list,order_count = OrderModelEx().get_order_excel_page_list(page_index,page_size,condition,param_list)
        else:
...
class RefundOrderListHandler(SevenBaseHandler):
    ...
    def get_async(self, *args, **kwargs):
        ...
        if is_excel == 1:
            refund_order_list,refund_order_count = OrderRefundModelEx().get_refund_order_excel_page_list(page_index,page_size,condition,param_list)
        else:
