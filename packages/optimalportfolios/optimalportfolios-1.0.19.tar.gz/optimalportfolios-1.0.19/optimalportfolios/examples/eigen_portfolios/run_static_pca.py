...
from optimalportfolios.optimization.solvers.tracking_error import maximize_alpha_over_tracking_error
...
def run_pca_analysis(prices: pd.DataFrame, benchmark_price: pd.Series, freq: str = 'W-WED'):
    ...
    covar = qis.compute_masked_covar_corr(data=returns, is_covar=False)
    corr = qis.compute_masked_covar_corr(data=returns, is_covar=False)

    eigen_values, eigen_vectors = la.eigh(corr)
    #print(eigen_values)
    print(eigen_vectors)
    # print(eigen_vectors @ eigen_vectors.T)
    # print(eigen_vectors @ np.diag(eigen_values) @ eigen_vectors.T - covar)
    asset_vols = np.std(returns_np, axis=0)
    scale = np.outer(asset_vols, np.sqrt(eigen_values).T)
    print(f"scale=\n{scale}")
    ...
    weights_pd.loc['EigenParity_0_11', :] = np.nanmean(weights_pd.iloc[:, :], axis=0)
    weights_pd.loc['EigenParity_8_11', :] = np.nanmean(weights_pd.iloc[8:, :], axis=0)
    weights_pd.loc['EigenParity_9_11', :] = np.nanmean(weights_pd.iloc[9:, :], axis=0)
    weights_pd.loc['EigenParity_10_11', :] = np.nanmean(weights_pd.iloc[10:, :], axis=0)
    print(weights_pd)

    benchmark_weights = weights_pd.iloc[-1, :].to_numpy()
