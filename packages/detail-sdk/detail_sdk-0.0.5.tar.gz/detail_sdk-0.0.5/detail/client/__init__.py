...
from detail.client import constants,logs
...
def instrument(api_key=None):
	D='true';B=api_key;logs.init();E=f"0x{format_trace_id(RandomIdGenerator().generate_trace_id())}";F=os.environ.get('__DETAIL_DEV','').lower()==D;G=os.environ.get('__DETAIL_USE_LOCAL_BACKEND','').lower()==D;A=None;B=B or os.environ.get('DETAIL_API_KEY')
	if B:
