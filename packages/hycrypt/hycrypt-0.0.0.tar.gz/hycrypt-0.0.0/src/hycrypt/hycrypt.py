...
def encrypt_data(
    ...
    """
    return b"\\\\".join(encrypt(plaintext, public_key, padding_hash_algorithm))
...
def decrypt_data(
    ...
    Raises:
        ValueError: Unrecognized encryption format. Raises when the data is not splited by \\\\ into
    ...
    """
    try:
        encrypted_symmetric_key, ciphertext = encrypted_data.split(b"\\\\")
    except ValueError:
...
def encrypt_with_password(
    ...
    Args:
        ...
        padding_hash_algorithm (HashAlgorithm, optional): Hash algorithm for asymmetric padding. Defaults to SHA256().
        salt_length (int, optional): The length of salt in bytes. Defaults to 16. Salt is a random bytes added to the
        password protecting the encrypted private key to defend against precomputed table attacks.
        public_exponent (int, optional): The public exponent of the key. You should always use 65537. Defaults to 65537.
        key_size (int, optional): The size of the new key in bits. The key should be at least 2048 bits.
        The computation time for the key increases non-linearly by the key size. For security beyond 2030,
        3072-bit is recommended. Defaults to 2048.
    ...
    Returns:
        ...
        The public key can be stored and used to encrypt data at other times. Public keys can be shared. The encryption
        is one way, which means other people or you can encrypt the new data using this public key, and you can decrypt
        the message with password.
