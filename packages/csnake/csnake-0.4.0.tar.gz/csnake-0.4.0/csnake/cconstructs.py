...
class LiteralFormatters:
    ...
    def replace(self, *args, **kwargs):
        ...
        if kwargs and not args:
            ...
            if diff:
                raise TypeError(
                    f"Arguments {diff!s} are invalid for this function"
                )
...
class Variable:
    ...
    def merge_formatters(
        value: CLiteral, formatters: Optional[LiteralFormatters] = None
    ) -> Tuple[CLiteral, LiteralFormatters]:
        formatters_ = formatters or LiteralFormatters()

        while isinstance(value, FormattedLiteral):
            formatters_ = formatters_.replace(value.formatter)
            value = value.value

        return (value, formatters_)
    ...
    def init_target_code(
        self, formatters: Optional[LiteralFormatters] = None
    ) -> str:
    ...
    def declaration(self) -> str:
        ...
        """
        return (
            self.generate_declaration(extern=False)
            + ";"
...
class BitField:
    ...
    Note:
        ...
        :meth:`Struct.add_bit_field`.

    Args:
        name: (Optional) name of the bit field
        width: width of the bit field
        primitive: primitive type of the bit field
        comment: accompanying comment
    """
    ...
    def declaration(self) -> str:
        ...
        Ends with a semicolon (;).
        Includes the comment if present.
        """

        retval = "{type} {name}: {width};".format(
            type=self.primitive,
...
def _get_variable(variable: Union[Variable, Collection, Mapping]) -> Variable:
    ...
    * a Mapping (dict-like) with keys (name, primitive)
    ...
    if isinstance(variable, Collection):
        if len(variable) != 2:  # noqa: PLR2004
            raise TypeError(
                "variable must be a Collection with len(variable) == 2"
            )
        var = Variable(*variable)
        return var
    raise TypeError("variable must be one of (Variable, Collection, Mapping)")
...
class Struct:
    ...
    def __init__(self, name: str, typedef: bool = False) -> None:
        self.name = assure_str(name)
        self.variables: List[Union[Variable, BitField]] = []
    ...
    def add_variable(self, variable: Union[Variable, Collection, Mapping]):
        ...
        The order is synced with the :meth:`add_bit_field` function.
    ...
    def add_bit_field(self, bit_field: BitField):
        """Add a bit field to `struct`.

        Bit fields inside of a :class:`Struct` are ordered (added sequentially).
        The order is synced with the :meth:`add_variable` function.

        see :class:`BitField` for more information.

        Args:
            bit_field: bit field to add.
        """
        self.variables.append(bit_field)
...
class Dot(VariableValue):
    ...
    def init_target_code(
        ...
        if isinstance(self.member, VariableValue):
            return (
...
class Arrow(VariableValue):
    ...
    def init_target_code(
        ...
        if isinstance(self.item, VariableValue):
            return (
                self.variable.init_target_code(formatters)
                + "->"
...
class Enum:
    ...
    class EnumValue:
        ...
        def __init__(
            self,
            name: str,
            value: Optional[Union[CLiteral, VariableValue]] = None,
            comment: Optional[str] = None,
        ) -> None:
            self.name = assure_str(name)
    ...
    def __init__(
        self, name: str, prefix: Optional[str] = None, typedef: bool = False
    ) -> None:
        self.typedef = bool(typedef)
        # enum values
        ...
        if isinstance(values, Mapping):
            ...
            for value in values:
                ...
                # lists and tuples
                    ...
                    value_ = seq_get(value, defaults, 1)
                    comment = seq_get(value, defaults, 2)

                    name = assure_str(name)

                    self.add_value(name, value_, comment)
