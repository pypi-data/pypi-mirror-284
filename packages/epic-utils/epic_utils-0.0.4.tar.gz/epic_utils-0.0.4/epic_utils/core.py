...
class ClassPropertyMeta(type):
    def __setattr__(cls, name, value):
        if isinstance(getattr(cls, name, None), ClassProperty):
...
class Converter:
    def dec2hex(value : int):
        ...
        if len(result) < 2:
            result = "0" + result
        return result
class Vector2(metaclass=ClassPropertyMeta):
    ...
    @classmethod
    def bezierInterpolation(p0, p1, p2, time):
...
class Color(metaclass=ClassPropertyMeta):
