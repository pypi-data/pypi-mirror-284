...
from resemble.settings import MAX_BEARER_TOKEN_LENGTH
...
TRANSACTION_IDS_HEADER = 'x-resemble-transaction-ids'
...
class Headers:
    ...
    transaction_coordinator_actor_id: Optional[ActorId] = None
    ...
    def from_grpc_metadata(cls, metadata: GrpcMetadata) -> Headers:
        ...
        # We use `from_maybe_readable` as an affordance for hand-written actor calls.
        actor_id = ActorId.from_maybe_readable(extract(ACTOR_ID_HEADER))
        ...
        transaction_coordinator_actor_id = (
            ActorId(transaction_coordinator_actor_id)
            if transaction_coordinator_actor_id is not None else None
        )
