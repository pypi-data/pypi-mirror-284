...
class StateManager(ABC):
    ...
    ) -> AsyncIterator[tuple[ActorId, bytes]]:
    ...
    async def import_states(
        self, states: AsyncIterator[tuple[StateTypeName, ActorId, Message]]
    ) -> None:
        """Imports state for actors, overwriting on collision."""
        raise NotImplementedError
...
class SidecarStateManager(
    ...
    async def export_states(
        ...
        async for item in self._sidecar_client.export_states(state_type):
            yield item

    async def import_states(
        self, states: AsyncIterator[tuple[StateTypeName, ActorId, Message]]
    ) -> None:
        async for state_type, actor_id, state in states:
            # TODO: Acquiring one lock per state is likely to be slow. Should
            # figure out how to batch this.
            ...
            if effects._colocated_upserts is not None:
                ...
                colocated_upserts.extend(
                    sidecar_pb2.ColocatedUpsert(
                        ...
                        key=actor_id.
                        colocate(SORTED_MAP_ENTRY_TYPE_NAME, ActorKey(key)),
                        value=value,
    ...
    async def transactionally(
        ...
        if (
            ...
            raise RuntimeError(
                ...
                f"'{actor_id}' of type '{state_type}' which is already used "
        ...
        *,
        ...
        try:
            ...
            self._coordinator_commit_control_loop_tasks[
                transaction.root_id] = asyncio.create_task(
        ...
        *,
        ...
        page = await self._sidecar_client.colocated_range(
            ...
            start=(
                ...
                ActorId.from_key(SORTED_MAP_ENTRY_TYPE_NAME, ActorKey(start))
            ),
            end=(
                None if end is None else
                ActorId.from_key(SORTED_MAP_ENTRY_TYPE_NAME, ActorKey(end))
            ),
            limit=limit,
            transaction=transaction,
        )

        parent_actor_id_with_separator = f"{parent_actor_id}/"

        def decode_key(k: str) -> ActorKey:
            assert k.startswith(parent_actor_id_with_separator)
            return ActorId(k[len(parent_actor_id_with_separator):]).key
