...
# Picked so that we don't get a huge amount of load on the system, but do:
# 1. Effectively use multiple CPUs in `rsm dev` and `rsm serve`.
# 2. Are likely to discover any multi-consensus-related bugs in unit tests.
#
# Note that this number does NOT influence the number of replicas in a Resemble
# Cloud deployment, since it gets its `ApplicationConfig` from elsewhere. It may
# still follow `resemble.controller.plan_makers.DEFAULT_NUM_REPLICAS`.
...
class Resemble:
        ...
        *,
        ...
        if config is not None:
            if servicers is not None or legacy_grpc_servicers is not None:
                ...
                raise ValueError(
                    "Passing 'replicas' is not valid when passing 'config'"
                )
        ...
        if len(serviceables) > 0:
            ...
            replicas = replicas or DEFAULT_NUM_REPLICAS
    ...
    async def down(self, config: Optional[ApplicationConfig] = None) -> None:
        ...
        if config is None:
            ...
            # Now wait for the resolver to see that all of those services are no
            # longer part of the plan.
            while len(self._resolver.application_id_by_service_name()) > 0:
                await self._resolver.wait_for_change()
        else:
