...
from resemble.admin.exim_servicer import EximServicer
...
from resemble.aio.placement import PlacementClient, StaticPlacementClient
...
from resemble.v1alpha1.admin import exim_pb2
            ...
            while True:
                try:
                    ...
                    if aborted.code == grpc.StatusCode.UNAVAILABLE:
                        ...
                        # workflow (which may idempotently re-do some parts of
...
class _ServiceServer(Server):
    ...
    class EximRoutable(Routable):
        """Helper "routable" that can be provided to gRPC to route to the
        `EximServicer` system service that we run manually."""

        def service_name(self) -> ServiceName:
            # TODO(benh): get from exim_pb2.DESCRIPTOR.
            return ServiceName('resemble.v1alpha1.admin.Exim')

        def file_descriptor(self) -> FileDescriptor:
            return exim_pb2.DESCRIPTOR.services_by_name['Exim'].file
    ...
    def __init__(
        ...
        *,
        ...
        for serviceable in serviceables:
            if isinstance(serviceable, ResembleServiceable):
                try:
                    ...
                    raise InstantiateError(parent_exception=e)
                ...
                state_types_by_name[servicer.__state_type_name__
        ...
        # Construct 'Exim' system service for the Resemble servicers.
        self._exim_servicer = EximServicer(
            self._application_id,
            consensus_id,
            self._state_manager,
            placement_client,
        ...
        self._exim_servicer.add_to_server(self._grpc_server)
