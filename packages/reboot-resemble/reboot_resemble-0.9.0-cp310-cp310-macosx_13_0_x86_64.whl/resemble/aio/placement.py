...
class PlacementClient(ABC):
    ...
    def shard_for_actor(
        ...
        # TODO: change the following to a single `ActorId` parameter that
    ...
    def consensus_for_actor(
        ...
        Raises `UnknownApplicationError` if the `application_id` is not known.
        """
        shard_id = self.shard_for_actor(application_id, service_name, actor_id)
...
class PlanOnlyPlacementClient(PlacementClient):
    ...
    def shard_for_actor(
        ...
        # TODO: instead of combining this from a separate `service_name` and
        #       `actor_id`, get a fully-qualified service-tagged ActorId passed
        #       in and hash that. See:
        #         https://github.com/reboot-dev/respect/issues/2972.
        #
        #       NOTE: if it is important to remain backwards comptabile (i.e.
        #       that the same shards continue to receive the same traffic as
        #       this change is made), we must preserve the format of the
        #       following "full id" for top-level actors.
        SEPARATOR = "\0"
        actor_full_id = f"{service_name}{SEPARATOR}{actor_id}".encode("utf-8")
        actor_id_hash: bytes = hashlib.sha1(actor_full_id).digest()

        # TODO: binary search.
        for entry in reversed(shard_list):
            if entry.first_key <= actor_id_hash:
