...
from resemble.cli.cloud import add_cloud_options, cloud_workflow
...
async def secret_write(args) -> None:
    ...
    workflow = cloud_workflow(args)
    user_id = await client.user_id(workflow, args.api_key)
    try:
        await client.secret_write(
            workflow, user_id, args.secret_name, secret_value
        )
...
async def secret_delete(args) -> None:
    ...
    user_id = await client.user_id(workflow, args.api_key)
    try:
        await client.secret_delete(workflow, user_id, args.secret_name)
    except Exception as e:
        terminal.fail(
