...
from resemble.aio.types import ActorId, StateTypeName
...
SORTED_MAP_TYPE_NAME = StateTypeName(
    "resemble.integrations.collections.v1.SortedMap"
)
SORTED_MAP_ENTRY_TYPE_NAME = StateTypeName(
    "resemble.integrations.collections.v1.SortedMapEntry"
)
...
class SidecarClient:
    ...
    async def colocated_range(
        ...
        *,
        ...
        start: Optional[ActorId] = None,
        end: Optional[ActorId] = None,
        limit: int = 128,
        transaction: Optional[sidecar_pb2.Transaction] = None,
    ) -> list[tuple[ActorId, bytes]]:
    ...
    async def transaction_participant_prepare(
        self, state_type: StateTypeName, actor_id: ActorId
    ) -> None:
    ...
    async def export_states(
        ...
        for actor in response.actors:
            yield actor.actor_id, actor.state
