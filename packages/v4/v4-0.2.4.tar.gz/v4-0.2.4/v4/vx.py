...
def vxfwopen ( fname, cmnd, hist ):
    ...
    xf.write (struct.pack('I',len(xfname ) + 1 ))
    xf.write (bytearray(xfname.encode()) + bytes([0]))
    xf.write (struct.pack('I', eldcodes['VX_FCMND']))
    xf.write (struct.pack('I',len(cmnd) + 1 ))
    xf.write (bytearray(cmnd.encode()) + bytes([0]))
    ...
    xf.write (struct.pack('I',len(hist) + 1))
    xf.write (bytearray(hist.encode()) + bytes([0]))
...
def vxfwrite ( vxst, fname):
    ...
    if len(hist) == 0:
        hist='no-history '
    cmnd=f'vx.py write {os.path.basename(fname)}'
...
class Vx:
   def __init__(self, *args, cmnd=True):
     if is_notebook():
...
def vwfile ( fname, head, body, cmnd="vx.py-vwfile" ):
...
def vxinfo( vxarg, show=False):
    ''' vxinfo find information about arg and returns 
    a dict of values. If show=True then values are printed
    '''
    #if type(vxim)  == np.ndarray:
    if isinstance (vxarg, list):
      fl = {}
      for di in vxarg:
          key, value = di
          #for key in di.keys():
      ...
      if show:
         ...
         for k in fl.keys():
            print (k, fl[k])
      return (fl)
    #elif isinstance (vxarg, np.ndarray):
    #  print ('arg is numpy') 
    #  return (vxarg)
    else:
      ptype = 'vx.Vx struct'
      if isinstance (vxarg, np.ndarray):
         ptype = 'ndarray'
      res = {}
      vxim = vximp(vxarg)
...
def vxstats(vxarg, show=False):
    res = {}
    vxim = vximp(vxarg)
    res['number'] = np.size(vxim.i)
    ...
    res['stdev'] = round(np.std(vxim.i), 5)
    vq = vxim.i.astype(float)
    #vq = np.where(vxim.i == 0, 'NaN',vq)
    vq[vxim.i==0] = np.nan
    res['numn0'] = np.count_nonzero(vxim.i)
    ...
    res['stdev0'] = round(np.nanstd(vq), 5)
    if show:
       print('Statistic  Value    (Omitting zero)')
       print(f"number: {res['number']:8}    ({res['numn0']:8})")
       print(f"mean  : {res['mean']:8}   ({res['mean0']:8})")
       print(f"median: {res['median']:8}    ({res['median0']:8})")
       print(f"min   : {res['min']:8}    ({res['median0']:8})")
       print(f"max   : {res['max']:8}    ({res['median0']:8})")
       print(f"stdev : {res['stdev']:8}    ({res['stdev0']:8})")
