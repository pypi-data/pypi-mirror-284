...
fn encode_fields(
    ...
    for (i, field) in fields.iter().enumerate().rev() {
        ...
        if_body.extend(quote! {
            let value_node = <#ty as #crate_name::ToKlvm<#node_name>>::to_klvm(&#value_name, encoder)?;
        });
    ...
    let body = if enum_info.is_untagged {
        ...
        if enum_info.default_repr == Repr::Atom {
            ...
            quote! {
                ...
                match self {
                    #( Self::#variant_names => {
                        <#discriminant_type as #crate_name::ToKlvm<#node_name>>::to_klvm(
                            &#discriminant_names,
                            encoder,
                        )
                    }, )*
            ...
            quote! {
                ...
                match self {
                    #( #variant_destructures => {
                        ...
                        let discriminant_node = <#discriminant_type as #crate_name::ToKlvm<#node_name>>::to_klvm(
                            &#discriminant_names,
                            encoder,
                        )?;
...
fn trait_impl(
    ...
    quote! {
        ...
        impl #impl_generics #crate_name::ToKlvm<#node_name>
