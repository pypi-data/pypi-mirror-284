...
class Runner(SendableMixin):
    ...
    def insert_history(
        self, t: Union[datetime, int, float], newstate: str, force: bool = False
    ) -> None:
        """
        ...
        logger.info(
            "(%s) updating runner %s history -> %s", t, self.short_uuid, newstate
        )
        try:
    ...
    def stage_python(
        ...
        runscript = [
            ...
            f"# check start time against manifest, don't output if outdated",
            f"manifest_starttime = manifest.last_time('started').get('"
            f"{self.short_uuid}', -1)",
            f"if manifest_starttime == starttime:",
