...
class Dataset(SendableMixin):
    ...
    def run(
        ...
        **run_args,
        ...
        for runner in self.runners:
            ...
            runline = [
                f"submit_job_{submitter} {runner.short_uuid} "
                f"{jobfile} {errorpath} {runner.resultfile.name}"
            ]
            if runner.remote_dir != runner.run_path:
                runline.insert(0, f"mkdir -p {runner.run_dir} &&")
    ...
    def check_runner_outputs(self) -> Tuple[list, list, list]:
        """Polls the remote, updating the runner and file states"""
    ...
    def update_runners(
        ...
        if log_collection.succeeded:
            ...
            for runner in runners:
                ...
                if runner.short_uuid in runner_data:
                    logger.debug("Runner %s:", runner)
                    for logline in runner_data[runner.short_uuid]:
...
submit_job_{submitter} () {
    echo $(date +'%d/%m/%Y %H:%M:%S') $1 submitted >> $sourcedir/{manifest_filename}
    sed -i -e "s#$1_master#$sourcedir#" $2
    {submission_section}
}"""

    submission_normal = """{submitter} $2 2> $3 || 
    echo $(date +'%d/%m/%Y %H:%M:%S') $1 failed >> $sourcedir/{manifest_filename}
"""
    submission_script = """if {submitter} $2 > $4 2> $3 ; then
        echo $(date +'%d/%m/%Y %H:%M:%S') $1 completed >> $sourcedir/{manifest_filename}
    else
        echo $(date +'%d/%m/%Y %H:%M:%S') $1 failed >> $sourcedir/{manifest_filename}
    fi"""
