...
class MeanStdScaleOp(_ods_ir.OpView):
  ...
  def __init__(self, output, input, quant_mode, customization_format, channel_order, scale, std, mean, zero_points, resize_dims, multi, rshift, offset, rounding_mode, *, sign=None, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('I64ArrayAttr')) else
      _ods_ir.AttrBuilder.get('I64ArrayAttr')(multi, context=_ods_context))
...
class Yuv2rgbFormulaOp(_ods_ir.OpView):
  ...
  def __init__(self, output, Y, U, V, width, height, src_format, dst_format, image_format, formula_mode, *, round_mode=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    operands.append(_get_op_result_or_value(Y))
    operands.append(_get_op_result_or_value(U))
    ...
    not _ods_ir.AttrBuilder.contains('UI32Attr')) else
      _ods_ir.AttrBuilder.get('UI32Attr')(width, context=_ods_context))
    attributes["height"] = (height if (
    issubclass(type(height), _ods_ir.Attribute) or
    not _ods_ir.AttrBuilder.contains('UI32Attr')) else
      _ods_ir.AttrBuilder.get('UI32Attr')(height, context=_ods_context))
  ...
  @builtins.property
  def height(self):
    return self.operation.attributes["height"]

  @height.setter
  def height(self, value):
    if value is None:
      raise ValueError("'None' not allowed as value for mandatory attributes")
    self.operation.attributes["height"] = value
