...
class Client(object):
    ...
    def list_models(
        ...
        :param workspace_id: Optional[int]: The workspace id to search in. If not provided, the current workspace is used.
    ...
    def list_pipelines(
        ...
        if not workspace_id and not workspace_name:
            ...
            res = self._gql_client.execute(
                gql.gql(
                    f"""
                        query ListPipelinesFilteredByWorkspace({query_param_str}) {{
                          pipeline({where_clause_str}, order_by: {{id: desc}}) {{
                            id
                            pipeline_tags {{
                              tag {{
                                id
