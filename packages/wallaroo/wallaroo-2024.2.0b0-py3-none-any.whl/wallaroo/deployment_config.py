...
class DeploymentConfigBuilder(object):
    ...
    def gpus(self, gpu_count: int) -> "DeploymentConfigBuilder":
        ...
        self._clear_gpu_resources(self._config["engine"])
        key = (
            "gpu.intel.com/i915"
    ...
    def accel(self, accel: Acceleration) -> "DeploymentConfigBuilder":
        old_accel = str(self._config["engine"].get("accel", "none"))
        self._config["engine"]["accel"] = str(accel) if accel is not None else None
        self._adjust_gpu_resources(self._config["engine"], accel, old_accel)
        return self
    ...
    def sidekick_gpus(
        ...
        self._clear_gpu_resources(sidekick)
        key = (
            "gpu.intel.com/i915"
    ...
    def sidekick_accel(
        ...
        config = self._guarantee_sidekick_stanza(model_version)
        old_accel = str(config.get("accel", "none"))
        config["accel"] = str(acceleration) if acceleration is not None else None
        self._adjust_gpu_resources(config, acceleration, old_accel)
        return self

    def _adjust_gpu_resources(
        self, component: Dict[str, Any], accel: Acceleration, old: str
    ):
        resources = component.get("resources", {"limits": {}, "requests": {}})
        if accel == Acceleration.OpenVINO and old == "cuda":
            resources["limits"]["gpu.intel.com/i915"] = resources["limits"].pop(
                "nvidia.com/gpu", 0
            )
            resources["requests"]["gpu.intel.com/i915"] = resources["requests"].pop(
                "nvidia.com/gpu", 0
            )
        elif accel == Acceleration.CUDA and old == "openvino":
            resources["limits"]["nvidia.com/gpu"] = resources["limits"].pop(
                "gpu.intel.com/i915", 0
            )
            resources["requests"]["nvidia.com/gpu"] = resources["requests"].pop(
                "gpu.intel.com/i915", 0
            )

    def _clear_gpu_resources(self, component: Dict[str, Any]):
        resources = component.get("resources", {"limits": {}, "requests": {}})
