...
class Pipeline:
    ...
    def invoke(
        ...
        dynamic_system_prompt=None,
        output_parser=None,
        dynamic_system_prompt_variables=None,
    ):  
        ...
        # Determine system prompt to use
        if(self.therix_api_key):
            pipeline_system_prompt = {"system_prompt": dynamic_system_prompt} if dynamic_system_prompt else (self.pipeline_service.get_prompt_by_name(saved_system_prompt[0].config.get('system_prompt'), dynamic_system_prompt_variables if dynamic_system_prompt_variables else None) if saved_system_prompt else None)
        else:
            pipeline_system_prompt = (
            {"system_prompt": dynamic_system_prompt}
            if dynamic_system_prompt
