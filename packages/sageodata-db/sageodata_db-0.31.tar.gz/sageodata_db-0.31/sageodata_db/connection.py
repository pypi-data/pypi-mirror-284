...
class SAGeodataConnection(cx_Oracle.Connection):
    ...
    def open_db_file(self, file_no):
        ...
        cursor = self.cursor()
        var = cursor.var(cx_Oracle.BLOB)
        cursor.execute(f"select file_name, file_contents from dhdb.fi_file where file_no = {file_no}")
        for item in cursor:
            file_name = item[0]
            file_object = item[1]
        return file_name, io.BytesIO(file_object.read())

    def open_db_file_as_text(self, file_no, encoding=('ascii', 'cp1252', 'utf-8'), raise_error=True):
        ...
        while i < len(encoding):
            ...
            try:
                contents = buffer.decode(try_encoding)
            except:
                pass
            i += 1
        if contents is False:
            if raise_error:
                ...
                print(f"Encoding error reading file_no={file_no}")
                contents = ""
