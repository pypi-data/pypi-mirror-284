...
class Shellium:
    ...
    def run(self) -> Chrome:
        ...
        """
        if self.driver:
            raise ShellDriverAlreadyRunningError(
                f'The ShellDriver is already running: {self.driver}.'
            )
        self._driver = Chrome(
            self.options,
            self.service,
