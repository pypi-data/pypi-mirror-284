...
def _transform_variable_or_list(
    ...
    return [_transform(VariableOr[cls], item) for item in value]
...
def _transform_variable_or_dict(
    ...
    return {key: _transform(VariableOr[cls], item) for key, item in value.items()}


def _transform_optional(cls: Type[_T], value: Any) -> Optional[_T]:
    if value is None:
        return None
...
def _transform_field(cls: Type[_T], field: Field, value: Any) -> _T:
    ...
    if isinstance(field.type, ForwardRef) or isinstance(field.type, str):
        ...
        return _transform(field_type, value)
    else:
    ...
    if origin := get_origin(tpe):
        if origin == Union:
            ...
            if len(args) == 2 and args[1] == type(None):
                return args[0]
            elif len(args) == 2 and args[1] == type(None):
                return args[1]
