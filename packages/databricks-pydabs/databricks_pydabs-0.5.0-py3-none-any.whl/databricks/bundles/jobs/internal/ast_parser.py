...
class InvalidAttributeAccessError(InternalJobError):
    def __init__(self, node: ast.AST, message: str) -> None:
...
def _eval_expr_unchecked(expr: ast.expr, scope: Scope) -> Any:
    if isinstance(expr, ast.Call):
        ...
        if isinstance(lhs, TaskWithOutput) and expr.attr in ["output", "result"]:
            if not lhs.return_type:
                raise InternalJobError(expr, "Task doesn't have any output")

            return_type = lhs.return_type
            ...
            return TaskValueParameter(
                task_key=resolve_variable(lhs.task_key),
                value_key="return_value",
                value_type=return_type["return_value"],
            )
        ...
        if isinstance(lhs, TaskValueParameter) or isinstance(lhs, JobParameter):
            raise InvalidAttributeAccessError(
                expr,
                "Accessing attributes of task values or job parameters is not supported",
            )
...
def _map_to_syntax_error(
    ...
    if isinstance(e, InvalidAttributeAccessError):
        assert isinstance(e.node, ast.Attribute)
        col_offset = e.node.value.end_col_offset or col_offset
...
def _get_location(scope: Scope, ast: ast.AST) -> Optional[Location]:
...
def _eval_for_each(stmt: ast.For, scope: Scope):
    ...
    if isinstance(iter, TaskValueParameter):
...
def _parse_condition_step(expr: ast.expr, scope: Scope) -> Task:
    ...
    if isinstance(left, TaskValueParameter):
        depends_on.append(TaskDependency(left.task_key))

    if isinstance(right, TaskValueParameter):
...
def _generate_task_key_part(param: TaskParameter):
    if isinstance(param, TaskValueParameter):
        ...
        if isinstance(param.value, int):
            ...
            fixed_str = param.value.replace(" ", "_")[0:limit]
...
def _parse_operator(op: ast.cmpop, parent: ast.AST) -> ConditionTaskOp:
