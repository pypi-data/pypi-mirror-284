...
class PassCriteria():
    ...
    def calc_for_stage_uplink(pass_value:int, stage_name:str) -> int:
        if 'Init' in stage_name and pass_value > MINIMUM_SCORE:
            return PERFECT_SCORE
        else:
            return pass_value

    @staticmethod
    def calc_for_stage_downlink(rsquared, slope, stage_name:str) -> int:
        if 'Sanity' in stage_name:
            ...
            if rsquared > 0.8 and slope > 0.7:
                ...
                return INCONCLUSIVE_MINIMUM
            else:
                return MINIMUM_SCORE

    @staticmethod
    def calc_for_test(test_pass_value:int, stage_pass_value:int, stage_name:str) -> int:
        if stage_pass_value < test_pass_value:
            if 'Geolocation' in stage_name:
                return test_pass_value
            else:
                return stage_pass_value
        else:
            return test_pass_value
