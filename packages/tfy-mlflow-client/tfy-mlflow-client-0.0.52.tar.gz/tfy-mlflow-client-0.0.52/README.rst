...
Setup
-----
Step to setup the repo on you local machine: https://truefoundry.atlassian.net/wiki/spaces/EN/pages/28114961/Setting+up+Mlfoundry-server+on+your+local+machine

Steps to update the version and push automatically to pypi:

* Increment the version manually in mlflow/version.py and merge the changes with main
* Pull the changes to 'main' branch locally
* Add a version tag and push the tag
``git tag v-tf-x.y.z && git push --tag``
