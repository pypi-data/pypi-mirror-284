...
.. py:function:: calculate_axial_forces(meta_data, z_ref, T0:float = None, is_export:bool = False)
        ...
        :type meta_data: dict
        :param z_ref: Bezugsteufe, die Referenzteufe wird aus dem Mittelwert aus Bezugsteufe und z_0 = 0 berechnet. 
        :type z_ref: _type_
...
+----------------+------------------------+--------------------------------+------------+
| z_bezug        | 425                    | Bezugsteufe für RSSF           | m          |
+----------------+------------------------+--------------------------------+------------+
| rho_rrsf       | 1200                   |Dichte Ringraumschutzflüssigkeit| kg/m3      |
+----------------+------------------------+--------------------------------+------------+
...
Aus der Bezugsteufe ``z_ref`` wird mit dem Kopf der Bohrung :math:`z_0 = 0 m` die Referenzteufe :math:`z_m = 0.5 \cdot (z_{ref} + z_0)` für die Axialkraftberechnung gerechnet.
Damit muss die Bezugsteufe nicht zwingend am Rohrschuh der Bohrung liegend, weil oft der Zementkopf für die Bezugsteufe auschlaggebend ist.
...
.. code-block:: python
    ...
    T_0 = 15 # °C
    depth_ref = meta_data['z_bezug'] # Die Bezugsteufe der RRSF ist gleich der Bezugsteufe für die Druck- und Temperaturdaten.
