...
class DatabaseBrowser:
    ...
    def search_table(self, table_name, search_criteria, print_value=False):
        """
        Search a table for specific values in multiple columns.

        Args:
            table_name (str): The name of the table to search.
            search_criteria (dict): A dictionary where keys are column names and values are the search values.
        ...
        for column_name in search_criteria.keys():
        ...
        where_clauses = [f"{col} LIKE :{col}" for col in search_criteria.keys()]
        where_clause = " AND ".join(where_clauses)

        try:
            query = text(f"SELECT * FROM {table_name} WHERE {where_clause}")
            result = self.session.execute(query, search_criteria)
            rows = result.fetchall()
            ...
            if rows:
                ...
                print(f"No results found for the search criteria in {table_name}")
        except Exception as e:
