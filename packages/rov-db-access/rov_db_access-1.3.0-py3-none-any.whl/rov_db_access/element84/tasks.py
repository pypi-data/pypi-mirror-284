...
def get_runs(df:geopandas.GeoDataFrame, polygon:shapely.Polygon, alpha:float) -> List:
    ...
    lista_resultado.append({
        "t1": {
            ...
            "date": datetime.strptime(candidate["datetime_left"], date_format).strftime("%d-%m-%Y")
        },
        "t2": {
            "title":
            candidate["s2:product_uri_right"],
            "date": datetime.strptime(candidate["datetime_right"], date_format).strftime("%d-%m-%Y")
        },
        "mask":mask 
