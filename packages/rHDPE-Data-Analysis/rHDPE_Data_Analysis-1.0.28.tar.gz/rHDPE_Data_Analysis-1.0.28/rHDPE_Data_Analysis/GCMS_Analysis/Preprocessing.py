...
def standardise_data( data ):
    ...
    for i in range( len( data[1] ) ):
        ...
        rt_mask = np.where( (np.array( data[0] ) <= 4.2) & (np.array( data[0] ) >= 4) )[0]

        maximum.append( data[1][i][rt_mask].max() )

    maximum_array = np.array( maximum ).astype( np.float32 )

    maxi = maximum_array.max()

    maximum_array_2 = np.zeros_like( maximum_array )

    for i in range( len( maximum_array ) ):

        maximum_array[i] = maxi / maximum_array[i]

    # for i in range( len( data[1] ) ):
    #
    #     data[1][i] = data[1][i] * maximum_array[i]

    # data[1].append( data[1][1] * maximum_array[1] )
    # data[2].append( data[1][1] * maximum_array[1] )
...
def read_csv( directory, output_directory, merge_groups ):
    ...
    for i in range( 1, len( df.columns ), 2 ):
        ...
        data[2].append( df.iloc[:, i + 1].to_numpy() )
...
def compute_mean( output_directory, file_data, data ):
    ...
    m = gu.sample_mean( file_data, data[2] )
