...
def extract_DSC_features( output_directory, file_data, data, first_derivative_data, second_derivative_data ):
    ...
    for i in range( len( file_data ) ):
        ...
        integral_2 = gu.integral_2( data[2][temp_mask_2], -data[3][i][temp_mask_2], 0, len( data[2][temp_mask_2] ) - 1, min( -data[3][i][temp_mask_2][0], -data[3][i][temp_mask_2][len( data[3][i][temp_mask_2] ) - 1] ) ) / 208 * 6
        ...
        integral_1 = integral_1 / (1 - fpp)

        feature_1.append( integral_1 )
        feature_2.append( fpp )
