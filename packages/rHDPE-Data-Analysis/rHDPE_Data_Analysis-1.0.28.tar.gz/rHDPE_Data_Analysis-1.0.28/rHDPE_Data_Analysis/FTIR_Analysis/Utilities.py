...
def pp_percentage_model( directory, output_directory, file_data, data ):
    ...
    blend_samples = [500, 502, 504, 506, 508, 510, 512, 514, 516]

    resin_data = gu.get_list_of_resins_data( directory )

    sample, sample_array, samples_present, samples_present_array = gu.sample_data_from_file_data( file_data )

    sample_mask = [11, 14, 10, 4, 13, 21, 23, 18, 22, 20, 2, 3, 17, 16, 19, 1, 15, 12, 6, 5, 7, 9, 8, 401, 402, 403, 404, 405, 406, 407, 408, 409, 410, 411, 412, 413, 414, 415, 416, 601, 602, 603, 604, 605, 500, 502, 504, 506, 508, 510, 512, 514, 516, 518]
    ...
    # gu.normalise_features( features_array ) # Believe this is no longer needed.
    ...
    # plt.scatter( [(model - mean_features[i, 0]).roots[1] for i in range( 23 )], mean_features[:23, 0], color = scatter_colours, s = 100 )
    #
    # plt.plot(polyline, xequaly_line( polyline ) )
    #
    # plt.xlabel( "Known concentration of PP (%)", fontsize = 13 )
    # plt.ylabel( "Model value", fontsize = 13 )
    #
    ...
    final_array = np.array( final_pp_prediction )

    pearson, _ = pearsonr( [0, 2, 4, 6, 8, 10, 12, 14, 16], final_array[[sample_mask.index( j ) for j in blend_samples]] )
    ...
    #
    #     for j in range( len( features_array[0] ) ):
    #
    #         specimen_pp_predictions.extend( features_array_copy[:, j].tolist() )
    #
    ...
    #     for j in range( len( features_array[0] ) ):
    #
    #         specimen_pp_predictions.append( features_array_copy[:, j].tolist() )
    #
    #     array = np.array( specimen_pp_predictions )
    #
    #     array = np.vstack( (array, np.mean( array, axis = 0 )[np.newaxis, :]) )
    #     array = np.hstack( (array, np.mean( array, axis = 1 )[:, np.newaxis]) )
    #
    #     df = gu.array_with_column_titles_and_label_titles_to_df( array, [i for i in range( len( array[0] ) - 1 )] + ["Mean"], feature_names + ["Mean"] )
    #
    #     gu.plot_df_heatmap( df )
...
def crystallinity_determination( directory, output_directory, file_data, data ):
    ...
    df.to_csv( output_directory + "FTIR/Sandbox/Mean_Features.csv" )

    std_of_features_plus_sample_mask = np.hstack( (np.array( sample_mask )[:, np.newaxis], std_of_features) )

    df = gu.array_with_column_titles_to_df( std_of_features_plus_sample_mask, ["sample"] + mean_feature_names )

    df.to_csv( output_directory + "FTIR/Sandbox/Std_of_Features.csv" )

    df = gu.array_with_column_titles_and_label_titles_to_df( mean_distance_matrix, sample_mask, sample_mask )

    df.to_csv( output_directory + "FTIR/Sandbox/Distance_Matrix.csv" )

    gu.plot_features( output_directory, features, feature_names, [f[2] for f in file_data_mask], specimen = True, subdirectory = "FTIR/Sandbox/", title = "Specimen_Features.pdf" )

    gu.plot_features( output_directory, mean_features, mean_feature_names, [resin_data.loc[i]["Label"] for i in sample_mask], subdirectory = "FTIR/Sandbox/", title = "Means_Features.pdf" )
