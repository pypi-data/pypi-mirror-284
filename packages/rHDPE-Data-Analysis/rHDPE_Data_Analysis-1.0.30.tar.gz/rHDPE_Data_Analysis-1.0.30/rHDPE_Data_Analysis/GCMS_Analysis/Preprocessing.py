...
def standardise_data( data ):
    ...
    for i in range( len( data[1] ) ):
        ...
        rt_mask = np.where( (np.array( data[0] ) <= 4.3) & (np.array( data[0] ) >= 4) )[0]

        maximum.append( data[1][i][rt_mask].max() )
        retention_time_shift.append( int( math.floor( (data[0][rt_mask][data[1][i][rt_mask].argmax()] - 4.18) * 1200 ) ) )

    maximum_array = np.array( maximum ).astype( np.float32 )

    for i in range( len( maximum_array ) ):

        maximum_array[i] = 100 / maximum_array[i]

    normalised_headspaces = []

    for i in range( len( data[1] ) ):

        normalised_headspaces.append( data[1][i] * maximum_array[i] )
