...
def compute_derivatives( data, width = 1 ):
    ...
    for i in range( len( data[3] ) ):
        ...
        derivative_data[3].append( gu.derivative( data[1], data[3][i], width ) )

    for i in range( len( data[5] ) ):

        derivative_data[5].append( gu.derivative( data[1], data[5][i], width ) )
...
def extract_TGA_features( output_directory, file_data, data, first_derivative_data, second_derivative_data ):
    ...
    for j in range( 28 ):
        ...
        mask = np.where( (np.array( first_derivative_data[1] ) >= lower_bound) & (np.array( first_derivative_data[1] ) <= upper_bound) )[0]

        for i in range( len( file_data ) ):

            feature_1.append( -np.mean( np.array( first_derivative_data[3][i] )[mask] ) )
