...
def GCMS_Analysis_Main( ip ):
    ...
    # Data of form [retention time, [Absolute intensity], [Relative intensity]].

    print( str( len( file_data ) ) + " files have been read." )

    if len( file_data ) != 6:

        print( "Warning: 6 files have not been read." )
    ...
    # Data now of form [retention time, [Absolute intensity], [Relative intensity], [m (relative intensity)]].
    ...
    if ip.plot_data:
        ...
        data_to_plot = GCMS_plotting.plot_data( ip.directory, ip.output_directory, file_data, data, first_derivative_data, second_derivative_data, third_derivative_data, ip.shiny, ip.shiny_samples_to_plot, ip.shiny_split, ip.shiny_specimen, ip.shiny_mean )
