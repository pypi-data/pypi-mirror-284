...
class Mechanism():
    ...
    def __init__(self,
        ...
        if isinstance(sales_price, int) or isinstance(sales_price, float):
            ...
            upper_limits = sales_price * 1.25
            max_purchase_price = self.ATTR["PURCHASE_PRICE"].max()
            upper_limits = np.where(upper_limits>max_purchase_price, max_purchase_price, upper_limits)
            lower_limits = sales_price * 0.75
            if dim_array == 1:
                if shape_array[0] == subsidy_period:
                    ...
                    for t in range(1, subsidy_period):
                        ...
                        SALES_PRICE_ARRAY_2D[t, :] = SALES_PRICE_ARRAY_2D[t - 1, :] + (drift * 1) + SALES_PRICE_ARRAY_2D[t - 1, :] * self.ATTR["VOLATILITY"] * random_shocks
