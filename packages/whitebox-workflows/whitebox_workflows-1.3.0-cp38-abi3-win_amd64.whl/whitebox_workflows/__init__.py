...
def show(obj, ax=None, title=None, figsize=None, skip=None, clip_percent=None, plot_as_surface=False, vert_exaggeration=None, colorbar_kwargs=None, **kwargs):
    ...
    clip_percent : float, optional
        ...
        function to clip the tails of the raster histogram. Depending on the size of the raster, this
        function can significantly increase time to display. If the same raster is being added to
        multiple Axes in a multi-part figure, it may be advisable to only set the clip_percent 
    ...
    **kwargs : key, value pairings, optional
    ...
    if isinstance(obj, Vector):
        ...
        if clip_percent is not None:
            is_clipped = True
            if clip_percent <= 0.0 or clip_percent >= 50.0:
                is_clipped = False
        ...
        if obj.configs.photometric_interp != PhotometricInterpretation.RGB and not plot_as_surface: # A continuous image, e.g. a DEM
            ...
            kwargs['vmin'] = obj.configs.minimum
            kwargs['vmax'] = obj.configs.maximum
        ...
        if 'cmap' in kwargs and 'color' not in kwargs:
            kwargs['c'] = zs
    ...
    # end1 = time()
    # print(f"Elapsed time: {end1 - start1} seconds")
