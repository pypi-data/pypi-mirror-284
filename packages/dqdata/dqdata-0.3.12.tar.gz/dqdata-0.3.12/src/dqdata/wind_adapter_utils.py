...
from wind_request import WDAdaptorClient as wc
...
class WindUtil:
    ...
    def sectors(cls, sectorid, date=None):
        ...
        if cls.adapter is None:
            logging.error('set adapter url first!')
            return None
        wd = wc.wset(cls.adapter, 'sectorconstituent', f'sectorid={sectorid};date={cls.get_date_str(date)}', usedf=True)
    ...
    def sectors_by_code(cls, wind_code, date=None):
        ...
        """
        # wd = w.wset('indexconstituent', f'windcode={wind_code};date={cls.get_date_str(date)}', usedf=True)
        if cls.adapter is None:
            logging.error('set adapter url first!')
            return None
        wd = wc.wset(cls.adapter, 'indexconstituent', f'windcode={wind_code};date={cls.get_date_str(date)}', usedf=True)
        if wd[0] != 0: logging.error(f' wind error: {wd[0]}')
        return wd[1] if wd[0] == 0 else None

    @classmethod
    @startwind
    def sectors_by_index(cls, wind_code, date=None):
        """
        获取板块成分(通过Wind代码)
        ~~~~~~~~~~~~~~~~
        df = WindUtil.sectors_by_index('000300.SH') #沪深300成分股
        """
        # wd = w.wset('indexconstituent', f'windcode={wind_code};date={cls.get_date_str(date)}', usedf=True)
        if cls.adapter is None:
            logging.error('set adapter url first!')
    ...
    def fu_contracts(cls, wind_code, start_date=None, end_date=None):
        ...
        if cls.adapter is None:
            logging.error('set adapter url first!')
            return None
        wd = wc.wset(cls.adapter, 'futurecc',
                     f'wind_code={wind_code};startdate={cls.get_date_str(start_date)};enddate={cls.get_date_str(end_date)}',
                     usedf=True)
        if wd[0] != 0: logging.error(f' wind error: {wd[0]}')
        return wd[1] if wd[0] == 0 else None
    ...
    def wsd(cls, codes, fields, start_date=None, end_date=None, options=None):
        ...
        if cls.adapter is None:
            logging.error('set adapter url first!')
            return None
        wd = wc.wsd(cls.adapter, codes, fields, cls.get_date_str(start_date), cls.get_date_str(end_date), options)
        ...
        if wd is not None and wd.ErrorCode == 0 and len(wd.Times) > 0 and len(wd.Data[0]) > 0:
            data = {'date': wd.Times}
            fields = wd.Codes if len(wd.Codes) > 1 else wd.Fields
    ...
    def wset(cls, name, options):
        ...
        if cls.adapter is None:
            logging.error('set adapter url first!')
            return None
        wd = wc.wset(cls.adapter, name, options, usedf=True)
        if wd[0] != 0: logging.error(f' wind error: {wd[0]}')
        return wd[1] if wd[0] == 0 else None
    ...
    def edb(cls, codes, start_date=None, end_date=None, options=None):
        ...
        if cls.adapter is None:
            logging.error('set adapter url first!')
            return None
        wd = wc.edb(cls.adapter, codes, cls.get_date_str(start_date), cls.get_date_str(end_date), options)
        if wd.ErrorCode != 0: logging.error(f' wind error: {wd.ErrorCode}')
        if wd is not None and wd.ErrorCode == 0 and len(wd.Times) > 0 and len(wd.Data[0]) > 0:
            data = {'date': wd.Times}
    ...
    def wsq(cls, codes, fields):
        ...
        """
        df = None
        wd = w.wsq(codes, fields)
        if wd.ErrorCode != 0: logging.error(f' wind error: {wd.ErrorCode}')
        if wd is not None and wd.ErrorCode == 0 and len(wd.Times) > 0 and len(wd.Codes) > 0 and len(wd.Data[0]) > 0:
            logging.info(f'Time: {wd.Times[0]}')
            data = {'code': wd.Codes}
            for idx, field in enumerate(wd.Fields):
                data[field] = wd.Data[idx]
            df = pd.DataFrame(data)
