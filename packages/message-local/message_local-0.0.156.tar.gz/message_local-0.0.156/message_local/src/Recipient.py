...
class Recipient:
    ...
    def get_usernames(self, system_id: int) -> list[str]:
        # Return all the usernames that the recipient has. Gets all the usernames for the system_id, and gets all the
        # usernames for the recipient, then returns the common values
        custom_condition = f"system_id = {system_id}"
        if self.__email_address_str:
            custom_condition += f" AND email_address_str = {self.__email_address_str}"
        if self.__user_id:
            custom_condition += f" AND user_id = {self.__user_id}"
        if self.__contact_id:
            custom_condition += f" AND contact_id = {self.__contact_id}"
        if self.__profile_id:
            custom_condition += f" AND profile_id = {self.__profile_id}"
        if self.__telephone_number:
            custom_condition += f" AND telephone_number = {self.__telephone_number}"

        usernames = GenericCRUD.select_multi_value_by_where(schema_name="user_external",
                                                            view_table_name="user_external_general_view",
                                                            select_clause_value="username", where=custom_condition)
