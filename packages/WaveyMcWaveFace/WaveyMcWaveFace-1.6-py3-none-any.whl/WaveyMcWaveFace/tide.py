...
class CustomDataset(Dataset):
    def __init__(self, categorical_data, continuous_data, targets, prices, apply_log_transform=True):
        ...
        self.targets = targets  # Apply log transformation to targets; +1 to avoid log(0)
        self.prices = prices
        self.apply_log_transform = apply_log_transform  # Store the log transform flag
    ...
    def __getitem__(self, idx):
        ...
        # Apply log transformation if the flag is set
        if self.apply_log_transform:
            y = np.log(y + 1)  # Apply log transformation to targets; +1 to avoid log(0)
            prices = np.log(prices + 1)  # Apply log transformation to prices
...
class TiDE:
    def __init__(self, data, embedding_dims, target_size, test_size=0.2, random_state=42, apply_log_transform=True):
        self.label_encoders = {}
    ...
    def train_model(self, num_epochs=10, lr=0.001, batch_size=32, patience=5, criterion_=None):
        ...
        train_dataset = CustomDataset(self.train_data['cat'], self.train_data['cont'], self.train_targets,
                                      self.train_prices, apply_log_transform=self.apply_log_transform)
    ...
    def predict(self, new_data):
        ...
        if self.apply_log_transform == True:
            ...
            return pd.DataFrame(predictions.numpy(),
                                columns=[f'Prediction_{i}' for i in range(predictions.numpy().shape[1])])
