...
def get_today_weather(api_key, location):
    url = f"https://devapi.qweather.com/v7/weather/3d?location={location}&key={api_key}"
    response = requests.get(url)
    if response.status_code == 200:
        data = response.json()
        if data.get('code') == '200':
            return data['daily'][0]  # 获取明天的天气数据
        else:
            return {"error": data.get('message', '无法获取天气数据')}
    else:
        return {"error": "请求失败"}
...
def will_it_rain_today(today_weather):
...
if __name__ == "__main__":
    ...
    today_weather = get_today_weather(api_key, location)
    if 'error' not in today_weather:
        if will_it_rain_today(today_weather):
            print("今天会下雨")
        else:
            print("今天不会下雨")
    else:
