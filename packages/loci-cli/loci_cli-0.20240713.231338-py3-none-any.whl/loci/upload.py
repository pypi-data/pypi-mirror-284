...
from loci_client.models import (
    ArtifactDescriptor,
    ArtifactNoteIn,
    ArtifactNoteInArtifactStatus,
    ArtifactNoteInArtifactPriority,
    ArtifactNoteInArtifactType,
...
def upload(
    ...
    for src_code_zipfile in zipfiles:
        if not dry_run:
            utils.print_info(f"Processing {src_code_zipfile}...")
        else:
            utils.print_info(f"Processing {src_code_zipfile}... (dry run)")
            ...
            if (
                ...
                with utils.get_api_client(project_info) as api_client:
                    ...
                    artifact_note_in = ArtifactNoteIn(
                        ...
                        artifact_status=ArtifactNoteInArtifactStatus(
                            ArtifactStatusEnum.TODO
                        ),
                        artifact_priority=ArtifactNoteInArtifactPriority(
                            ArtifactPriorityEnum.LOW
                        ),
                        artifact_descriptor=ArtifactDescriptor(
                            full_artifact_descriptor
                        ),
                        contents=file_content,
                        artifact_type=ArtifactNoteInArtifactType(
                            ArtifactTypeEnum.SOURCE_CODE_FILE
                        ),
                    )
        ...
        utils.print_info(f"Finished processing [bold]{src_code_zipfile}[/bold]")
