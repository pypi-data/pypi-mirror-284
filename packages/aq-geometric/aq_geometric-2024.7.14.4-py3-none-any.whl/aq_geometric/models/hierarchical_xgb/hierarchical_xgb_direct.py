    ...
    * if the target is missing we can just skip the row!
    ...
    def _generate_forecasts_direct(
        ...
        verbose: Union[List[str], None] = None,
            ...
            self.node_id_to_h3_index_map = {i: v for i, v in enumerate(graph.h3_index)}
            self.h3_index_to_node_id_map = {v: i for i, v in enumerate(graph.h3_index)}
            ...
            for i, target in enumerate(targets):
                tmp = pd.DataFrame(pred[:, :, i], columns=target_timestamps, index=valid_h3_indices)
                tmp = tmp.join(
                    pd.DataFrame(h3_indices, columns=["h3_index"]).set_index("h3_index"),
                    how="right",
                )
            ...
            if verbose:
                print(f"[{datetime.now()}] Before re-indexing, predictions shape: {pred.shape}")
            ...
            if include_history:
                ...
                forecasts = testmask.reshape(-1, 1, len(targets)) * pred  # Apply mask before multiplication
            else:
                forecasts = pred
