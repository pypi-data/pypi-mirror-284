...
class AqGeometricLinearModel(BaseModel):
    ...
    def predict(self, g: "Data") -> Tuple[np.ndarray, np.ndarray]:
        """Fit a linear model using the data in g.x."""
        def linear_fit(x, m, b):
            return (m * x) + b

        data = g.x.detach().numpy()
        h3_indices = g.h3_index
        pred = np.zeros((len(h3_indices), self.num_samples_in_node_target, self.num_features_in_node_target))
        
        
        for i in range(len(h3_indices)):
            for k in range(self.num_features_in_node_target):
                # fit a linear model to the observations for this station and feature
                h3_data_values = data[i, :, k].reshape(-1)
                h3_data_idx = np.array([range(len(h3_data_values))]).reshape(-1)

                # some obervations might be missing
                # if all are missing, update preds
                if np.all(np.isnan(h3_data_values)):
                    pred[i, :, k] = np.nan
                else:
                    # otherwise we fit a curve through the points in h3_data
                    params = curve_fit(linear_fit, h3_data_idx, h3_data_values)
                    (m, b) = params[0]
                    target_h3_data_idx = np.array([range(len(h3_data_values), len(h3_data_values) + self.num_samples_in_node_target)])
