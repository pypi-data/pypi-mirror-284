    ...
    def predict(self, g: "Data") -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(self.num_samples_in_node_target):
            ...
            for j, target in enumerate(self.targets):
                tmp = pd.DataFrame(
                    pred[:,:,j], index=h3_indices
                )
                tmp = tmp.join(pd.DataFrame(graph_h3_indices, columns=["h3_index"]).set_index("h3_index"), how="right")
                preds_dfs.append(tmp)
            pred = np.stack([p.to_numpy() for p in preds_dfs], axis=2)
            # update g.x with the new data
            g.x = torch.cat([g.x[:, 1:, :], torch.tensor(pred, dtype=torch.float32)], axis=1)
            # determine a mask for missing values in pred
            mask = np.isnan(pred)
            g.x_mask = torch.cat([g.x_mask[:, 1:, :], torch.tensor(mask, dtype=torch.bool)], axis=1)
            
        predictions = np.stack(predictions, axis=2)  # n_h3_index, n_target, n_timestamps
        predictions = np.swapaxes(predictions, 1, 2)  # n_h3_index, n_timestamps, n_target
