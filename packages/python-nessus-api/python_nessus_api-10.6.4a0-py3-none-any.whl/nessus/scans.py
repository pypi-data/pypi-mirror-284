...
class ScansAPI(APIEndpoint):
    ...
    def attachment_pre(self, scan_id: int, attachment_id: int, key: str, fobj: BytesIO | None = None) -> BytesIO:
        """
        ...
        resp = self._get(f"{scan_id}/attachments/{attachment_id}/prepare", params={"key": key}, stream=True)
    ...
    def copy(self, scan_id: int, folder_id: int | None = None, name: str | None = None) -> dict:
        """
        ...
        Example:
            ...
            >>> nessus.scans.copy(1)
        """
        return self._post(f"{scan_id}/copy", json=dict_clean({"folder_id": folder_id, "name": name}))
    ...
    def delete_bulk(self, scan_ids: list[int]) -> list:
        """
    ...
    def export_formats(self, scan_id: int, schedule_id: int | None = None) -> dict:
        """
        ...
        """
        return self._get(f"{scan_id}/export/formats", params=dict_clean({"schedule_id": schedule_id}))

    def export_scan(
        self, scan_id: int, history_id: int | None = None, fobj: BytesIO | None = None, **kwargs
    ) -> BytesIO:
        """
        ...
        token = self._post(f"{scan_id}/export", params=dict_clean({"history_id": history_id}), json=payload)["token"]
        return self._api.tokens._fetch(token, **dlopts)

    def host_details(self, scan_id: int, host_id: int, history_id: int | None = None) -> dict:
        """
        ...
        Args:
            ...
            history_id (int): Id of the thistory object to retrieve

        Example:

            >>> nessus.scans.host_details(1,1,1)
        """
        return self._get(f"{scan_id}/hosts/{host_id}", params=dict_clean({"history_id": history_id}))
    ...
    def import_scan(
        ...
        """
        if not file_id:  # pragma: no branch
            file_id = self._api.files.upload(fobj)
        return self._post("import", json=dict_clean({"file": file_id, "folder_id": folder_id, "password": password}))
    ...
    def launch(self, scan_id: int, alt_targets: list[str] | None = None) -> str:
        """
        ...
        """
        return self._post(f"{scan_id}/launch", json=dict_clean({"alt_targets": alt_targets}))["scan_uuid"]

    def list(self, folder_id: int | None = None, last_modification_date: int | None = None) -> dict:
        """
        ...
        Example:
            ...
            ...     print(scan)
        """
        return self._get(params=dict_clean({"folder_id": folder_id, "last_modification_date": last_modification_date}))
    ...
    def plugin_output(self, scan_id: int, host_id: int, plugin_id: int, history_id: int | None = None) -> dict:
        """
        ...
        """
        return self._get(
            f"{scan_id}/hosts/{host_id}/plugins/{plugin_id}", params=dict_clean({"history_id": history_id})
        )
    ...
    def schedule(self, scan_id: int, enabled: bool) -> dict:
        ...
        """
        return self._put(f"{scan_id}/schedule", params={"enabled": str(enabled).lower()})
