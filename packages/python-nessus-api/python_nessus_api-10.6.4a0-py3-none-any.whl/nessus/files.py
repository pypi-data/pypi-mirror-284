...
class FilesAPI(APIEndpoint):
    ...
    def upload(self, fobj: BytesIO, encrypted: bool = False) -> str:
        ...
        """
        return self._post("upload", data={"no_enc": int(encrypted)}, files={"Filedata": fobj})["fileuploaded"]
    ...
    def _download(
        ...
        Args:
            ...
            fobj (BytesIO, optional): The file object to write to. If unspecified, an in-memory object will be created
                and returned
            chunk_size (int, optional):
                The chunk size to use when writing to the file object. The default is unspecified is ``1024`` bytes.
            stream_hook (Callable[Response, BytesIO, int], optional):
                If specified, the output will be passed to the stream hook instead of processing ourselves.
            hook_kwargs (Dict, optional):
                Any additional keyword arguments that should be passed on to the stream hook.
        ...
        if fobj is None:
            fobj = BytesIO()
        if hook_kwargs is None:  # pragma: no branch
            hook_kwargs = {}
