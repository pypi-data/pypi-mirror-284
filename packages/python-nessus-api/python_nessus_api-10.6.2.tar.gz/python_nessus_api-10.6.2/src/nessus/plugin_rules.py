...
class PluginRulesAPI(APIEndpoint):
    ...
    def create(
        self,
        plugin_id: int,
        type: Literal["recast_critical", "recast_high", "recast_medium", "recast_low", "recast_info", "exclude"],
        host: str | None = None,
        date: int | None = None,
    ) -> None:
        """
        ...
        Example:
            ...
            ... )
        """
        self._post(json={"plugin_id": str(plugin_id), "type": type, "host": host if host else "", "date": date})
    ...
    def edit(
        ...
        type: None | (
            Literal["recast_critical", "recast_high", "recast_medium", "recast_low", "recast_info", "exclude"]
        ) = None,
        host: str | None = None,
        date: int | None = None,
    ) -> None:
        ...
        """
        rule = self.details(1)
        payload = dict_merge(rule, dict_clean({"plugin_id": str(plugin_id), "type": type, "host": host, "date": date}))
