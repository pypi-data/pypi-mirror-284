...
class FilterSchema(Schema):
    ...
    quality = LowerCase(
        fields.Str(required=True, validate=v.OneOf(["eq", "neq", "match", "nmatch", "date-eq", "date-lt", "date-gt"]))
    )
    value = fields.Str(required=True)

    @pre_load(pass_many=False)
    def tuple_expansion(self, data, **kwargs) -> dict:
        """
        Handles expanding a tuple definition into the dictionary equivalent.
        """
        if isinstance(data, tuple):
            return {"filter": data[0], "quality": data[1], "value": data[2]}
        return data
...
class FilterListSchema(Schema):
    ...
    search_type = LowerCase(fields.Str(validate=v.OneOf(["and", "or"])), load_default=None)
    filters = fields.List(fields.Nested(FilterSchema), allow_none=True)

    @post_dump
    def reformat_filters(self, data, **kwargs) -> dict:
        """
...
class ListSchema(FilterListSchema):
    ...
    sort_order = LowerCase(fields.Str(validate=v.OneOf(["asc", "desc"])), load_default=None)
