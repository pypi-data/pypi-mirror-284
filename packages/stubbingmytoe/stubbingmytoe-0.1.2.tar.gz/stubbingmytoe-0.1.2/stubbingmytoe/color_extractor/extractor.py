...
def render_html(image_path, final, output_path='output.html'):
    with open('./color_extractor/template.txt', 'r') as file:
        template_str = file.read()
    template = Template(template_str)
    colors, counts = zip(*final)
    rendered_html = template.render(image_path=image_path, colors=colors, counts=counts)
...
def proccessImage(image_path, name, resolution=5, cycles=10, depth=100, multiplier=1):
    ...
    renderColors = final
    render_html(image_path, renderColors, output_path='output.html')
    plot_colors(final, f'/path/to/save/{name}/r{resolution}_c{cycles}_d{depth}_m{multiplier}.jpg')
