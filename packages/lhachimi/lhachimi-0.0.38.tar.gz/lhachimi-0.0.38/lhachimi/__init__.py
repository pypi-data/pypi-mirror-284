...
def plot(function,min,max,step=1,speed=0,verbose=False,color="blue",show=True,fig=0):
    x_values=[]
    y_values=[]
    i = min
...
def poly_plot(functions_list,steps_list,colors_list,min,max,distinct=False):
    if distinct:
        for i in range(len(functions_list)):
            plot(functions_list[i],min,max,step=steps_list[i],color=colors_list[i],show=False,fig=i+1)
            plt.legend()
        plt.show()
    else:
        for i in range(len(functions_list)):
            plot(functions_list[i],min,max,step=steps_list[i],color=colors_list[i],show=False)
            plt.legend() 
        plt.show()
