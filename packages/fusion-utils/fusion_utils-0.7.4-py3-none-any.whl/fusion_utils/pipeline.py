...
class Pipeline:
    ...
    def print_initial_summary(self):
        """
        Print an initial summary of tasks, stages, and other relevant information to the terminal.
        """
        print("\nInitial Pipeline Summary")
        print("========================")
        print(f"Pipeline Name: {self.name}")
        print(f"QA Mode: {'Enabled' if self.QA else 'Disabled'}")
        print(f"Total Tasks: {len(self.tasks)}")
        print(f"Total Stages: {len(set(task.stage for task in self.tasks))}")
        ...
        try:
            ...
            if not self.errors and not self.halt_execution:
                ...
                if qa_tasks and Task.QA_STAGE not in stages_executed:
                    print(f"Executing stage {Task.QA_STAGE} with {len(qa_tasks)} task(s).")
                    self.execute_stage(qa_tasks)
                    stages_executed.add(Task.QA_STAGE)
            else:
                print("Skipping QA tasks due to errors in regular tasks.")
    ...
    def send_completion_email(self):
        ...
        for task_name, updates in self.status.items():
            ...
            if any(task.name == task_name and task.is_qa for task in self.tasks):
