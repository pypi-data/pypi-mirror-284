...
class TruMusic:
    ...
    def run(self):
        ...
        Run process
        """
        if selected_item := self._get_selection(self._get_artist_albums()):
            ...
            num_tracks = len(selected_item.tracks)
            for index, track in enumerate(selected_item.tracks, 1):
                    ...
                    with YoutubeDL(self.ydl_opts) as ydl:
                        ...
                        while tries <= self.max_tries:
                            ...
                            try:
                                ...
                                self.logger.error(f"Try {tries}/{self.max_tries} failed: {track.get('title')}")
                                sleep(5)
                        if tries >= self.max_tries:
                            self.logger.error(f"Max tries reached, skipping: {track.get('title')}")
            self._cleanup()
            return True
        return False
