...
def leap(query, db, tables, description, verbose = False, udf = None, saving_mode=False):
    autopipeline.cost = 0
    ...
    if udf == None:
        precheck, functions = check_vague(query, tables, description, verbose, client)
    else:
        func_desc = ""
        for f in udf:
        ...
        precheck, functions = check_vague(query, tables, description, verbose, client, func_desc)
    ...
    require_new, feedback, result, table = pipeline_gen(query, db, tables, hint, description, verbose, client, udf, gpt4)
    if require_new:
