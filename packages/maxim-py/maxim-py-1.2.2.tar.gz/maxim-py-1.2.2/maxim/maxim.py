...
class Maxim:
    def __init__(self, config: Config):
        ...
        self.__syncThread = threading.Timer(60, self.__syncEntities)
    ...
    def __syncEntities(self):
        if self.__isDebug:
            print("[MaximSDK] Syncing prompts and folders")
        ...
        with self.mutex:
            ...
            self.__syncFolders()
            if self.__isDebug:
                print("[MaximSDK] Syncing completed")

    def __syncPrompts(self):
        if self.__isDebug:
            print("[MaximSDK] Syncing prompts")
        prompts = MaximAPI.getPrompts(self.baseUrl, self.apiKey)
        if self.__isDebug:
            print(f"[MaximSDK] Found {len(prompts)} prompts")
    ...
    def __syncFolders(self):
        if self.__isDebug:
            print("[MaximSDK] Syncing folders")
        folders = MaximAPI.getFolders(self.baseUrl, self.apiKey)
        if self.__isDebug:
            print(f"[MaximSDK] Found {len(folders)} folders")
