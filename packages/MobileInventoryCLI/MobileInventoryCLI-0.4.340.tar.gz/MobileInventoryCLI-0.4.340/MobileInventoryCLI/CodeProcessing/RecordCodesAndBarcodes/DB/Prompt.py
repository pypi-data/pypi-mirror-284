...
{Back.grey_35}:{Fore.light_green}{Back.grey_15} ''')
            ...
            def GetAsciiOnly(cmd):
                hws=f'{chr(27)}P{chr(27)}P'
                stripped=''
                if stripped.startswith(hws):
                   stripped=stripped[len(hws):]
                removed=[]
                for i in cmd:
                ...
                msg=f'''{Fore.light_green}{cmd.encode()} ->{Fore.light_salmon_1} stripped('{removed}').stripped('OPOP') -> {Fore.light_blue}cmd('{stripped}'){Style.reset}'''
                print(msg)
                return stripped
