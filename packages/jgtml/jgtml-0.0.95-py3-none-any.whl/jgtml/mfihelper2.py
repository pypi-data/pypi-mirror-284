...
def column_mfi_str_in_dataframe_to_id(df:pd.DataFrame,t:str,inplace=False):
    """
...
def column_mfi_str_back_to_str_in_dataframe(df:pd.DataFrame,t:str,inplace=False):
    if not inplace:
        df = df.copy()    
    mfi_str_selected_columns=get_mfi_features_column_list_by_timeframe(t)
    for col_name in mfi_str_selected_columns:
