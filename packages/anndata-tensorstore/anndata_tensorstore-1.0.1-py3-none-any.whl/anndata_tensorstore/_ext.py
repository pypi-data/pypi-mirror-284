...
def load_anndata_from_tensorstore(
    input_path: str, 
    row_indices: Union[slice, np.ndarray] = None,
    col_indices: Union[slice, np.ndarray] = None,
    selected_var_names: List[str] = None,
    ...
    :param row_indices: The row indices to load.
    :param col_indices: The column indices to load.
    :param selected_var_names: The variable names to load.
    """
    if selected_var_names is not None:
        var = pd.read_parquet(os.path.join(input_path, 'var.parquet'))
        col_indices = var.index.isin(selected_var_names)
