...
class CameraDelegate(metaclass=ABCMeta):
    @abstractmethod
    async def snapshotRequest(self, request_id: str) -> Union[bytes, None]: ...

    @abstractmethod
    async def startStream(self, request_id: str) -> None: ...

    @abstractmethod
    async def stopStream(self, request_id: str) -> None: ...

    @abstractmethod
    async def onTalkbackSdp(
        self, request_id: str, source_name: str, sdp: str, codec: TalkbackCodec
    ) -> None: ...

    @abstractmethod
    async def onTalkbackRtp(self, request_id: str, source_name: str, rtp: bytes) -> None: ...
...
class CameraControllerOptions(TypedDict):
    delegate: Optional[CameraDelegate]
    prebuffer_delegate: Optional[CameraPrebufferDelegate]


class CameraDelegateController(Protocol):
    def configure_controller(self, controller_options: CameraControllerOptions) -> None: ...
...
class CameraDevice(Protocol):
    ...
    camera_controller: CameraDelegateController
...
class CameraConfigWithoutSources(BaseCameraConfig):
...
CameraConfig = CameraConfigWithoutSources
