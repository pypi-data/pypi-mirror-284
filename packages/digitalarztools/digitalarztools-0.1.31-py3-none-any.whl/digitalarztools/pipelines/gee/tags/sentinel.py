...
class SentinelData:
    ...
    def get_water_level_using_change_detection_sentinel_1(region: GEERegion, water_mask: ee.Image,
        ...
        calibration_factor = 0.1  # Example value (replace with your calibrated value)
        ...
        # Apply the existing water_mask to focus on changes in water areas
        masked_difference = difference_image.updateMask(water_mask)

        # Convert backscatter difference to estimated water level change (in meters) for each pixel
        # relative_water_level_change_image = masked_difference.multiply(calibration_factor)
