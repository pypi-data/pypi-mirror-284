...
As a knowledge curation engine, STORM grabs information from the Retriever module. The Retriever modules are implemented in [`src/rm.py`](knowledge_storm/rm.py). Currently, STORM supports the following retrievers:
...
STORM provides the following language model implementations in [`src/lm.py`](knowledge_storm/lm.py):
