...
class PageFive(BasePage):
    ...
    def __init__(self, parent, controller):
        ...
        label = tk.Label(master=self.headframe, text = 'Addtional Support',font = controller.sub_title_font)
        label.pack(side="top", fill="x", pady=2)

        note = tk.Label(master=self.headframe, text = 'Instruction: This contains some feature which might not require information from VISION OSLO.',font = controller.text_font)
        note.pack(side="left", fill="x", pady=5)

        explain1 = tk.Label(master=self.optionframe, text = '1. Battery Train Assessment Support', font = controller.text_font)
        explain1.grid(row = 0, column = 0, sticky = "w", padx=5, pady=2)

        button1 = tk.Button(master=self.optionframe, text = 'execute 1', command=lambda: self.set_main_option("1","A01"))
        button1.grid(row = 0, column = 1, sticky = "w", padx=5, pady=2)

        explain2 = tk.Label(master=self.optionframe, text = '2: To be developed', font = controller.text_font)
        explain2.grid(row = 1, column = 0, sticky = "w", padx=5, pady=2)

        button2 = tk.Button(master=self.optionframe, text = 'execute 2', command=lambda: self.set_main_option("2","A02"))
        button2.grid(row = 1, column = 1, sticky = "w", padx=5, pady=2)

        explain3 = tk.Label(master=self.optionframe, text = '3: To be developed', font = controller.text_font)
        explain3.grid(row = 2, column = 0, sticky = "w", padx=5, pady=2)

        button3 = tk.Button(master=self.optionframe, text = 'execute 3', command=lambda: self.set_main_option("3","A03"))
        button3.grid(row = 2, column = 1, sticky = "w", padx=5, pady=2)
