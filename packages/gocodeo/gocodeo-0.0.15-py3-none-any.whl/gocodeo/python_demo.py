...
def explain_api_request(code):
    ...
    prompt =f"You are Python unittest unit testing expert. Your job is to first understand the given code {code} then explain the code so that every team member should understand the logic and working of the function"
    payload = {
...
def open_api_request(code, type, number, context):
    ...
    return message
def generate_api_request(code, context, type, number,src_path):

   
    prompt = f"""You are Python unittest unit testing expert. Your job is to write {number} numbers high quality unit test code  using jasmine and karma framework only, for given angular code: {code} for the given {type} scenarios in the context: {context}.
      ...
      You have to  make sure to import all the necessary dependencies at the top for execution of tests . 
       The source code file present at {src_path} location. 
...
def generate_tests_for_directory(directory):
    ...
    for root, dirs, files in os.walk(directory):
        for file_name in files:
            if file_name.endswith('.py') and not file_name.startswith('test_') and not os.path.basename(root) == 'gocodeo_tests':
...
def generate_tests(file_name):
    ...
    parent=os.path.dirname(file_path_from_app)

    relative_path = os.path.relpath(file_path_from_app, parent)
    
    
    # Query the database for the relative path
    document = collection.find_one({"file_path": relative_path})

    if document:
        test_content = document.get("test_content", "")
        # print(f"Test content for {file_path_from_app}:")
        # print(test_content)
        ...
        if File_name!="__init__.py":
            print("Generating Test Cases For File:",File_name)
            output_file_path = os.path.join(test_folder_path, f'test_{File_name}')
            ...
            output_file_path = os.path.join(test_folder_path, f'{File_name}')
            with open(output_file_path, 'w') as output_file:
                output_file.write(test_content)
                time.sleep(4)
                return
...
def generate_tests_cli():
    ...
    animation_thread = threading.Thread(target=start_animation, args=(10,))
