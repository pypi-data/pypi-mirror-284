...
class Channel(BaseChannel):
    ...
    def process_data(self, callback, message, to_tuple=False):
        if to_tuple:
    ...
    def process_data_events(self, to_tuple=False, auto_decode=True):
        ...
        for message in self.build_inbound_messages(break_on_empty=True,
            ...
            consumer_callback = self._consumer_callbacks.get(consumer_tag)

            if consumer_callback:
                self.process_data(callback=consumer_callback, message=message, to_tuple=to_tuple)
            else:
                self._cached_messages.setdefault(consumer_tag, []).append(message)
    ...
    def start_consuming(self, to_tuple=False, auto_decode=True):
        ...
        while not self.is_closed:
            ...
            for tag, messages in list(self._cached_messages.items()):
                if tag not in self._consumer_callbacks:
                    continue
                consumer_callback = self._consumer_callbacks[tag]
