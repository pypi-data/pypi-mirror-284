...
def set_wallpaper(image_path):
    """Set the wallpaper to the specified image path on macOS."""
    try:
        script = f"""
...
def change_wallpaper(tray_icon):
    ...
    if not rows:
        logging.info("No titles found in the database.")
        return 1, ""
    ...
    try:
        ...
        if not media_id:
            logging.info(f"No title found with the name: {title_name}")
            return 1, ""

        image_url = fetch_backdrop_image(media_id, media_type, api_key)
        if not image_url:
            logging.info(f"No backdrops found for the title: {title_name}")
            return 1, ""

        image_path = save_image(image_url, title_name)
        if not image_path:
            logging.info(f"Failed to save the image for the title: {title_name}")
            return 1, ""

        if set_wallpaper(image_path):
            return 0, title_name
        else:
            logging.info("Failed to set the wallpaper.")
            return 1, ""
