...
def mongo_find_query_v3_batch_size(collection, query, sort_field=None, sort_order=1, projection=None, limit=None, filter_query=None, batch_size=None, cursor_timeout=False):
    """
    在 MongoDB 中进行查询，并可选地应用排序和字段投影。
    ...
    :param limit (int, optional): 限制返回的文档数量。默认返回所有匹配的文档。
    :param filter_query (dict, optional): 过滤条件，用于指定复杂查询操作符。
    :param batch_size (int, optional): 批量大小,用于指定大批量数据查询。
    返回:
    :return: pymongo.cursor.Cursor: 查询结果的游标对象。
    """
    if filter_query:
    ...
    if cursor_timeout:
        cursor = cursor.no_cursor_timeout(True)
    return cursor
