...
class ServiceClient:
    ...
    @staticmethod
    def _validate_response(response: httpx.Response, method_name, only_version_check=False):
        ...
        if not only_version_check:
            ...
            try:
                if (
                    ...
                    relevant_reponse_text = reponse_split_up[1].split("debug_error_string")[
                        0
                    ]
                    if "ValueError" in relevant_reponse_text:
                        # Extract the ValueError message
                        value_error_msg = relevant_reponse_text.split("ValueError. Arguments: (")[1].split(",)")[0]
