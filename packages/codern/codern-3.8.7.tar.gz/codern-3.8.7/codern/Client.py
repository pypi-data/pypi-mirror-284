...
def inks(url):
    try:
        ...
        if response.status_code == 200:
            ...
            url = [link.get('href') for link in soup.find_all('a') if link.get('href') and link.get('href').endswith('.mp3')]
            try:return {'title':title, "link":random.choice(url)}
            except:...
    except:...
...
class api():
    ...
    def music(self,text:str,result_count:None = 5):
        sr=search(text, num_results=result_count)
        return [m for m in sr]