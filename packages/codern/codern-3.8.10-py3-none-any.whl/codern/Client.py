...
class api():
    ...
    def Upload_file(self,byte_file)-> any:
        ...
        return req
    def search_music(self,text:str,result_count:None = 5):
    ...
    def create_voice(
            self,
            text:str,
            type:Literal['FaridNeural', 'DilaraNeural'] = 'FaridNeural'
        ) -> None:
        request=urllib3.PoolManager()
        headers = {'Content-Type': 'application/json'}
        response=request.request(
            'POST',
            'https://api-free.ir/api2/voice.php',
            body=json.dumps({'token':self.token,'text':text,'mod':type}).encode('utf-8'),
            headers=headers
        )
