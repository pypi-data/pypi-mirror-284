...
def _seg_preprocess(msk: np.ndarray):  # list of bool ndarray
    detector = [sp := tuple(3 for _ in range(msk.ndim)),
...
@FuncTools.params_setting(data=T[Null: _is_mask_pair], in_spacing=T[None: Optional[list[Numeric]]],
                          scope_radius=T[None: Optional[Numeric]],
                          **{'~differential_tolerance': T[3: lambda x: isinstance(x, int) and x > 0]})
...
def vascular_invasion(**params):
    ...
    if True not in s:
    ...
    elif s.sum() in [v.sum(), v.sum()-1] or _is_cross(v, t, _tol):  # fully enclosed, internally tangent, cut-through
        res = {True: 2 * np.pi}
        ...
        if len(segs) == 1:  # single superposition region
            ...
            ref_c = _local_scope(v, t, sp, _scope)
            centroid = np.argwhere(ref_c).mean(axis=0)

            if _is_enclosed(v, t):  # almost enclosed
                _v1, _v2 = _points_pair(_diff_set(v, t))
                v1, v2 = _apply_spacing(_v1 - centroid, sp), _apply_spacing(_v2 - centroid, sp)
                _theta = np.arccos(np.dot(v1, v2) / np.prod(np.linalg.norm(np.array([v1, v2]), ord=2, axis=1)))
                theta = 2 * np.pi - _theta
            else:
                _v1, _v2 = _points_pair(segs[0])
                v1, v2 = _apply_spacing(_v1 - centroid, sp), _apply_spacing(_v2 - centroid, sp)
            ...
            _base = _diff_set(v, t)
            _res = [vascular_invasion(data=(v, _union_set(_, _base)), in_spacing=sp) for _ in segs]
