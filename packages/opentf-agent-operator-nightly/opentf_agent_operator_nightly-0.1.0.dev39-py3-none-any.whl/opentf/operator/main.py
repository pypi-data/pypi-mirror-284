...
def _patch_pools_live(name: str, body: Dict[str, Any]):
...
def _get_live_status(name: str, logger) -> Dict[str, Any]:
...
def _kill_orphan_agent(
    ...
    with PENDING_AGENTS_LOCK:
        ...
        _patch_pools_live(name, _create_body(CREATE_AGENTS_PATH, agents_patch))
        logger.debug(
...
def _remove_agent_and_pod(
    ...
    try:
        ...
        _patch_pools_live(resource_name, _create_body(AGENTS_PODS_PATH, None))
    except KeyError:
        pass
    _delete_exec_pod(pod_name, api_instance, logger)


def handle_agent_command(stop_event, name, spec):
    ...
    logger.info('Starting agent commands handling thread...')
    config.load_incluster_config()
    api_instance = client.CoreV1Api()
    while not stop_event.is_set():
        ...
        try:
            ...
            status = _get_live_status(name, logger)
            ...
            if command['kind'] == 'exec' and ('-2.sh' in command['command']):
                ...
                if not _maybe_get_agent_command(agent_id, spec, logger):
                    _remove_agent_and_pod(
                        agent_id, resource_id, name, pod_name, api_instance, logger
                    )
                    continue
    ...
    logger.info('Stopping agent commands handling thread...')


def monitor_agents(stop_event, name, spec):
    ...
    logger.info('Starting agents monitoring thread...')
    ...
    logger.info('Stopping agents monitoring thread...')
...
def _start_threads(name: str, spec: Dict[str, Any]):
    monitor_stop_event = threading.Event()
    handle_stop_event = threading.Event()

    monitor_thread = threading.Thread(
        target=monitor_agents, args=(monitor_stop_event, name, spec)
    )
    monitor_thread.start()

    handle_thread = threading.Thread(
        target=handle_agent_command, args=(handle_stop_event, name, spec)
    )
    handle_thread.start()
...
def _stop_threads(name: str):
    if name in POOLS_THREADS:
        thread = POOLS_THREADS.pop(name)
...
def delete_agents(name, spec, status, logger, **kwargs):
    try:
        agents, pod = (
            _get_param_or_fail(
                status,
        ...
        _attempt_agent_removal(
            agents, name, resource_id, pod, _make_headers(token), logger
        )
    ...
    try:
        ...
        for agent in otf_busy_agents:
            ...
            if agent in kube_agents_pods:
                _delete_exec_pod(kube_agents_pods[agent], api_instance, logger)
        _patch_pools_live(name, _create_body(AGENTS_PODS_PATH, None))
...
def cleanup_threads(**kwargs):
    try:
        ...
        logger.error(f'Threads cleanup failed: {err}.')
        ...
        if action == 'change' and (current_pool != new_pool):
            ...
            if current_pool < new_pool:
                ...
                if kill <= len(idle_agents):
                    ...
                    _attempt_agent_removal(
                        current_agents, name, resource_id, pod, headers, logger
                    )
    except Exception as err:
...
def update_tags(diff, spec, status, name, logger, **kwargs):
    try:
