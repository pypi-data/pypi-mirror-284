...
def config_get():
    ...
    typer.echo(__('THESTAGE TOKEN: %token%', {'token': config.main.auth_token}))
    typer.echo(__('THESTAGE CONFIG FILE: %dir%', {'dir': config.main.config_file_name}))
    typer.echo(__('THESTAGE API LINK: %link%', {'link': config.main.config_api_link}))
    if config.main.config_local_path:
        typer.echo(__('THESTAGE LOCAL CONFIG PATH: %path%', {'path': str(config.main.config_local_path)}))
    if config.main.config_global_path:
        typer.echo(__('THESTAGE GLOBAL CONFIG PATH: %path%', {'path': str(config.main.config_global_path)}))
