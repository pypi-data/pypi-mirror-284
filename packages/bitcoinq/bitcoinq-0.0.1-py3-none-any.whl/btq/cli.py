...
from btq.core.Wallet import Wallet, WalletDecryptionError
from btq.core.misc.helper import parse_hexblob, parse_qaddress
from btq.core.MultiSigAddressState import MultiSigAddressState
...
from btq.crypto.xmss import XMSS, hash_functions
from btq.generated import btq_pb2_grpc, btq_pb2
...
class CLIContext(object):
    ...
    def get_stub_public_api(self):
        ...
        return btq_pb2_grpc.PublicAPIStub(channel)
...
def _public_get_address_balance(ctx, address):
    ...
    get_address_state_req = btq_pb2.GetAddressStateReq(address=parse_qaddress(address))
...
@click.version_option(version=config.dev.version, prog_name='BTQ Command Line Interface')
@click.group()
...
@click.pass_context
def btq(ctx, verbose, host, port_pub, wallet_dir, json):
    """
...
@btq.command(name='wallet_recover')
...
@btq.command(name='wallet_secret')
...
@btq.command(name='wallet_encrypt')
@click.pass_context
...
@btq.command(name='wallet_decrypt')
@click.pass_context
...
def tx_push(ctx, txblob):
    ...
    stub = ctx.obj.get_stub_public_api()
    pushTransactionReq = btq_pb2.PushTransactionReq(transaction_signed=tx.pbdata)
...
@btq.command(name='tx_message')
@click.option('--src', type=str, default='', prompt=True, help='signer BTQ address')
@click.option('--master', type=str, default='', prompt=True, help='master BTQ address')
@click.option('--addr_to', type=str, default='', prompt=True, help='BTQ Address receiving this message (optional)')
...
def tx_message(ctx, src, master, addr_to, message, fee, ots_key_index):
    ...
    try:
        ...
        push_transaction_req = btq_pb2.PushTransactionReq(transaction_signed=tx.pbdata)
...
@btq.command(name='tx_multi_sig_create')
@click.option('--src', type=str, default='', prompt=True, help='source BTQ address')
@click.option('--master', type=str, default='', prompt=True, help='master BTQ address')
...
@btq.command(name='tx_multi_sig_spend')
@click.option('--src', type=str, default='', prompt=True, help='signer BTQ address')
@click.option('--master', type=str, default='', help='master BTQ address')
...
def tx_token(ctx, src, master, symbol, name, owner, decimals, fee, ots_key_index):
    ...
    while True:
        ...
        initial_balances.append(btq_pb2.AddressAmount(address=parse_qaddress(address),
                                                      amount=amount))
...
@btq.command(name='tx_transfertoken')
@click.option('--src', type=str, default='', prompt=True, help='source BTQ address')
@click.option('--master', type=str, default='', prompt=True, help='master BTQ address')
...
@btq.command(name='slave_tx_generate')
...
def slave_tx_generate(ctx, src, master, number_of_slaves, access_type, fee, pk, ots_key_index):
    ...
    try:
        ...
        click.echo('Move slaves.json file from current directory to the mining node inside ~/.btq/')
    except Exception as e:
        click.echo("Unhandled error: {}".format(str(e)))
        quit(1)


@btq.command(name='token_list')
@click.option('--owner', default='', prompt=True, help='source BTQ address')
@click.pass_context
def token_list(ctx, owner):
    ...
    try:
        stub = ctx.obj.get_stub_public_api()
        address_state_req = btq_pb2.GetAddressStateReq(address=owner_address)
        ...
        for token_hash in address_state_resp.state.tokens:
            get_object_req = btq_pb2.GetObjectReq(query=bytes(hstr2bin(token_hash)))
...
def state(ctx):
    ...
    """
    stub = ctx.obj.get_stub_public_api()
    nodeStateResp = stub.GetNodeState(btq_pb2.GetNodeStateReq())
