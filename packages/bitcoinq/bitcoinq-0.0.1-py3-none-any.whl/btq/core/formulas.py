...
def remaining_emission(block_n, dev_config: DevConfig) -> Decimal:
    # TODO: This is more related to the way BTQ works.. Move to another place
    """
...
def block_reward(block_number: int, dev_config: DevConfig) -> Decimal:
    ...
    if block_number < dev_config.hard_fork_heights[1]:
     ...
     # This calculates the emission-based reward component
    emission_reward = factor * (remaining_emission(block_number - 1, dev_config) - remaining_emission(block_number, dev_config))

    # Now integrate the decrease-based calculation
    decrease_interval = 525600  # 525600 Block = 525600 Mins (Block Time Avarage: 1 Mins)

    # Calculate how many decrease have occurred
    decrease = block_number // decrease_interval

    # Adjust the reward by decrease it for each interval that has passed
    reward = emission_reward * (Decimal('0.95') ** decrease)
