...
from btq import __version__ as version
...
class UserConfig(object):
    ...
    def __init__(self, ignore_check=False):
        ...
        self.genesis_prev_headerhash = b'Mainnet'
        self.genesis_timestamp = 1713628989
        ...
        self.peer_list = ['66.29.133.207', '192.64.118.237', '199.188.203.201', '162.254.32.51']

        self.p2p_local_port = 19001  # Locally binded port at which node will listen for connection
        self.p2p_public_port = 19019  # Public port forwarding connections to server
        ...
        self._btq_dir = expanduser(os.path.join("~/.btq"))
    ...
    @property
    def btq_dir(self):
        return self._btq_dir

    @btq_dir.setter
    def btq_dir(self, new_btq_dir):
        self._btq_dir = new_btq_dir
        self.load_yaml(self.config_path)

    @property
    def wallet_dir(self):
        return expanduser(self.btq_dir)

    @property
    def data_dir(self):
        return expanduser(os.path.join(self.btq_dir, "data"))

    @property
    def config_path(self):
        return expanduser(os.path.join(self.btq_dir, "config.yml"))

    @property
    def log_path(self):
        return expanduser(os.path.join(self.btq_dir, "btq.log"))

    @property
    def walletd_log_path(self):
        return expanduser(os.path.join(self.btq_dir, "walletd.log"))

    @property
    def mining_pool_payment_wallet_path(self):
        return expanduser(os.path.join(self.btq_dir, 'payment_slaves.json'))
...
class DevConfig(object):
    ...
    def __init__(self, pbdata, ignore_check=False, ignore_singleton=False):
        ...
        self.slave_dat_filename = 'slave.btq'
        self.banned_peers_filename = 'banned_peers.btq'

        self.trust_min_msgcount = 10
        self.trust_min_conntime = 10

        self.supplied_coins = 3000000 * (10 ** 9)
    ...
    def coin_remaining_at_genesis(self):
        return decimal.Decimal(self.pbdata.chain.max_coin_supply - 3000000)
    ...
    def create(prev_state_key,
        ...
        chain = btq_pb2.DevConfig.Chain(reorg_limit=300,
                                        max_coin_supply=21000000,
                                        complete_emission_time_span_in_years=1000) #1000 years and 1.1 Multiplier HashRate
        block_size_controller = btq_pb2.DevConfig.Block.BlockSizeController(number_of_blocks_analyze=10,
                                                                            size_multiplier=110,
                                                                            block_min_size_limit_in_bytes=1024 * 1024)
        block = btq_pb2.DevConfig.Block(mining_nonce_offset=39,
                                        extra_nonce_offset=43,
                                        mining_blob_size_in_bytes=76,
                                        block_timing_in_seconds=60,
                                        block_size_controller=block_size_controller)

        message = btq_pb2.DevConfig.Transaction.Message(max_length=80)

        slave = btq_pb2.DevConfig.Transaction.Slave(slave_pk_max_length=67)

        token = btq_pb2.DevConfig.Transaction.Token(symbol_max_length=10,
                                                    name_max_length=30)

        lattice = btq_pb2.DevConfig.Transaction.Lattice(pk1_max_length=1088,
                                                        pk2_max_length=1472,
                                                        pk3_max_length=65)

        foundation_multi_sig = btq_pb2.DevConfig.Transaction.FoundationMultiSig(threshold_percentage=10)

        # proposal_config_activation_delay should not be less than reorg_limit
        proposal = btq_pb2.DevConfig.Transaction.Proposal(threshold_per=51,
        ...
        transaction = btq_pb2.DevConfig.Transaction(multi_output_limit=100,
                                                    message=message,
                                                    slave=slave,
                                                    token=token,
                                                    lattice=lattice,
        ...
        dev_config = btq_pb2.DevConfig(chain=chain,
                                       block=block,
                                       transaction=transaction,
                                       pow=btq_pb2.DevConfig.POW(N_measurement=30, kp=5)) #N_measurement = 30 KP=5
