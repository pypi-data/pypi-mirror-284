...
from btq.core.misc import logger, db
...
class State:
    ...
    def put_fork_state(self, fork_state: btqstateinfo_pb2.ForkState, batch=None):
        self._db.put_raw(b'fork_state', fork_state.SerializeToString(), batch)

    def get_fork_state(self) -> Optional[btqstateinfo_pb2.ForkState]:
        try:
            data = self._db.get_raw(b'fork_state')
            fork_state = btqstateinfo_pb2.ForkState()
    ...
    def get_slave_pk_access_type(self, address: bytes, slave_pk: bytes) -> btq_pb2.SlaveMetadata:
        ...
        try:
            slave_metadata = btq_pb2.SlaveMetadata()
    ...
    def get_token(self, address: bytes, token_txhash: bytes) -> btq_pb2.TokenBalance:
        try:
            token_balance = btq_pb2.TokenBalance()
