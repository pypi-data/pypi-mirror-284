...
class GDI:
    ...
    def draw_random_scribble():
        """
        Drawing of random scribbles on the screen.
        """
        hdc = win32gui.GetDC(0)
        user32 = ctypes.windll.user32
        user32.SetProcessDPIAware()
        ...
        try:
            while True:
                ...
                win32gui.MoveToEx(hdc, points[0][0], points[0][1])
                for point in points[1:]:
                    win32gui.LineTo(hdc, point[0], point[1])
    ...
    def type_text(text):
        """
        Display text at random positions on the screen
        ...
        while True:
            ...
            win32gui.ExtTextOut(hdc, x, y, win32con.ETO_CLIPPED, None, str(text), None)
            time.sleep(0.3)


    def flip_screen_upside_down():
        """
        Continuously flip the computer screen upside down.
        """
        user32 = ctypes.windll.user32
        user32.SetProcessDPIAware()
        hwnd = win32gui.GetDesktopWindow()
        ...
        hdc = win32gui.GetDC(hwnd)
        while True:
            win32gui.StretchBlt(hdc, 0, 0, sw, sh, hdc, 0, sh, sw, -sh, win32con.SRCCOPY)
            time.sleep(0.1)
