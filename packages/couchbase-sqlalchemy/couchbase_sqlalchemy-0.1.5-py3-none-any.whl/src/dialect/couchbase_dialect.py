...
class CouchbaseDialect(DefaultDialect):
    ...
    def get_schema_names(self, connection, **kw):
        ...
        schemas = []
        for r in result:
            schemas.append((r['DatabaseName'][1:]+'.'+ r['DataverseName'][1:]))
        return schemas
    ...
    def get_view_names(self, connection, schema=None, **kw):        
        database_name, dataverse_name = schema.split('.')
        print("Database Name ",database_name," ",dataverse_name)
        query = """
                SELECT ds.DatasetName
                FROM Metadata.`Dataset` ds
                JOIN Metadata.`Datatype` dt
                ON ds.DatatypeDataverseName = dt.DataverseName
                AND ds.DatasetType='VIEW'
                AND ds.DatatypeName = dt.DatatypeName
                AND array_length(dt.Derived.Record.Fields) > 0
                AND ds.DataverseName = '{dataverse_name}'
                AND ds.DatabaseName = '{database_name}';
                """.format(database_name=database_name, dataverse_name=dataverse_name)
        result = connection.execute(query)
        views = [row['DatasetName'][1:] for row in result]
        return views
