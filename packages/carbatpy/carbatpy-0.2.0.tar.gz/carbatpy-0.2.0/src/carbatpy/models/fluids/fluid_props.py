...
if carbatpy._TREND["TREND_INSTALLED"]:
    trend_dll = carbatpy._TREND["TREND_DLL"]
    trend_path = carbatpy._TREND["TREND_PATH"]
    try:
...
_TRANS_TREND = copy.copy(_TRANS_TREND.extend(["ETA", "TCX", "WS"]))
_TV_STRING = "T;V"
...
class Fluid:
    ...
    def set_composition(self, composition):
        ...
        if self.fluidmodel.props == "TREND":
            ...
            self.fluidmodel.rp_instance.set_moles(self.composition)
            # here the mass fraction to internal mol fraction conversion
    ...
    def set_state(self, values, given="TP",
                  ...
                  composition=[]):
        if composition == []:  # mole fractions only!
        ...
        if self.props == "TREND":
            ...
            if verbose:
                print("comp-set", composition, self.composition, self.comp_mass,
                      self.comp_mole)
        ...
        if self.fluidmodel.props == "REFPROP":
            ...
            if quality_in > 0:
                if pressure_in >= 0:
                    ...
                    match values[quality_in]:
                        ...
                        case _:
                            raise Exception(
                                'Error: only qualities 0 or 1 for mixtures !')
                    values[0] = values[pressure_in]
                    values[1] = 10

                elif temp_in >= 0:

                    match value[quality_in]:
                        case 0.:
                            given = "TLIQ"
            ...
            all_values = self.fluidmodel.rp_instance.ALLPROP(given, values[0],
                                                             values[1])
