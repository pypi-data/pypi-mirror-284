...
def usaggregate(data, level, agg_numeric='mean', agg_character='first'):
    ...
    def aggregate_dataframe(df, level):
        ...
        highest_granularity = min(geo_columns.keys(), key=lambda k: level_hierarchy[k], default=None)
        
        if highest_granularity is None:
        ...
        char_cols = df.select_dtypes(include=['object']).columns.difference([geo_id_col, 'year'])
