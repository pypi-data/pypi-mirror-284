...
def usaggregate(data, level, agg_numeric='mean', agg_character='first'):
    ...
    def aggregate_dataframe(df, level):
        ...
        if agg_numeric == 'mean':
            numeric_agg = df.groupby([geo_id_col, 'year'], drop=True)[numeric_cols].mean()
        elif agg_numeric == 'median':
            numeric_agg = df.groupby([geo_id_col, 'year'], drop=True)[numeric_cols].median()
        elif agg_numeric == 'mode':
            numeric_agg = df.groupby([geo_id_col, 'year'], drop=True)[numeric_cols].agg(lambda x: x.mode()[0])
        elif agg_numeric == 'sum':
            numeric_agg = df.groupby([geo_id_col, 'year'], drop=True)[numeric_cols].sum()
        elif agg_numeric == 'first':
            numeric_agg = df.groupby([geo_id_col, 'year'], drop=True)[numeric_cols].first()
        elif agg_numeric == 'last':
            numeric_agg = df.groupby([geo_id_col, 'year'], drop=True)[numeric_cols].last()
        else:
            raise ValueError("agg_numeric must be 'mean', 'median', 'mode', 'sum', 'first', or 'last'.")
        
        if agg_character == 'first':
            char_agg = df.groupby([geo_id_col, 'year'], drop=True)[char_cols].first()
        elif agg_character == 'last':
            char_agg = df.groupby([geo_id_col, 'year'], drop=True)[char_cols].last()
        elif agg_character == 'mode':
            char_agg = df.groupby([geo_id_col, 'year'], drop=True)[char_cols].agg(lambda x: x.mode()[0])
        else:
