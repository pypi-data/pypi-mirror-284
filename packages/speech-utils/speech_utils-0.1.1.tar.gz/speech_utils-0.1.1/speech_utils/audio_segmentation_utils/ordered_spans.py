...
from speech_utils.data_models.time_spans import TimeSpan, TTimeSpan_co

logger = logging.getLogger(__name__)


class OrderedSpans(tuple[TTimeSpan_co, ...]):
    __slots__ = ()

    def __new__(cls, __iterable: Iterable[TTimeSpan_co]):  # noqa: PYI063
    ...
    def create_dont_validate(
        cls,
        time_spans: NeList[TTimeSpan_co],
    ) -> Self:
        """
    ...
    def parse(
        cls,
        time_spans: NeSequence[TTimeSpan_co],
...
class OrderedByMergingNeighbouringAndDroppingOutliers(OrderedSpans[TTimeSpan_co]):
    """
    no full-fledged sorting here! only merging
    """

    @classmethod
    def parse(cls, time_spans: NeSequence[TTimeSpan_co]) -> Self:
        order_merged: list[TTimeSpan_co] = []
        for seg in time_spans:
            ...
            if not order_merged:
                ...
                order_merged.append(seg)
        order_merged_dropped = OrderedByDroppingSpans[TTimeSpan_co].parse(order_merged)
...
class OrderedByDroppingSpans(OrderedSpans[TTimeSpan_co]):
    """
...
class SortedSpans(OrderedSpans[TTimeSpan_co]):
    @classmethod
    def parse(
        cls,
        time_spans: NeSequence[TTimeSpan_co],
...
class OrderedOverlappingSpans(tuple[TTimeSpan_co, ...]):  # noqa: SLOT001
    def __new__(cls, __iterable: Iterable[TTimeSpan_co]):  # noqa: PYI063
        sequence = tuple(__iterable)
        if len(sequence) == 0:
            raise ValueError("empty sequence")  # noqa: EM101, TRY003
    ...
    def parse(
        cls,
        time_spans: OrderedSpans[TTimeSpan_co] | NeSequence[TTimeSpan_co],
        ...
        if not isinstance(time_spans, OrderedSpans):
            time_spans = OrderedByDroppingSpans[TTimeSpan_co].parse(time_spans)
        filtered_segments = [
            seg
            for k, seg in enumerate(time_spans)
...
def segment_starts_are_weakly_monoton_increasing(seq: Sequence[TimeSpan]) -> bool:
    """
...
def segment_end_are_monoton_increasing(seq: Sequence[TimeSpan]) -> bool:
    """
    they can still be overlapping
    """
