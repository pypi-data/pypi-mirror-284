...
def is_full_day_closed(exchange: str = "KRX", now: dtm.datetime = None) -> bool:
    """
    주식 거래소가 휴장일인지 확인합니다.
...
def is_before_opening(exchange: str = "KRX", now: dtm.datetime = None):
    """
...
def is_after_closing(exchange: str = "KRX", now: dtm.datetime = None):
    """
...
def is_trading_time(exchange: str = "KRX", now: dtm.datetime = None):
    """
    ...
    return not (is_full_day_closed(exchange, now) or is_before_opening(exchange, now) or is_after_closing(exchange, now))
