...
class Station:
    ...
    def save_fast_delivery(self, din_path: str, path: str, is_test_mode: bool = False, merge: bool = True,
        ...
        for month in self.month_collection:
            ...
            # Because sometimes the station might not have the sensor specified in the din file
            ...
            if month_end + 1 > 12:
                ...
                year_end = year + 1
            if is_hourly:
                data_hr = data_obj_primary.copy()
                data_hr['hou']['time'] = np.array(data_obj_primary['hou']['time'])
                data_hr['hou']['sealevel'] = np.array(data_obj_primary['hou']['sealevel'])
                hourly_merged = filt.channel_merge(data_hr, ch_params)
            else:
                # Hourly data based only on one primary sensor, saved in the output folder
                data_hr = filt.hr_process(data_obj_primary, datetime(year, month.month, 1, 0, 0, 0),
                                          datetime(year_end, month_end + 1, 1, 0, 0, 0))
                # Merged hourly data based on all the sensors specified in the din file
                # Used to generate daily data, fills in empty data
                data_hr_all = filt.hr_process(data_obj, datetime(year, month.month, 1, 0, 0, 0),
                                              datetime(year_end, month_end + 1, 1, 0, 0, 0))
                if merge:
                    hourly_merged = filt.channel_merge(data_hr_all, ch_params)
                    data_hr = data_hr_all.copy()
                else:
                    hourly_merged = filt.channel_merge(data_hr, ch_params)
            ...
            # int -> channel name mapping is inside filtering.py var_flag function
            ...
            if is_hourly:
                hr_flat = data_hr[reliable_sensor.lower()]['sealevel'].flatten()
            else:
                hr_flat = np.concatenate(data_hr[reliable_sensor.lower()]['sealevel'], axis=0)
