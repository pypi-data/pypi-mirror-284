...
class SourceStatsList(BaseModel):
    source_stats_list: list[SourceStats] = Field(default_factory=list)
...
class AirfoldApi:
    ...
    def get_metrics(
        ...
        if res.ok:
            ...
            if json_compact:
                decoded_metrics: dict[str, Any] = compact_format_to_dict(metrics_of_sources, SourceStatsList.schema())
                return {sm["name"]: sm for sm in decoded_metrics["source_stats_list"]}
