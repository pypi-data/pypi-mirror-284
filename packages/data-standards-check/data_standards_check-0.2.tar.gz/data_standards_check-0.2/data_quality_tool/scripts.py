...
def check_and_export(data_folder_path, log_file_path):
    # Load data standards
    standard_df_path = [os.path.join(data_folder_path, f) for f in os.listdir(data_folder_path) if f.endswith('.csv') and 'STANDARD' in f.upper()][0]
    standard_df   = pd.read_csv(standard_df_path)

    # Define folder path and Load new data
    all_files = [os.path.join(data_folder_path, f) for f in os.listdir(data_folder_path) if f.endswith('.csv') and 'STANDARD' not in f.upper()]
    # print(all_files)
...
def generate_standard_file(data_folder_path, input_path=None):
    if input_path:
        ...
        for column in data_df.columns:
            data_type = str(data_df[column].dtype)
            sample_values = ','.join(map(str, data_df[column].dropna().unique()[:10]))  # Get up to 10 unique values
            standards.append({'Variable Name': column, 'Data Type': data_type, 'Sample Values': sample_values})
        ...
        standards = []
        while True:
            variable_name = input("Enter variable name (or 'done' to finish): ").strip()
            if variable_name.lower() == 'done':
                break
            data_type = input("Enter data type: ").strip()
            sample_values = input("Enter sample values (comma-separated): ").strip()
            standards.append({'Variable Name': variable_name, 'Data Type': data_type, 'Sample Values': sample_values})

        standards_df = pd.DataFrame(standards)

    # Save to CSV file in the data folder
    output_path = os.path.join(data_folder_path, 'data_value_standard.csv')
    standards_df.to_csv(output_path, index=False)
    print(f"Standard file generated: {output_path}")