...
def display_claim_summary(claim_summary, payer_id):
    ...
    # Table header
    table_header = "{:<10} | {:<10} | {:<20} | {:<10} | {:<10} | {:<8} | {:<8} | {:<8} | {:<8}".format(
        "Claim #", "Status", "Patient", "Processed", "Service", "Charged", "Allowed", "Paid", "Pt Resp")
    print(table_header)
    ...
    for claim_number, claim_data_list in sorted_claims:
        ...
        if len(claim_data_list) > 1:
            ...
            if len(unique_claims) == 1:
                ...
                table_row = "{:<10} | {:<10} | {:<20} | {:<10} | {:<10} | {:<8} | {:<8} | {:<8} | {:<8}".format(
                    claim_number, claim_data['claim_status'], claim_data['patient_name'], 
                    claim_data['processed_date'], claim_data['first_service_date'],
                    claim_data['total_charged_amount'], claim_data['total_allowed_amount'], 
                    claim_data['total_paid_amount'], claim_data['total_patient_responsibility_amount']
                )
                print(table_row)
