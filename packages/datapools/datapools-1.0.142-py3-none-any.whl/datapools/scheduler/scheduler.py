...
class CrawlerScheduler(Stoppable):
    ...
    async def _maybe_hard_stop(self, session: Session, meta: Optional[dict] = None, task: Optional[WorkerTask] = None):
        if meta is None:
    ...
    async def _process_iteration(self, session_id):
        ...
        if session is None:
            return False
        await self._maybe_hard_stop(session)
        ...
        if task.status in (
            ...
            if not self.cfg.CLI_MODE:
                ...
                if (
                    ...
                    hard_stop = await self._maybe_hard_stop(session, meta, task)
