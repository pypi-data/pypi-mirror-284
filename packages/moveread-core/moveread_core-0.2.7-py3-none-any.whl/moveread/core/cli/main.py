...
Verbose = typer.Option(False, callback=lambda _: STATE['verbose'], hidden=True)
...
def dump(
  ...
  P.run(inp_core.dump)(out_core, overwrite=overwrite, logstream=sys.stderr if verbose else None) # type: ignore
...
def export_callback(
  ...
  if glob:
    STATE['cores'] = core.glob(glob, recursive=recursive, err_stream=sys.stderr if verbose else None)
    return
