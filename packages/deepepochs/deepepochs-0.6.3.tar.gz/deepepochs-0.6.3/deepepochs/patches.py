...
3. 内置Patch
    - PatchBase
        ...
        - patches.ConfusionPatch:   参数为 preds和targets，存储混淆矩阵并计算基于混淆矩阵的指标
4. 定制Patch
    - 定制用于计算指标的Patch可继承MetricPatch，实现load方法、forward方法、add方法
    - 确保新的Patch的 __init__ 方法参数列表为 (metric, batch_size, name)
    - 确保新的Patch的 load 方法参数列表为 (batch_preds, batch_targets)
    - add方法用于基于利用sum基于多个mini-batch的Patch计算Epoch的Patch
"""
import abc
...
class PatchBase(abc.ABC):
    ...
    def __add(self, obj):
        if obj == 0:
            # 如是TensorPatch则返回自身，否则深复制一份返回（否则ValuePatch和MeanPatch会计算错误）
