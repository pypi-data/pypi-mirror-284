...
class DefaultCallback(Callback):
    ...
    def make_patch_dict(self, trainer, loss, model_out, batch_y, metrics, stage):
        ...
        # loss_patch = ValuePatch(b_size)     # 创建ValuePatch
        # loss_patch.load(loss)               # 装入数据
        # patch_dict = {} if loss is None else  {'loss': loss_patch}
        # for m in metrics:
        #     m_patch = trainer.metric_patch(m, batch_size=b_size)    # 创建Patch
        #     m_patch.load(model_out, batch_y)                        # 装入数据
        #     patch_dict[m.__name__] = m_patch
        # return patch_dict
        ...
        for m in metrics:
            # 创建Patch并装入数据
            patch_dict[m.__name__] = trainer.metric_patch(m, batch_size=b_size).load(model_out, batch_y)
        return patch_dict
