...
class CommerceApi(ApiClient):
    ...
    def get_coupons_by_creation_date_page(self, start_date=None,
                              end_date=None, page=1, limit=100):
        if not start_date:
            start_date = self.today() - timedelta(days=1)
        if not end_date:
        ...
        yield from self.coupon_search(params, page, limit)
    ...
    def coupons_by_creation_date(
            self,
            start_date=None,
            end_date=None, limit=100):
        for page in range(1, 100):
            log.info('[%s => %s] page %s', start_date, end_date, page)
            results = list(self.get_coupons_by_creation_date_page(
                start_date, end_date, page, limit
            ))
            log.info('%s results', len(results))
            if results:
    ...
    def get_coupons_by_modified_date_page(self,
                                          start_date=None,
                                          end_date=None, page=1, limit=100):
        if not start_date:
            start_date = self.today() - timedelta(days=1)
    ...
    def coupons_by_modified_date(
            self,
            start_date=None,
            end_date=None, limit=100):
        for page in range(1, 100):
            results = list(self.get_coupons_by_modified_date_page(
                start_date, end_date, page, limit
            ))
            if results:
                yield from results
    ...
    def coupon_search(self, params, page, limit):
        ...
        coupon_search_data = self.post(url, json_data=params)
        return coupon_search_data
