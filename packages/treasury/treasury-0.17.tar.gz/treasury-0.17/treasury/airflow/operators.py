...
class TreasuryCouponUnload(BaseOperator):
    ...
    def get_coupons(self, output_filename, context):
        from .hooks import TreasuryHook
        hook = TreasuryHook(self.treasury_conn_id)
        self.log.info('looking for coupons created between %s => %s', self.start_dttm, self.end_dttm)
        c1 = hook.api.coupons_by_creation_date(self.start_dttm, self.end_dttm)
        self.log.info('looking for coupons modified between %s => %s', self.start_dttm, self.end_dttm)
        c2 = hook.api.coupons_by_modified_date(self.start_dttm, self.end_dttm)
        rg = []
        codes = set()
