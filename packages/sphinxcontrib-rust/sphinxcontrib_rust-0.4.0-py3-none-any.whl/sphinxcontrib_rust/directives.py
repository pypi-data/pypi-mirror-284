...
class RustDirective(ABC, ObjectDescription[Sequence[str]]):
    ...
    def add_target_and_index(
        ...
        # Add to the domain
        self.env.domains["rust"].items[self.rust_item_type].append(item)
...
class RustUseDirective(SphinxDirective):
    """Directive for handling the use statements in the file.

    When the directive is encountered, all the options to the directive
    are added to the domain data for the document as potential links.

    This is not an object description and does not inherit from
    :py:class:`RustDirective`. It also has no content.
    """

    has_content = False
    optional_arguments = 999

    def group_arguments(self) -> list[tuple[str, str]]:
        """Collect data into non-overlapping fixed-length chunks or blocks."""
        iterators = [iter(self.arguments)] * 2
    ...
    def run(self):
        ...
        doc_uses = {
            target.strip(":"): rust_path for target, rust_path in self.group_arguments()
        }

        domain.uses[self.env.docname].update(doc_uses)
        return []
