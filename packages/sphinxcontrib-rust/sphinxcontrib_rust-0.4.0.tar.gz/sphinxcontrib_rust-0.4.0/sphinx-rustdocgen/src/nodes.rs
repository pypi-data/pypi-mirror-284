...
pub(crate) enum Node {
    ...
    Link { link_text: String, target: String },
        ...
        S: Serializer,
    {
        match self {
            ...
            Node::Link {
                ...
                map.serialize_entry("value", link_text)?;
    ...
    match ty {
        ...
        Type::BareFn(f) => format!(
            ...
            f.inputs
                ...
                .collect::<Vec<String>>()
                .join(", "),
            if let ReturnType::Type(_, r_ty) = &f.output {
...
fn nodes_for_path_segment(segment: &PathSegment) -> Vec<Node> {
    let mut nodes = vec![Node::Name(segment.ident.to_string())];
...
pub(crate) fn nodes_for_path(path: &Path) -> Vec<Node> {
    ...
    for segment in &path.segments {
        nodes.extend(nodes_for_path_segment(segment));
        nodes.push(Node::Punctuation("::"));
    }
    nodes.pop();
    nodes
...
fn nodes_for_bound_lifetimes(bound_lts: &BoundLifetimes) -> Vec<Node> {
    let mut nodes = vec![];
    nodes.extend_from_slice(&[Node::Keyword("for"), Node::Space, Node::Punctuation("<")]);
...
/// Use :rust:fn:`sphinx-rustdocgen::nodes::nodes_for_where_clause` to generate
/// nodes for it.
