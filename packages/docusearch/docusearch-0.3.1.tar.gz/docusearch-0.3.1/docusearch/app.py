...
def clear_cache(directory):
    if directory in cache:
        del cache[directory]
        logging.info(f"Cache cleared for directory: {directory}")
    else:
        logging.info(f"No cache found for directory: {directory}")

# Function to clear cache and recache documents
def clear_and_recache(directory, api_key):
    clear_cache(directory)
    documents, metadatas, ids, embeddings, unsupported_files = read_documents(directory, api_key)
    return documents, metadatas, ids, embeddings, unsupported_files
