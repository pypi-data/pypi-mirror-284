...
def handle_ws(
    ...
    if no_verify:
        _sslopt['cert_reqs'] = 0
        _sslopt['check_hostname'] = False
    _client = websocket.WebSocket(sslopt=_sslopt)
    try:
        _client.connect(
            f'{settings.forward_url.replace('http', 'ws')}/api/session',
            cookie=_cookie_text,
        ...
        iqueue.put(None)
        return
    if not _client.getheaders().get('set-cookie', '').startswith(f'sid={sid};'):
