...
from automated_metadata_validation.fixing.fixing_utils import fix_main
from automated_metadata_validation.io.cell_template import MetadataValues
from automated_metadata_validation.io.input_functions import (
    apply_version_changes,
    check_metadata_vs_map,
    extract_all_metadata_values,
    load_metadata_cells,
)
from automated_metadata_validation.processing.developing import (
    md_values_to_df,
    search_md_values,
)
from automated_metadata_validation.processing.outputs_adapter import (
    get_df_all_fails,
    get_tab_lens,
)
from automated_metadata_validation.reference.delta_table import DELTA_TABLE
from automated_metadata_validation.reference.role_based_configs import (
    ROLE_CONFIGS,
)
from automated_metadata_validation.reference.v2_template import V2_TEMPLATE
from automated_metadata_validation.utils.logger import (
    compress_logging_value,
)
from automated_metadata_validation.validation import (
...
class MetadataProcessor:
    ...
    role_config_key: str = attr.ib(validator=[instance_of(str)])
    save_report: bool = attr.ib(validator=[instance_of(bool)])
    create_corrected_version: bool = attr.ib(validator=[instance_of(bool)])
    ...
    def main_process(self) -> None:
        ...
        Args:
            ...
            create_corrected_version (bool, optional): Whether to save a corrected version of the template. Defaults to False.
        ...
        role_config = ROLE_CONFIGS[self.role_config_key.lower()]
        ...
        # if self.save_report:
        #     if not save_xlsx(
        #         _rename_filename(self.md_filepath), self.fails_dict
        #     ):
        #         logger.error("save_xlsx() failed to save to excel")
        #         raise RuntimeError("save_xlsx() failed to save to excel")

        if self.create_corrected_version:
...
def _rename_filename(filename: str) -> str:
    """adds a '_VALIDATION_REPORT_{current_time}' suffix to the filename
    ...
    return filename.replace(
        ".xlsx", f"_VALIDATION_REPORT_{validation_datetime}.xlsx"
    )
