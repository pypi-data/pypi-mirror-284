...
class TestExecutionReport:
    ...
    def test_streaming_response_get_quote_execution_report(self, client: Cadenza) -> None:
        with client.trading.execution_report.with_streaming_response.get_quote_execution_report(
            quote_request_id="string",
        ) as response:
...
class TestAsyncExecutionReport:
    ...
    async def test_streaming_response_get_quote_execution_report(self, async_client: AsyncCadenza) -> None:
        async with async_client.trading.execution_report.with_streaming_response.get_quote_execution_report(
            quote_request_id="string",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
