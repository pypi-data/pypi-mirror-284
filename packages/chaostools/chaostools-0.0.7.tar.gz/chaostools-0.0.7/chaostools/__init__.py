...
def bdbk(word, id=None, mu_first=False):
    ...
    for content_item in bdbk_contents:
        bdbk_text = bdbk_bs4.find_all(class_="J-lemma-content")[0].get_text().replace("播报编辑", "").replace(content_item, "\n"+content_item+"\n")
...
def calories(kind, appId, appSecret, key="苹果", foodId="befa2163948534a9", page=1, id=1):
    ...
    if kind == "type/list":
        return str(requests.get(calories_baseurl + "?app_id=%s&app_secret=%s"%(appId, appSecret)).content, encoding="utf-8")
    elif kind == "food/list":
        return str(requests.get(calories_baseurl + "?id=%d&page=%d&app_id=%s&app_secret=%s"%(id, page, appId, appSecret)).content, encoding="utf-8")
    elif kind == "food/search":
        return str(requests.get(calories_baseurl + "?keyword=%s&page=%d&app_id=%s&app_secret=%s"%(key, page, appId, appSecret)).content, encoding="utf-8")
    elif kind == "food/details":
        return str(requests.get(calories_baseurl + "?foodId=%s&app_id=%s&app_secret=%s"%(foodId, appId, appSecret)).content, encoding="utf-8")