...
async def on_message(message):
    ...
    if(MESSAGE["metadata"]["message_type"] == "session_welcome"):
        tp.TWITCH_EVENTSUB_TICKET = MESSAGE["payload"]["session"]["id"]
        for i in tp.EVENT_SUBSCRIPTION:
            headers = {
                "Content-Type": "application/json",
                "Client-ID": tp.TWITCH_CLIENT_ID,
                "Authorization": f"Bearer {tp.TWITCH_ACCESS_TOKEN}"
            }
            data = {
                "type": i["type"],
                "version": i["version"],
                "transport": {
                    "method": "websocket",
                    "session_id": tp.TWITCH_EVENTSUB_TICKET
                }
            }
            ...
            if conditions:
                ...
                for condition in conditions:
                    data["condition"][condition] = tp.TWITCH_USER_ID
            try:
...
def initEventSub(userid):
    ...
    tp[userid]["TWITCH_ACCESS_TOKEN"] = dat.get("access_token")
    tp[userid]["TWITCH_CLIENT_ID"] = memory.app_id
    ...
    dat = overwrites.get_user(userid)
    tp[userid]["EVENT_SUBSCRIPTION"] = dat.get("event_subscription")
...
async def actualInit(userid):
        ...
        async with websockets.connect('wss://eventsub.wss.twitch.tv/ws') as ws:
            tp[userid]["ws"] = ws
            try:
                tp.EVENT_SUBSCRIPTION = tp[userid]["EVENT_SUBSCRIPTION"]
                tp.TWITCH_ACCESS_TOKEN = tp[userid]["TWITCH_ACCESS_TOKEN"]
                tp.TWITCH_CLIENT_ID = tp[userid]["TWITCH_CLIENT_ID"]
                tp.TWITCH_USER_ID = userid
                while True:
