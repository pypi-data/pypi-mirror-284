...
# L = 0.59 * zref#70  # length scale
# Gamma = 3.9  # time scale
# sigma = 3.2 * ustar**2.0 / zref ** (2.0 / 3.0) #0.04  # magnitude (σ = αϵ^{2/3})
...
pb = CalibrationProblem(
    ...
    prob_params=ProblemParameters(
        data_type=DataType.CUSTOM, tol=1e-9, nepochs=5, learn_nu=False
    ),
...
f = CustomData[:, 0]
k1_data_pts = 2 * torch.pi * f / Uref * zref
