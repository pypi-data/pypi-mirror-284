...
from networks import create_sppmi_mtx, construct_w_pkn, create_sppmi_mtx_torch, construct_w_pkn_torch, opt_p_torch, \
    wshrink_obj_torch
from networks import solve_l1l2, opt_p, wshrink_obj
...
def st_acn_gpu(expression, spatial_network, lamb=0.001, dim=200, device='cpu'):
    expression = expression.T
    spatial_network = spatial_network.T
    data = [spatial_network, expression]
    W = [None] * 2
    # data normalization
    # for i in range(2):
    #     data[i] = data[i] / torch.sqrt(torch.sum(data[i] ** 2, dim=0)).unsqueeze(0).expand_as(data[i])
    W[0] = create_sppmi_mtx(data[0], 1)
    W[1] = create_sppmi_mtx(construct_w_pkn(data[1], 20), 1)
    ...
    for iter_ in pbar:
        ...
        for i in range(V):
            ...
            E = mnmstpy.solve_l1_l2_torch(B, lamb / mu)
            Eh[i] = E[d: (i + 1) * dim, :]
            d += dim
        ...
        if max_err <= thresh:
            pbar.set_description(f'[st_acn] Iter {iter_}: max_err={max_err:.4e} < thresh={thresh:.4e}. Breaking...')
            break

        formatted_errs = [f'{err:.4e}' for err in errs]
        pbar.set_description(f'[st_acn] Iter {iter_}: err={formatted_errs}, max_err={max_err:.4e}, thresh={thresh:.4e}')
