...
class CFTurnstileSolver:
    ...
    def init_and_start_browser(self):
        ...
        self.browser = self.playwright.firefox.launch(**browser_args)
    ...
    def solve_visible(self):
        iframe = self.page.query_selector("div[class='cf-turnstile-wrapper']")
        while not iframe:
            iframe = self.page.query_selector("div[class='cf-turnstile-wrapper']")
            time.sleep(0.1)
    ...
    def solve(self, auto_close=True):
        self.init_and_start_browser()
        self.target_url = self.target_url + "/" if not self.target_url.endswith("/") else self.target_url
        self.context = self.browser.new_context()
        self.page = self.context.new_page()
        self.build_page_data_v2()
        self.page.route(self.target_url, lambda route: route.fulfill(body=self.html, status=200))
