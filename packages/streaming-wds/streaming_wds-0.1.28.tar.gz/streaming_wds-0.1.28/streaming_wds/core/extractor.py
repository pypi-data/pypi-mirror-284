...
class ShardExtractor:
        ...
        with self.cache.with_shard_lock(shard_key):
            try:
                ...
                while (
                    ...
                    if not hasattr(self, "_logged_memory_warning"):
                        logger.warning(
                            "Exceeded memory buffer quota. Slowing down. Consider increasing memory_buffer_limit_bytes."
                        )
                        self._logged_memory_warning = True
                    time.sleep(0.01)
    ...
    def run_worker(self, rank: int):
        ...
        while True:
            ...
            for sample in self.extract_shard(shard_idx, shard_key, rank):
                if sample is None:
                    logger.debug("Sample is None. Breaking...")
                    break
    ...
    def run(self, num_threads: int, *, rank: int) -> None:
        ...
        while not (self.stop_event.is_set() or self.input_finish_event.is_set()):
            time.sleep(0.01)
