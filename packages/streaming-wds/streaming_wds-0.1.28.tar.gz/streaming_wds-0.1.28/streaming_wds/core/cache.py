...
class LocalShardLRUCache:
    ...
    def remove_shard(self, shard_key: str) -> bool:
        ...
        with self._cache_lock:
            ...
            self.shard_access_times.pop(shard_key)
            self.shard_locks.pop(shard_key)
            self.shard_read_count.pop(shard_key)
    ...
    def _evict_oldest_shard(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            shard_key = min(eligible_shards, key=self.shard_access_times.get)
            shard_path = self.get_shard_path(shard_key)
            shard_size = shard_path.stat().st_size
            ...
            logger.debug(f"[{self.worker_rank}] Evicting shard {shard_key} from cache")
