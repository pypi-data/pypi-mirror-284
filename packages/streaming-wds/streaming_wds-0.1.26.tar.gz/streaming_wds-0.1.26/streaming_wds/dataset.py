...
class StreamingWebDataset(IterableDataset):
    ...
    def prepare_worker_components(self):
        ...
        if self._worker_components_initialized:
            ...
            self.downloader.populate_download_queue(
                self.global_rank, self._worker_infos, self.epoch, self.shuffle
            )
            return
    ...
    def __iter__(self):
        ...
        self._workers = [
            threading.Thread(
                ...
                kwargs=dict(worker_info=self._worker_infos[self.global_rank]),
            ),
            threading.Thread(
