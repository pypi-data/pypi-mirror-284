    ...
    def do_GET(self):
        ...
        if parts[2].strip("/").endswith("oauth/callback"):
            ...
            content = DEFAULT_PAGE.format(
                config=json.dumps(
                    {"apiServerUrl": self.server.cloud_base_url, "whiteLabel": False}
                )
            ).encode('UTF-8', 'replace')
            self.send_response(HTTPStatus.OK)
            self.send_header("Content-type", "text/html")
...
def get_token(
    cloud_base_url: str, port: int | None = None, logger: logging.Logger = logger
) -> tuple[str, str] | None:
    """Get the user handle and token."""

    server_address = ("", port or find_http_port())
    httpd = DualStackServer(server_address, LoginRequestHandler, cloud_base_url)
...
if __name__ == "__main__":
    ...
    cloud_base_url = ("https://dev1.datalayer.run",)
    if len(argv) == 2:
        ans = get_token(cloud_base_url, port=int(argv[1]))
    else:
        ans = get_token(cloud_base_url)
