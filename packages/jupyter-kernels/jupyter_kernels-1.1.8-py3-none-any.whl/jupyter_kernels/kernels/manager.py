...
class KernelManager(LoggingConfigurable):
    ...
    def __init__(self, cloud_base_url: str, cloud_token: str, username: str, **kwargs):
    ...
    def start_kernel(self, **kwargs):
        ...
        if kernel_name:
            response = fetch(
                "{}/api/jupyter/v1/kernel/{}".format(self.cloud_base_url, kernel_name),
            ...
            response = fetch(
                "{}/api/jupyter/v1/kernels".format(self.cloud_base_url),
                token=self.cloud_token,
            )
            kernels = response.json().get("kernels", [])
            if len(kernels) == 0:
                raise RuntimeError(
                    "No remote kernel running. Please start one kernel using `jupyter kernel create <SPEC ID>`."
                )
            kernel = kernels[0]
