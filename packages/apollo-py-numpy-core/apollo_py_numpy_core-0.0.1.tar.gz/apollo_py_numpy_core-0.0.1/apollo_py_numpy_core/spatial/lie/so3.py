...
class LieGroupSO3(Rotation3):
    def __new__(cls, input_array: Union[List[List[float]], np.ndarray]) -> 'LieGroupSO3':
        return super().__new__(cls, input_array).view(cls)
...
class LieAlgSO3(M3):
    def __new__(cls, input_array: Union[List[List[float]], np.ndarray]) -> 'LieAlgSO3':
        return super().__new__(cls, input_array).view(cls)
    ...
    def exp(self) -> 'LieGroupSO3':
        ...
        rotation_matrix = np.eye(3) + p * self + q * (self @ self)
